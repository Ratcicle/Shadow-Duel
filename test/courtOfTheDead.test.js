...
function makeMonster(id, name, owner) {
  return makeCard(
    {
      id,
      name,
...
test("Court of the Dead declares separated text and pays counters before targeting", () => {
  ...
  const counterEffect = card.effects.find(
  ...
  assert.equal(
    reviveEffect.actions.some((action) => action.type === "remove_counter"),
    false,
  );
  assert.equal(reviveEffect.targets[0].owner, "any");
  assert.equal(reviveEffect.actions[0].scope, "both");
...
test("Court gains counters from either Graveyard and revives after paying eight", async (t) => {
  ...
  const ownMonster = makeMonster(99140, "Own Funeral Counter source", game.player);
