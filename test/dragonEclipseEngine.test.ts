...
function getEffect(cardId: number, effectId: string) {
  const effect = required(getDefinition(cardId).effects).find(
...
function simulatedCard(id: number, instanceId: number | string) {
...
test("Solar e Lunar declaram auto-banimento como custo e escolha na resolução", () => {
  ...
  for (const [cardId, effectId, zone] of [
    ...
    assert.equal(required(effect.actions).length, 1);
