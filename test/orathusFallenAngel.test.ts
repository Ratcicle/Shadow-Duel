...
function getEffect(id: string) {
  const effect = required(getOrathus().effects).find(
    (entry) => entry.id === id,
  );
...
function runtimeCard(data: CardConstructorData | undefined, owner: GamePlayer) {
...
function createGame(t: TestContext, options: { disableChains?: boolean } = {}) {
...
test("Orathus declara dados, arte, localização e materiais Sincro canônicos", (t) => {
  ...
  assert.equal(
    existsSync(
      new URL(
        "../public/assets/Orathus, The Fallen Angel.png",
        import.meta.url,
      ),
    ),
    true,
  ...
  const target = completeCard({ ...card, instanceId: "orathus-extra" });
...
test("Invocação correta é estabelecida somente após Sincro bem-sucedida e reinicia no Deck Adicional", async (t) => {
  ...
  assert.ok(blockedRevival.success === false);
...
test("Trigger de Sincro nega qualquer card face-up enquanto ele permanecer face-up", async (t) => {
  ...
  assert.ok(applied.success === true);
...
test("filtro de última origem distingue Deck Adicional de revival no runtime e na simulação", () => {
  const fromExtra = {
    ...
    lastSummonedFromZone: "extraDeck" as const,
  };
  const revived = {
    cardKind: "monster" as const,
    monsterType: "synchro" as const,
    lastSummonedFromZone: "graveyard" as const,
  };
  const filter = {
    cardKind: "monster" as const,
    lastSummonedFromZone: "extraDeck" as const,
...
test("compromisso da ativação proíbe ataque antes do Chain Link e é serializado", async (t) => {
  ...
  assert.ok(committed.success === true);
  ...
  assert.ok(link);
  assert.deepEqual(required(required(link.activationCommitment).actions), [
...
test("Ignition rejeita Orathus que já atacou e simulação aplica a restrição de compromisso", (t) => {
  ...
  assert.ok(condition.ok === false);

  const simSource = simulationCard({
    id: ORATHUS_ID,
  ...
  assert.equal(
    required(publicState.players.self.field[0]).properSummonEstablished,
    true,
  );
  assert.equal(
    required(publicState.players.self.field[0]).lastSummonedFromZone,
    "extraDeck",
  );
  assert.equal(
    required(replayState.players.player.zones.field[0]).properSummonProcedure,
    "synchro",
  );
});
