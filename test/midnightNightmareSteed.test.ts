...
function getEffect(id: string) {
  const effect = required(getDefinition(CARD_NAME).effects).find(
    (entry) => entry.id === id,
  );
...
function makeFiller(id: number, owner: GamePlayer) {
  return makeCard(
    {
      id,
...
function createGame(
  t: TestContext,
  { phase = "main1" }: { phase?: RuntimeGame["phase"] } = {},
) {
  ...
  game.chainSystem.offerChainResponses = async () => {
    ...
    if (link?.effectId === SUMMON_EFFECT_ID) {
      observedAtResponse =
        ...
        selectedCards(
          link.costSelections,
          "midnight_nightmare_steed_tribute_cost",
        )[0] === tribute;
    }
    return {
      lastActivator: null,
