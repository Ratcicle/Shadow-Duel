...
import {
  BANLIST_STATUS,
  CURRENT_BANLIST,
} from "../src/data/banlist.js";
import { cardDatabase, cardDatabaseById } from "../src/data/cards.js";
...
test("Monster Reborn is the initial limited card", () => {
  ...
  assert.equal(
    getBanlistStatus(MONSTER_REBORN_ID),
    BANLIST_STATUS.LIMITED,
  );
...
test("copy limits cover forbidden, limited, semi-limited and unlimited cards", () => {
  ...
  assert.equal(
    getCardCopyLimit(1, { deckType: DECK_TYPES.MAIN, banlist }),
    0,
  );
  assert.equal(
    getCardCopyLimit(2, { deckType: DECK_TYPES.MAIN, banlist }),
    1,
  );
  assert.equal(
    getCardCopyLimit(3, { deckType: DECK_TYPES.MAIN, banlist }),
    2,
  );
  assert.equal(
    getCardCopyLimit(4, { deckType: DECK_TYPES.MAIN, banlist }),
    3,
  );
  assert.equal(
...
test("banlist definition validation rejects unknown ids and statuses", () => {
  assert.equal(validateBanlistDefinition(cardDatabase).ok, true);
...
test("saved decks preserve old excess copies while generated decks obey the banlist", () => {
  ...
  assert.equal(
    toppedUp.filter((id) => id === MONSTER_REBORN_ID).length,
    1,
  );
});
...
test("all bot presets comply with the central copy limits", () => {
  for (const preset of Bot.getAvailablePresets()) {
    ...
    const deckIds = bot.deck.map((card) => card.id);
    const extraDeckIds = bot.extraDeck.map((card) => card.id);
    assert.equal(
      validateDeckAgainstBanlist({
        deck: deckIds,
  ...
  try {
    const controller = createDeckBuilderController({
      ...
      deckState: {
        ...
        getCurrentExtraDeck: () => [],
        saveDeck: () => {
          if (!allowSave) assert.fail("invalid deck must not be saved for play");
        },
        saveExtraDeck: () => {
          if (!allowSave) {
    ...
    invalidDeck.push(
      validDeck.filter((id) => id !== MONSTER_REBORN_ID).at(-1),
    );
    allowSave = true;
...
test("Monster Reborn locale and restricted badge styles are updated", () => {
  ...
  assert.equal(
    cardDatabaseById.get(MONSTER_REBORN_ID).description,
