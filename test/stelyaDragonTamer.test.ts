...
function getEffect(id: string) {
  const effect = required(getStelya().effects).find((entry) => entry.id === id);
...
function simulatedCard(
  ...
  return simulationCard({ ...structuredClone(definition), instanceId });
}

function makeSimulationState({
  stelyaZone = "hand",
  secondStelya = false,
} = {}) {
  ...
  const fieldCost = simulationCard({
    id: 99002,
  ...
  const searchTarget = simulationCard({
    id: 99003,
...
test("Stelya declara o novo texto e um único limite compartilhado", () => {
  ...
  assert.equal((required(card.description).match(/●/g) || []).length, 2);
  ...
  assert.equal(
    new Set(effects.map((effect) => effect.oncePerTurnName)).size,
    1,
  );
...
test("qualquer efeito da Stelya bloqueia os outros até o próximo turno", async (t) => {
  for (const firstEffectId of Object.values(EFFECT_IDS)) {
    await t.test(firstEffectId, () => {
      ...
      const card = new Card(getStelya(), "player");
      card.instanceId = unsafeFixture<number>(
        `runtime-${firstEffectId}`,
        "Legacy usage fixture identifies each copy with a string ID.",
      );
      ...
      for (const effectId of Object.values(EFFECT_IDS)) {
        ...
        assert.equal(
          check.ok,
          false,
          `${firstEffectId} must block ${effectId}.`,
        );
...
test("a política use mantém a escolha consumida após negar a ativação", (t) => {
  ...
  card.instanceId = unsafeFixture<number>(
    "negated-stelya",
    "Legacy usage fixture preserves its string identity.",
  );
  ...
  assert.ok(reservation && "reservationId" in reservation);
...
test("IA e simulação tratam Invocação e busca como a mesma escolha", (t) => {
  ...
  const game = createGame(t);
  const runtimeCard = new Card(getStelya(), "player");
  runtimeCard.instanceId = unsafeFixture<number>(
    "ai-stelya",
    "Legacy usage fixture preserves its string identity.",
  );
  game.reserveEffectUsage({
  ...
  assert.equal(
    searchFirst.bot.graveyard.some((card) => card.id === STELYA_ID),
    true,
  );
  assert.equal(
    searchFirst.bot.hand.some((card) => card.name === "Searchable Dragon"),
    true,
  );
  ...
  assert.equal(
    summonFirst.bot.field.some((card) => card.id === STELYA_ID),
    true,
  );
