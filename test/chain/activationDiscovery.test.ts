...
function createResponseContext(bot: ChainPlayer): FastEffectContextInput {
  return {
    type: "phase_change",
    event: "phase_end",
    player: bot,
...
test("rótulo do modal distingue efeitos da mesma carta", () => {
  assert.equal(
    getEffectDisplayLabel(
      Object.assign({}, { id: "first", activationLabel: "Primeiro efeito" }),
    ),
    "Primeiro efeito",
  );
  assert.equal(
    getEffectDisplayLabel(
      unsafeFixture<Parameters<typeof getEffectDisplayLabel>[0]>(
        { id: "second" },
        "Unlabelled legacy effect must use the generic modal fallback.",
      ),
    ),
    "effect",
  );
