...
function createPhaseResponse(chain: ChainSystem, controller: ChainPlayer) {
...
test("[CS-11] início de fase deixa o estado aberto para o jogador do turno", async () => {
  ...
  assert.equal(required(result.state).timingWindowId, null);
});
...
test("passe do oponente autoriza phase_end sem segunda oferta", async () => {
  ...
  chain.offerChainResponse = async (responder) => {
    offered.push(required(responder).id);
    return null;
  };
...
test("nextPhase preserva a fase e o Battle Step quando a intenção foi interrompida", async () => {
  ...
  const result = record(
    await nextPhase.call(
      unsafeFixture<ThisParameterType<typeof nextPhase>>(
        game,
        "Interrupted phase fixture implements only capabilities before the phase transition.",
      ),
    ),
  );
