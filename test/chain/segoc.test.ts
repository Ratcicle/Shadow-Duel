...
import type {
  ChainConfirmPromptOptions,
  ChainEventPayload,
  ChainPlayer,
  ChainSourceSnapshot,
  ChainTriggerEntry,
  ChainTriggerOrderModalOptions,
...
function activatedEffects(trace: HarnessTrace) {
  return trace.events
    .filter(
      (entry) =>
        entry.channel === "emit" && entry.eventName === "effect_activated",
    )
    .map((entry) => required(entry.payload).effectId);
}
...
test("[CS-01] triggers simultÃ¢neos formam uma Ãºnica corrente na ordem SEGOC", async () => {
  ...
  const prepared = required(
    trace.events.find((entry) => entry.eventName === "trigger_chain_prepared"),
  );
  assert.equal(required(prepared.payload).preparedCount, 4);
});

test("[CS-01] jogador escolhe a ordem dos efeitos dentro do prÃ³prio grupo", async () => {
  const modalCalls: Array<
    Pick<ChainTriggerOrderModalOptions, "candidates" | "optional">
  > = [];
...
test("[CS-09] trigger pendente revalida a localizaÃ§Ã£o antes de entrar na corrente", async () => {
  ...
  assert.equal(
    required(rejected.payload).rejectionReason,
...
test("[CS-09] When opcional perde timing quando o evento nÃ£o foi o Ãºltimo relevante", async () => {
  ...
  const result = await chain.resolveTriggerOccurrences([
    ...
    occurrence(chain, [], {
      atomicGroupId: laterGroup,
      eventName: "later_event",
    }),
  ]);
  ...
  const rejected = required(
    trace.events.find(
      (event) =>
        ...
        required(event.payload).effectId === "optional_when_effect",
    ),
  );
  assert.equal(
    required(rejected.payload).rejectionReason,
    "optional_when_missed_timing",
  );
});
...
test("modal opcional seleciona subconjunto e modal obrigatÃ³rio nÃ£o permite omissÃ£o", async () => {
  ...
  await chain.collectTriggerCandidates(required(opportunity));
  const invalid = await chain.prepareTriggerOpportunity(required(opportunity));
  assert.ok(invalid.ok === false);
  ...
  const run = async () => {
    ...
    return {
      ...
      ids: trace.events
        ...
        .flatMap((event) => required(event.payload).orderedCandidateIds),
    };
  };
...
test("estado e eventos de SEGOC sÃ£o serializÃ¡veis", async () => {
  ...
  const triggerEvents = trace.events.filter(
    (event) =>
  ...
  assert.doesNotThrow(() =>
    JSON.stringify(game.getPublicState!(player.id).chain.triggers),
  );
});
...
test("cancelamento limpa fila e seleÃ§Ã£o sem reutilizar IDs", () => {
  ...
  chain.pendingTriggerSelection = unsafeFixture<
    NonNullable<typeof chain.pendingTriggerSelection>
  >(
    { opportunityId: 1 },
    "Cancellation clears a legacy pending-selection shape without reading its fields.",
  );
  chain.cancelChain();
