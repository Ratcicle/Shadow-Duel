...
function createSummonGame(t: TestContext) {
...
test("[CS-03] Normal Set não abre summon_attempt nem after_summon", async (t) => {
  ...
  const timingOrigins: Array<FastEffectOrigin | undefined> = [];
  const afterSummons: Array<RuntimeEventMap["after_summon"]> = [];
  const monsterSets: Array<RuntimeEventMap["monster_set"]> = [];
  const originalTiming = game.chainSystem.runFastEffectTiming.bind(
    game.chainSystem,
  );
  game.chainSystem.runFastEffectTiming = async (input) => {
    timingOrigins.push(required(input).origin);
  ...
  const result = required(
    await game.performNormalSummon(game.player, 0, "defense", true),
  );
...
test("[CS-03] Flip Summon abre janela de negação", async (t) => {
  ...
  const afterSummons: Array<RuntimeEventMap["after_summon"]> = [];
  const moved: Array<RuntimeEventMap["card_moved"]> = [];
  const originalTiming = game.chainSystem.runFastEffectTiming.bind(
    game.chainSystem,
  );
  ...
  game.chainSystem.openChainWindow = async (context, options) => {
    if (required(context).type === "summon_attempt") {
      assert.equal(required(options).firstPlayer, game.player);
    }
    return await originalOpenWindow(context, options);
  };
  game.chainSystem.runFastEffectTiming = async (input) => {
    if (required(input).origin === FAST_EFFECT_ORIGINS.SUMMON_ATTEMPT) {
      attemptWindows += 1;
      assert.equal(game.player.field.includes(card), false);
      assert.equal(card.locationVersion, 0);
      assert.equal(required(required(input).context).summonId, 1);
    }
    return await originalTiming(input);
  };
  game.on("after_summon", (payload) => afterSummons.push(payload));
...
test("[CS-03] Tribute Summon negada mantém Tributos pagos e consome a tentativa", async (t) => {
  const game = createSummonGame(t);
  ...
  game.offerSummonAttempt = async (card, player, options) => {
    ...
    game.markSummonNegated(transaction.summonId, {
      ...
      destroyed: true,
      sourceCard: createMonster(game.bot, { id: 9999, name: "Summon Negator" }),
    });
  ...
  assert.equal(moved.filter((payload) => payload.card === summoned).length, 1);
  assert.equal(required(game.getSummonState().last).status, "negated");
  assert.equal(game.activeSummonTransaction, null);
});

test("[CS-03] Invocação produzida durante resolução não abre janela aninhada", async (t) => {
  const game = createSummonGame(t);
  ...
  game.on("after_summon", (payload) => afterSummons.push(payload));
  Reflect.set(game.chainSystem, "chainResolving", true);
  ...
  Reflect.set(game.chainSystem, "chainResolving", false);
  ...
  assert.equal(
    required(game.getSummonState().last).summonOrigin,
    "effect_resolution",
  );
  assert.equal(required(game.getSummonState().last).status, "succeeded");
});
...
test("sucesso move a carta e publica after_summon exatamente uma vez", async (t) => {
  ...
  const moved: Array<RuntimeEventMap["card_moved"]> = [];
  const afterSummons: Array<RuntimeEventMap["after_summon"]> = [];
  const transactionEvents: Array<RuntimeEventMap["summon_transaction"]> = [];
  game.on("card_moved", (payload) => moved.push(payload));
  game.on("after_summon", (payload) => afterSummons.push(payload));
  ...
  game.chainSystem.runFastEffectTiming = async (input) => {
    if (required(input).origin === FAST_EFFECT_ORIGINS.SUMMON_ATTEMPT) {
      ...
      assert.equal(
        required(required(publicSummon.transaction).card).name,
        card.name,
      );
      assert.equal(
        required(publicSummon.transaction).status,
        "awaiting_negation",
      );
    }
    return await originalTiming(input);
...
test("múltiplos Tributos são pagos sem janela intermediária", async (t) => {
  ...
  game.offerSummonAttempt = async (card, player, options) => {
    ...
    return {
      ok: true,
      transaction: required(required(options).summonTransaction),
      ownsTransaction: false,
    };
  };

  const result = required(
    await game.performNormalSummon(game.player, 0, "attack", false, [0, 1]),
  );

  assert.ok(result.success === true);
  assert.equal(attempts, 1);
  assert.deepEqual(game.player.field, [summoned]);
  assert.equal(required(result.transaction).costs.length, 2);
  assert.ok(required(result.transaction).costs.every((cost) => cost.paid));
});
...
test("handler de negação marca a transação exata sem mover a carta", async (t) => {
  ...
  assert.ok(begun.ok === true);
  const transaction = required(begun.transaction);
  const moveSpy = t.mock.method(game, "moveCard");
  const context: ActionNegationContext & { type: string } = {
    type: "summon_attempt",
  ...
  const handled = await handleNegateSummonOrActivationAndDestroy(
    ...
    {},
    unsafeFixture<ActionHandlerEnginePort>(
      { game },
      "This negation-handler fixture uses only the real game port; other EffectEngine capabilities are not reached.",
    ),
  );

  assert.equal(handled, true);
  assert.equal(moveSpy.mock.callCount(), 0);
  assert.equal(transaction.status, SUMMON_STATUSES.NEGATED);
  assert.equal(required(transaction.negationOutcome).destroyed, true);
  assert.equal(required(transaction.negationOutcome).sourceCard, negator);
...
test("erro pós-commit não reembolsa custos e limpa a tentativa", async (t) => {
  ...
  assert.equal(summoned.locationVersion, 1);
  assert.equal(required(game.getSummonState().last).status, "failed");
  assert.equal(game.activeSummonTransaction, null);
...
test("moveCard rejeita Invocação sem summonOrigin explícito", async (t) => {
  const game = createSummonGame(t);
  const card = createMonster(game.player, { name: "Explicit origin" });
  game.player.hand.push(card);
  const nextSummonId = game.nextSummonId;
  const invalidMove = unsafeFixture<
    (
      card: Card,
      player: GamePlayer,
      zone: "field",
      options: { fromZone: "hand" },
    ) => ReturnType<RuntimeGame["moveCard"]>
  >(
    game.moveCard,
    "This negative test deliberately omits the required summonOrigin.",
  );
  const result = await invalidMove.call(game, card, game.player, "field", {
    fromZone: "hand",
  });
  ...
  assert.equal(Reflect.get(game, "resolveSummonOrigin"), undefined);
});
...
test("IDs de Invocação são monotônicos e isolados por Game", async (t) => {
  ...
  Reflect.set(first.chainSystem, "chainResolving", true);
  Reflect.set(second.chainSystem, "chainResolving", true);

  const summonByEffect = async (game: RuntimeGame, id: number) => {
  ...
  assert.ok(
    "summonId" in firstResult &&
      "summonId" in secondResult &&
      "summonId" in isolatedResult,
  );
  assert.deepEqual(
