...
import type {
  AIAction, AIPlannedAction, AIPlanningContext, AIPlanningProfile, AIState,
  AIStrategyBotPort, AITributeRequirement, AILineMilestoneScore,
  StrategyRuntimePort, PositionChangeAIAction,
} from "../contracts/ai.js";
import type {
  AiStateShape, SimulatedCardState, SimulatedPlayerState,
  SimulationGameState, GameTreeSimulationGameState, PerspectiveGameState,
} from "../contracts/aiState.js";

type StrategySimulation = SimulationGameState | GameTreeSimulationGameState | PerspectiveGameState;
interface StrategyEvaluationContext {
  ...
  myLP?: number;
  oppLP?: number;
  oppField?: readonly SimulatedCardState[];
}

export default class BaseStrategy implements StrategyRuntimePort {
  declare bot: AIStrategyBotPort;

  constructor(bot: AIStrategyBotPort | null) {
    // Board-scoring singletons have no actor; actor-dependent hooks receive a Bot.
    this.bot = bot!;
  }
  ...
  think(_thought: string) {}

  getPlanningProfile(_game: AIState, _context: AIPlanningContext = {}): AIPlanningProfile {
    return {
      enabled: false,
      mode: "off",
      turnMode: "mainOnly",
  ...
  scoreLineMilestones(_context: AIPlanningContext = {}): AILineMilestoneScore {
    return {
      scoreDelta: 0,
      milestones: [],
    };
  ...
  describePlannedLine(context: AIPlanningContext = {}) {
    ...
    return sequence
      .map((action) => {
        const card =
          (action as AIAction)?.card?.name || (action as AIAction)?.cardName || (action as AIAction)?.name || (action as AIAction)?.index;
   ...
   * @returns {number}
   */
  evaluateMyMonster(monster: SimulatedCardState | null | undefined, context: StrategyEvaluationContext) {
    if (!monster) return 0;
   ...
   * @returns {string|null}
   */
  getMainArchetype(player: SimulatedPlayerState | null | undefined) {
  ...
  generateMainPhaseActions(_game: AIState): AIAction[] {
    return [];
  }
  ...
  simulateMainPhaseAction(state: StrategySimulation, action: AIPlannedAction): StrategySimulation | void {
    return state;
  }
  ...
  selectBestTributes(_field: SimulatedCardState[], _tributesNeeded: number, _cardToSummon: SimulatedCardState): number[] {
    return [];
  }
   ...
   * @returns {Array}
   */
  getPositionChangeActions(game: AIState & { canChangePosition?(card: SimulatedCardState): boolean }, bot: SimulatedPlayerState | null | undefined, opponent: SimulatedPlayerState | null | undefined) {
    const actions: PositionChangeAIAction[] = [];
    if (!bot || !opponent) return actions;

    const oppField = Array.isArray(opponent.field) ? opponent.field : [];
    const estimateOpponentStat = (card: SimulatedCardState) => {
    ...
    const canChangePosition = (card: SimulatedCardState) => {
   ...
   */
  getOpponent(gameOrState: AIState, perspectivePlayer: { id?: string } | null | undefined) {
  ...
  analyzeOpponentPosition(game: AIState) {
    try {
      const opponent = this.getOpponent(game, this.bot) || (game as AiStateShape).opponent;
      if (!opponent) return null;
  ...
  integrateP2IntoActionSelection(game: AIState, actions: AIAction[], analysis: unknown = null) {
    try {
      if (!actions || actions.length === 0) return actions;
      if (game?._isPerspectiveState === true || (game as AiStateShape)?._suppressP2Analysis) {
        return actions;
      }
