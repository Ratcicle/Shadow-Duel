...
function getBotState(state: Partial<AiStateShape> = {}): BurningWestReadPlayer {
  return (state.bot || state.player || {}) as BurningWestReadPlayer;
}

function getOpponentState(state: Partial<AiStateShape> = {}): BurningWestReadPlayer {
  return (state.player || state.opponent || {}) as BurningWestReadPlayer;
}

function getCards(player: BurningWestReadPlayer | null | undefined, zone: BurningWestArrayZone | "fieldSpell") {
  if (!player) return [];
  if (zone === "fieldSpell") return player.fieldSpell ? [player.fieldSpell] : [];
  return asArray(player[zone]);
}

function cardArchetypes(card: BurningWestCard) {
  if (!card) return [];
  if (Array.isArray(card.archetypes)) return card.archetypes;
  return card.archetype ? [card.archetype] : [];
}

function isBurningWest(card: BurningWestCard) {
  return cardArchetypes(card).includes(ARCHETYPE);
}

function mentionsBurningWest(card: BurningWestCard) {
  return isBurningWest(card) || String(card?.description || "").includes(ARCHETYPE);
}

function isBurningWestMonster(card: BurningWestCard) {
  return card?.cardKind === "monster" && isBurningWest(card);
}

function isFaceUpBurningWestMonster(card: BurningWestCard) {
  return isBurningWestMonster(card) && card.isFacedown !== true;
}

function isBurningWestSpellTrap(card: BurningWestCard) {
  return ["spell", "trap"].includes(card?.cardKind!) && mentionsBurningWest(card);
}

function hasName(cards: BurningWestCard[] = [], name: string | undefined) {
  return asArray(cards).some((card) => card?.name === name);
}

function countName(cards: BurningWestCard[] = [], name: string | undefined) {
  return asArray(cards).filter((card) => card?.name === name).length;
}

function countWhere(cards: BurningWestCard[] = [], predicate: (card: BurningWestCard) => boolean) {
...
function canDestroyByBattle(attacker: BurningWestCard, target: BurningWestCard) {
  return canAttack(attacker) && target?.cardKind === "monster" && getEffectiveAtk(attacker) > getBattleTargetStat(target);
}

function isExtraDeckMonster(card: Partial<BurningWestCard> = {}) {
  return ["fusion", "synchro", "ascension"].includes(card?.monsterType!);
}

function cardThreat(card: Partial<BurningWestCard> = {}) {
  if (!card || card.cardKind !== "monster") return 0;
  return (
...
function actionName(action: BurningWestLineAction = {}) {
  return action.cardName || action.card?.name || action.name || action.sourceName || "";
}

function sequenceNames(sequence: BurningWestLineAction[] = []) {
  return asArray(sequence).map(actionName).filter(Boolean);
}

function sequenceUses(sequence: BurningWestLineAction[] = [], predicate: (action: BurningWestLineAction) => boolean) {
  return asArray(sequence).some(predicate);
}

function addMilestone(entries: Required<Pick<BurningWestMilestone, "label" | "score" | "detail">>[], label: string, score: number, detail = "") {
  if (!Number.isFinite(score) || score === 0) return;
  entries.push({ label, score, detail });
}

function allOwnCards(player: BurningWestReadPlayer = {}) {
  return [
...
function countOpponentBoardCards(player: BurningWestReadPlayer = {}) {
  return (
...
function countFaceUpBurningWest(player: BurningWestReadPlayer = {}) {
  return countWhere(getCards(player, "field"), isFaceUpBurningWestMonster);
}

function countBurningWestCards(player: BurningWestReadPlayer = {}) {
  return allOwnCards(player).filter(mentionsBurningWest).length;
}

function countBurningWestSpellTrapsInHand(player: BurningWestReadPlayer = {}) {
  return countWhere(getCards(player, "hand"), isBurningWestSpellTrap);
}

function countBurningWestMonstersInHand(player: BurningWestReadPlayer = {}) {
  return countWhere(getCards(player, "hand"), isBurningWestMonster);
}

function hasFaceUpWanted(player: BurningWestReadPlayer = {}) {
  return getCards(player, "spellTrap").some(
    (card) => card?.name === BW.WANTED && card.isFacedown !== true,
  );
}

function hasRewardReady(player: BurningWestReadPlayer = {}) {
  return (
...
function hasPeacemakerAccess(player: BurningWestReadPlayer = {}) {
  return (
...
function hasExecutionerWindow(player: BurningWestReadPlayer = {}) {
  return (
...
function hasExecutionerOnField(player: BurningWestReadPlayer = {}) {
  return getCards(player, "field").some(
    (card) => card?.name === BW.EXECUTIONER && card.isFacedown !== true,
  );
}

function isExecutionerAscensionAction(action: BurningWestLineAction = {}) {
  return action?.type === "ascension" && actionName(action) === BW.EXECUTIONER;
}

function executionerAscensionActions(sequence: BurningWestLineAction[] = []) {
  return asArray(sequence).filter(isExecutionerAscensionAction);
}

function findAscensionMaterial(action: BurningWestLineAction = {}, player: BurningWestReadPlayer = {}) {
  const field = getCards(player, "field");
...
function materialRecoveryValue(card: BurningWestCard = {}) {
  switch (card?.name) {
    case BW.SPECIALIST:
      return 68;
    case BW.UNDERTAKER:
...
function hasSpecialistPeacemakerBattleValue(material: BurningWestCard = {}, player: BurningWestReadPlayer = {}, opponent: BurningWestReadPlayer = {}) {
...
function hasRelevantSheriffTypePressure(material: BurningWestCard = {}, player: BurningWestReadPlayer = {}, opponent: BurningWestReadPlayer = {}, turnCounter = 0) {
...
function hasUndertakerReviveValue(material: BurningWestCard = {}, player: BurningWestReadPlayer = {}) {
...
function isIdleExecutionerMaterial(material: BurningWestCard = {}, opponent: BurningWestReadPlayer = {}) {
...
function hasFavorableBattle(player: BurningWestReadPlayer = {}, opponent: BurningWestReadPlayer = {}) {
...
function hasLikelyBattlePayoff(analysis: Partial<BurningWestAnalysis> = {}, player: BurningWestReadPlayer = {}, opponent: BurningWestReadPlayer = {}) {
...
function hasBurningWestStarterLine(analysis: Partial<BurningWestAnalysis> = {}, player: BurningWestReadPlayer = {}) {
  const hand = getCards(player, "hand");
...
export function buildBurningWestPlanningProfile(analysis: Partial<BurningWestAnalysis> = {}, context: BurningWestContext = {}): BurningWestProfile {
  const game = (context.game || analysis.game || {}) as BurningWestGame;
  const player = (analysis.player || analysis.bot || context.bot || game.bot || {}) as BurningWestReadPlayer;
  const opponent = (analysis.opponent || context.opponent || game.player || {}) as BurningWestReadPlayer;
...
function getBattleSteps(sequence: BurningWestLineAction[] = []) {
  return asArray(sequence).filter((action) => action?.type === "simulatedBattle");
}

function battleRewardNames(sequence: BurningWestLineAction[] = []) {
  return getBattleSteps(sequence).flatMap((step) => asArray(step.rewardNames));
}

function battleDestroyedCards(sequence: BurningWestLineAction[] = []) {
  return getBattleSteps(sequence).flatMap((step) => asArray(step.destroyedCards));
}

function battleDamage(sequence: BurningWestLineAction[] = []) {
...
function countRemovedOpponentCards(initialOpponent: BurningWestReadPlayer = {}, finalOpponent: BurningWestReadPlayer = {}) {
  return Math.max(
    0,
...
function rewardMatches(rewards: string[] = [], pattern: RegExp) {
  return rewards.some((name) => pattern.test(String(name || "")));
}

function hasQuickDrawEffectRemoval(sequence: BurningWestLineAction[] = []) {
...
function isUnderPressure(player: BurningWestReadPlayer = {}, opponent: BurningWestReadPlayer = {}) {
...
function scoreBattleMilestones(entries: Required<Pick<BurningWestMilestone, "label" | "score" | "detail">>[], sequence: BurningWestLineAction[] = []) {
...
function scoreSetupMilestones(entries: Required<Pick<BurningWestMilestone, "label" | "score" | "detail">>[], context: BurningWestLineScoreContext = {}) {
...
function scorePenalties(entries: Required<Pick<BurningWestMilestone, "label" | "score" | "detail">>[], context: BurningWestLineScoreContext = {}) {
...
export function scoreBurningWestLineMilestones(context: BurningWestContext = {}) {
  const initialState: Partial<AiStateShape> = context.initialState || {};
  ...
  const entries: Required<Pick<BurningWestMilestone, "label" | "score" | "detail">>[] = [];
  const scoreContext = {
    sequence,
    initialBot,
    finalBot,
...
export function scoreBurningWestLineTerminal(context: BurningWestContext = {}) {
...
function compactActionStep(action: BurningWestLineAction = {}) {
  if (!action) return "";
...
function milestoneLabel(entry: BurningWestMilestone | string = {}) {
  if (typeof entry === "string") return entry;
  const score = Number(entry.score || 0);
  return `${score >= 0 ? "+" : ""}${score.toFixed(1)} ${entry.label || entry.reason || "milestone"}`;
}

export function describeBurningWestPlannedLine(context: BurningWestContext = {}) {
  const sequence = asArray(context.sequence);
