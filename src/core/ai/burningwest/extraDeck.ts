...
import type { GameCard, CardDeclaredValueDetail } from "../../contracts/cards.js";
...
function getCards(player: Player | null | undefined, zone: "fieldSpell" | "hand" | "field" | "spellTrap" | "extraDeck" | "graveyard") {
  if (!player) return [];
  if (zone === "fieldSpell") return player.fieldSpell ? [player.fieldSpell] : [];
  return asArray(player[zone]);
}

function cardArchetypes(card: ReadCard | null | undefined) {
  if (!card) return [];
...
function mentionsBurningWest(card: ReadCard | null | undefined) {
  return (
    isBurningWest(card) ||
...
function isBurningWestMonster(card: ReadCard | null | undefined): card is ReadCard {
  return card?.cardKind === "monster" && isBurningWest(card);
}

function isFaceUpBurningWestMonster(card: ReadCard | null | undefined): card is ReadCard {
  return isBurningWestMonster(card) && card.isFacedown !== true;
}

function isExecutioner(card: ReadCard | null | undefined) {
...
function canDestroyByBattle(attacker: ReadCard | null | undefined, target: ReadCard | null | undefined) {
  return (
    canAttack(attacker) &&
...
function sameCard(left: ReadCard | null | undefined, right: ReadCard | null | undefined) {
...
function hasWantedDeclaration(player: Player | null = {}, analysis: Analysis = {}) {
  ...
  return getCards(player, "spellTrap").some((card) => {
    const declaration = card?.declaredValues?.burning_west_wanted_type as CardDeclaredValueDetail | undefined;
...
function getEnteredTurn(material: ReadCard) {
  ...
  return values.length > 0 ? Math.max(...values as number[]) : null;
}

function isOldEnoughForSimulatedAscension(game: Game | null | undefined, material: ReadCard) {
...
function isValidExecutionerMaterial(material: ReadCard) {
  return (
...
function getSimulatedAscensionCandidates(game: Game | null | undefined, bot: AIStrategyBotPort, material: Card) {
  ...
  return getCards(bot, "extraDeck").filter(
    (card) => isExecutioner(card) && ascensionMaterialMatches(card as GameCard, material as GameCard),
  );
}

function getOpponentMonsters(analysis: Analysis = {}, opponent: Player | null = {}) {
...
function getOwnFaceUpBurningWest(analysis: Analysis = {}, bot: Player | null = {}) {
...
function isUnderPressure(analysis: Analysis = {}, bot: Player | null = {}, opponent: Player | null = {}) {
...
function needsExecutionerBody(ascensionCard: ReadCard, material: ReadCard, context: Context = {}) {
...
function hasBurningWestAttacker(analysis: Analysis = {}, bot: Player | null = {}) {
  return getOwnFaceUpBurningWest(analysis, bot).some(canAttack);
}

function hasAttackerWithoutPeacemaker(analysis: Analysis = {}, bot: Player | null = {}) {
  return getOwnFaceUpBurningWest(analysis, bot).some(
    (card) => canAttack(card) && !hasPeacemakerEquipped(card, bot),
  );
}

function hasDifficultBattleInteraction(analysis: Analysis = {}, bot: Player | null = {}, opponent: Player | null = {}) {
...
function hasBattlePayoff(analysis: Analysis = {}, bot: Player | null = {}, opponent: Player | null = {}) {
  return (
...
function monsterRecoveryScore(card: ReadCard | null | undefined) {
  switch (card?.name) {
    case BW.SPECIALIST:
      return 68;
    case BW.UNDERTAKER:
...
function scoreExecutionerRecoveryCard(card: ReadCard | null | undefined, analysis: Analysis = {}, context: Context = {}) {
  const bot = context.bot || analysis.player || {};
  const opponent = context.opponent || analysis.opponent || {};
...
export function rankBurningWestExecutionerRecoveryCandidates<Candidate extends ReadCard>(
  cards: Candidate[] = [],
  analysis: Analysis = {},
  context: Context = {},
) {
...
function getBestRecovery(bot: Player | null = {}, analysis: Analysis = {}, context: Context = {}) {
...
function getMaterialEffectActivations(game: Game | null | undefined, player: Player | null | undefined, material: ReadCard | null | undefined) {
...
function specialistMainValueUsed(bot: Player | null = {}) {
...
function materialHasProducedMainValue(material: ReadCard, context: Context = {}) {
...
function hasSpecialistPeacemakerAttackValue(material: ReadCard, context: Context = {}) {
...
function hasRelevantSheriffType(material: ReadCard, context: Context = {}) {
...
function hasBetterUndertakerRevive(material: ReadCard, context: Context = {}) {
...
function materialIsIdle(material: ReadCard, context: Context = {}) {
  const analysis = context.analysis || {};
  const opponent = context.opponent || analysis.opponent || {};
  return (
    !canAttack(material) ||
...
function evaluateExecutionerAscension(ascensionCard: ReadCard, material: ReadCard, context: Context = {}) {
  const analysis = context.analysis || {};
  const bot = context.bot || analysis.player || {};
  const opponent = context.opponent || analysis.opponent || {};
...
function getCachedEvaluation(ascensionCard: ReadCard, material: ReadCard, context: Context = {}) {
  if (
  ...
  return getGenericAscensionActions(
    {
      game,
      bot,
      opponent: analysis.opponent as AIStrategyBotPort | null | undefined,
      analysis,
      isSimulatedState,
    },
    {
...
export function selectBurningWestAutomaticAscension<Choice extends { ascensionCard: Card; material: Card }>({
  choices = [],
  game,
  bot,
  opponent,
  analysis,
}: AutomaticContext<Choice> = {}) {
  if (!Array.isArray(choices) || choices.length === 0) {
    return { skip: true, reason: "no Ascension choices" };
  }
  const resolvedAnalysis =
    analysis ||
    (bot?.strategy?.currentAnalysis as Analysis | null | undefined) ||
    {
      player: bot,
      opponent,
      game,
