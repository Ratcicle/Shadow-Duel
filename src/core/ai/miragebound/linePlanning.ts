import type { MirageboundCard, MirageboundPlayer, MirageboundAnalysis, MirageboundContext, MirageboundMilestone, MirageboundLineAction, MirageboundBattleContext } from "./contracts.js";
...
const MIRAGEBOUND_SPELL_TRAPS = new Set<string | null | undefined>([
  MB.OASIS,
  MB.MIRROR_PATH,
  MB.FALSE_HORIZON,
  MB.VANISHING_STEP,
...
const DEFENSIVE_CARD_NAMES = new Set<string | null>([
  MB.MIRROR_PATH,
  MB.FALSE_HORIZON,
  MB.VANISHING_STEP,
]);

function safeArray<Value>(value: Value[] | null | undefined): NonNullable<Value>[] {
  return Array.isArray(value) ? value.filter(Boolean) as NonNullable<Value>[] : [];
}
...
function actionName(action: MirageboundLineAction = {}) {
  return (
    action.cardName ||
    action.card?.name ||
...
function actionLabel(action: MirageboundLineAction = {}) {
...
function getBotState(state: Partial<import("../../contracts/aiState.js").AiStateShape>) {
  return (state?.bot || state?.player || {}) as Partial<MirageboundPlayer>;
}

function getOpponentState(state: Partial<import("../../contracts/aiState.js").AiStateShape>) {
  return (state?.player || state?.opponent || {}) as Partial<MirageboundPlayer>;
}

function getZone(player: Partial<MirageboundPlayer>, zone: "hand"|"field"|"graveyard"|"deck"|"extraDeck"|"spellTrap"|"banished"|"fieldSpell") {
  if (!player) return [];
  if (zone === "fieldSpell") return player.fieldSpell ? [player.fieldSpell] : [];
  return safeArray(player[zone]);
}

function getCards(player: Partial<MirageboundPlayer>, zones: Array<"hand"|"field"|"graveyard"|"deck"|"extraDeck"|"spellTrap"|"banished"|"fieldSpell"> = []) {
  return zones.flatMap((zone) => getZone(player, zone));
}

function countCards(player: Partial<MirageboundPlayer>, zones: Array<"hand"|"field"|"graveyard"|"deck"|"extraDeck"|"spellTrap"|"banished"|"fieldSpell">, predicate: (card:MirageboundCard)=>boolean = () => true) {
...
function hasCard(player: Partial<MirageboundPlayer>, zones: Array<"hand"|"field"|"graveyard"|"deck"|"extraDeck"|"spellTrap"|"banished"|"fieldSpell">, name: string) {
  return hasName(getCards(player, zones), name);
}

function isMonster(card: MirageboundCard | null | undefined): card is MirageboundCard {
  return card?.cardKind === "monster";
}

function isMiragebound(card: MirageboundCard) {
  return (
...
function isMirageboundMonster(card: MirageboundCard) {
  return isMonster(card) && isMiragebound(card);
}

function isBattleReady(card: MirageboundCard) {
  return (
    isMonster(card) &&
    !card.isFacedown &&
...
function totalVisibleAttack(field: MirageboundCard[] = []) {
...
function opponentThreatensLethal(bot: Partial<MirageboundPlayer> = {}, opponent: Partial<MirageboundPlayer> = {}) {
...
function cardIdentity(card: MirageboundCard, index = 0) {
  return (
    card?.instanceId ||
    card?._instanceId ||
    card?.uid ||
...
function pairCardsByIdentity(initialCards: MirageboundCard[] = [], finalCards: MirageboundCard[] = []) {
  const finalById = new Map<ReturnType<typeof cardIdentity>, MirageboundCard>();
...
function rawMonsterStats(card: MirageboundCard) {
  if (!isMonster(card)) return 0;
  return Math.max(0, Number(card.atk || 0)) + Math.max(0, Number(card.def || 0));
}

function totalRawMonsterStats(field: MirageboundCard[] = []) {
  return safeArray(field).reduce((sum, card) => sum + rawMonsterStats(card), 0);
}

function countOpponentCards(player: Partial<MirageboundPlayer> = {}) {
  return countCards(player, ["field", "spellTrap", "fieldSpell"]);
}

function countMirageboundZones(player: Partial<MirageboundPlayer> = {}, zones: Array<"hand"|"field"|"graveyard"|"deck"|"extraDeck"|"spellTrap"|"banished"|"fieldSpell"> = []) {
  return countCards(player, zones, isMiragebound);
}

function actionUsesName(sequence: MirageboundLineAction[] = [], name: string) {
  return safeArray(sequence).some(
    (action) => action.type !== "simulatedBattle" && actionName(action) === name,
  );
}

function actionUsesAny(sequence: MirageboundLineAction[] = [], names = new Set()) {
  return safeArray(sequence).some(
    (action) =>
...
function actionUsesMaterialName(action: MirageboundLineAction = {}, name: string) {
  return safeArray(action.materialNames).includes(name) ||
    safeArray(action.materials).some((material) => cardName(material) === name);
}

function sequenceUsesExtraDeckProcedure(sequence: MirageboundLineAction[] = [], name: string) {
  return safeArray(sequence).some(
    (action) => action.type === "extraDeckProcedure" && actionName(action) === name,
  );
}

function sequenceProcedureUsesMaterial(sequence: MirageboundLineAction[] = [], procedureName: string, materialName: string) {
  return safeArray(sequence).some(
    (action) =>
...
function sequenceHasScoutStarter(sequence: MirageboundLineAction[] = []) {
  return safeArray(sequence).some(
    (action) => action.type === "summon" && actionName(action) === MB.SCOUT,
  );
}

function sequenceHasBattle(sequence: MirageboundLineAction[] = []) {
  return safeArray(sequence).some((action) => action.type === "simulatedBattle");
}

function sequenceHasPositionAction(sequence: MirageboundLineAction[] = []) {
...
function getPositionImpact(initialOpponent: Partial<MirageboundPlayer> = {}, finalOpponent: Partial<MirageboundPlayer> = {}) {
  let toDefense = 0;
  let toAttack = 0;
...
function getLineImpact(context: MirageboundContext = {}) {
  const initialState: Partial<import("../../contracts/aiState.js").AiStateShape> = context.initialState || {};
  const finalState: Partial<import("../../contracts/aiState.js").AiStateShape> = context.finalState || {};
...
function addMilestone(list: MirageboundMilestone[], label: string, score: number, detail: unknown = null) {
  if (!Number.isFinite(score) || score === 0) return;
  list.push({ label, score, detail });
}

function getFinalMirageboundSpellTrapAccess(initialBot: Partial<MirageboundPlayer> = {}, finalBot: Partial<MirageboundPlayer> = {}) {
  const initialNames = new Map<string | null, number>();
...
function scoreEngineMilestones(entries: MirageboundMilestone[], impact: ReturnType<typeof getLineImpact>) {
...
function scoreBounceMilestones(entries: MirageboundMilestone[], impact: ReturnType<typeof getLineImpact>) {
...
function scoreControlMilestones(entries: MirageboundMilestone[], impact: ReturnType<typeof getLineImpact>) {
...
function scoreResourceMilestones(entries: MirageboundMilestone[], impact: ReturnType<typeof getLineImpact>) {
...
function scoreBattleMilestones(entries: MirageboundMilestone[], impact: ReturnType<typeof getLineImpact>) {
...
function scoreDefenseAndRiskMilestones(entries: MirageboundMilestone[], impact: ReturnType<typeof getLineImpact>) {
...
function finalizeImpactPayoff(impact: ReturnType<typeof getLineImpact>) {
  impact.realPayoff =
    impact.hasFinalSovereign ||
    (impact.hasFinalLeviathan && impact.initialOpponentMonsterCount >= 2) ||
    impact.removedOpponentCards > 0 ||
...
function formatMilestone(entry: string | Partial<MirageboundMilestone>) {
...
function collectPlanningReasons(analysis: Partial<MirageboundAnalysis> = {}) {
...
function hasBattleBridgeSignal(analysis: Partial<MirageboundAnalysis> = {}) {
  ...
  const opponent: Partial<MirageboundPlayer> = analysis.opponent || {};
...
export function buildMirageboundPlanningProfile(analysis: Partial<MirageboundAnalysis> = {}, context: MirageboundContext = {}) {
  ...
  return {
    ...
    turnMode: (requestedTurnMode || (battleBridge ? "mainBattleMain2" : "mainOnly")) as AIPlanningProfile["turnMode"],
...
export function scoreMirageboundLineMilestones(context: MirageboundContext = {}) {
  const impact = finalizeImpactPayoff(getLineImpact(context));
  const entries: MirageboundMilestone[] = [];
...
export function scoreMirageboundLineTerminal(context: MirageboundContext = {}) {
...
export function describeMirageboundPlannedLine(context: MirageboundContext = {}) {
  const sequence = safeArray(context.sequence);
