...
import type { GameCard, CardDynamicBuffMap } from "../contracts/cards.js";
...
type VoidPlayer = AIStrategyBotPort & {
  usedEffects?: number[];
  _simMaterialEffectActivationsByMaterialId?: unknown;
};
interface VoidGame extends Omit<AiLiveGamePort, "player" | "bot" | "effectEngine" | "_gameRef"> {
  player: VoidPlayer;
  bot: VoidPlayer;
  _gameRef?: VoidGame;
...
type VoidAnalysisInput = Omit<Partial<ReturnType<typeof buildStrategyAnalysis>>, "phase" | "game"> & {
  ...
  hollowCount?: number;
  voidCount?: number;
  swarmPayoffs?: ReturnType<VoidStrategy["analyzeSwarmPayoffs"]>;
...
type VoidAnalysis = VoidAnalysisInput & {
  hand: StrategyCard[];
  field: StrategyCard[];
  ...
  macroStrategy?: ReturnType<VoidStrategy["decideMacroStrategy"]>;
};
type VoidActivationContext = AIActivationContext & {
  actionContext?: {
...
type VoidCompleteAnalysis = VoidAnalysis & Required<Pick<VoidAnalysis, "availableCombos" | "readyCombos" | "hollowEconomy" | "swarmPayoffs" | "finisherPlans" | "hollowResourcePolicy" | "macroStrategy">>;
interface VoidPassiveState {
  bot?: Pick<SimulatedPlayerState, "field" | "graveyard">;
  player?: Pick<SimulatedPlayerState, "field" | "graveyard">;
}
...
function cloneDynamicBuffsForSim(dynamicBuffs: CardDynamicBuffMap | null | undefined) {
...
function detachDynamicBuffsForSim(card: StrategyCard | null | undefined) {
...
function shouldPreserveArcturusSoloBuff(player: VoidPlayer, opponent: VoidPlayer | null, analysis: VoidAnalysisInput = {}) {
...
function getMirrorDimensionCandidateScore(card: StrategyCard | null | undefined) {
...
function evaluateMirrorDimensionSetup(bot: VoidPlayer, opponent: VoidPlayer | null, mirrorCard: StrategyCard) {
...
function getMaterialEffectActivationCount(game: VoidGame | null | undefined, player: VoidPlayer | null | undefined, materialId: number | undefined) {
  ...
  const readMapLike = (value: unknown): number => {
    ...
    return (value as Record<string, number>)[materialId!] || (value as Record<string, number>)[String(materialId)] || 0;
  };
...
function getSimulatedVoidAscensionCandidates(game: VoidGame, player: VoidPlayer, material: StrategyCard) {
...
export default class VoidStrategy extends BaseStrategy {
  ...
  generateMainPhaseActions(gameInput: AIState): AIAction[] {
    ...
    // ASCENSION CHECK
    // ═══════════════════════════════════════════════════════════════════════════
    const getAscensionFinisherPlan = (ascensionCard: StrategyCard) =>
        ...
        if (card.cardKind === "monster") {
          ...
          if (handIgnitionEffect) {
            ...
            if (card.id === VOID_IDS.SLAYER_BRUTE) {
              if (voidFieldCount >= 2) {
                const costPrefs: Partial<ReturnType<typeof buildVoidCostPreferences>> =
  ...
  getVoidSimFieldOwners(state: VoidPassiveState = {}) {
    return [state.bot, state.player].filter(Boolean);
  }

  findVoidSimOwner(state: VoidPassiveState = {}, card: SimulatedCardState | null = null) {
    return this.getVoidSimFieldOwners(state).find((player) =>
      (player?.field || []).includes(card!),
    );
  }

  clearSimulatedVoidPassives(card: SimulatedCardState | null = null) {
    ...
    for (const key of Object.keys(card.dynamicBuffs)) {
      ...
      const stats: ("atk" | "def")[] = Array.isArray(entry?.stats) ? entry.stats : ["atk", "def"];
      if (value !== 0) {
  ...
  applySimulatedVoidPassiveBuff(card: SimulatedCardState | null = null, key: string, value: number, stats: ("atk" | "def")[] = ["atk", "def"]) {
  ...
  applySimulatedVoidPassives(state: VoidPassiveState = {}) {
  ...
  recordSimulatedMaterialActivation(state: AiStateShape, player: SimulatedPlayerState, card: SimulatedCardState) {
    ...
    const playerId = (player?.id || "bot") as "player" | "bot";
    ...
    (bucket as Record<number, number>)[card.id!] = ((bucket as Record<number, number>)[card.id!] || 0) + 1;
    if (!player._simMaterialEffectActivationsByMaterialId) {
      player._simMaterialEffectActivationsByMaterialId = {};
    }
    (player._simMaterialEffectActivationsByMaterialId as Record<number, number>)[card.id!] =
      ((player._simMaterialEffectActivationsByMaterialId as Record<number, number>)[card.id!] || 0) + 1;
  }
  ...
  handleVoidSimulatedSpecialSummon(payload: VoidSummonPayload = {}) {
    ...
    this.simulateVoidHollowRecruit({ state: state!, player, card, fromZone, action });
  }

  handleVoidSimulatedNormalSummon({ state, player, newCard }: VoidSummonPayload = {}) {
  ...
  buildVoidSimulationOptions(action: AIAction) {
    ...
    return {
      ...
      placeSpellCard,
      onAfterSummon: (payload: VoidSummonPayload) => this.handleVoidSimulatedNormalSummon(payload),
