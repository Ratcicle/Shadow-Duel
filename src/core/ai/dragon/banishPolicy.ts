...
function candidateCard(entry: DragonCandidate): DragonCard {
  const wrapped = entry as { candidate?: DragonCard; card?: DragonCard };
  return wrapped?.candidate || wrapped?.card || entry as DragonCard;
}

function instanceId(card: DragonCard) {
  return (
    card?.instanceId ??
    card?._instanceId ??
    card?.uid ??
...
function candidateIds(cards: DragonCard[] = []) {
  return (cards || [])
    .map((entry) => instanceId(candidateCard(entry)))
    .filter((id) => id !== null && id !== undefined);
}

function isDragonMonster(card: DragonCard | null | undefined): card is DragonCard {
...
function isLowDragon(card: DragonCard) {
  return isDragonMonster(card) && (card.level || 0) <= 4;
}

function countName(cards: DragonCard[] = [], name: string) {
  return (cards || []).filter((card) => card?.name === name).length;
}

function getEffectiveAtkValue(card: DragonCard | null) {
  if (!card) return 0;
  return (
    (card.atk || 0) +
    (card.tempAtkBoost || 0) +
...
function rankSameShape<Entry extends DragonCandidate>(candidates: Entry[] = [], scoreFn: (card: DragonCard, entry: Entry) => number) {
  return (candidates || [])
...
function isUsedFieldBody(card: DragonCard) {
  return !!(card?.usedEffectThisTurn || card?.hasAttacked || card?.positionChangedThisTurn);
}

function hasImmediateStelyaPayoff(ctx: ReturnType<typeof makeContext>) {
...
export function scoreDragonFieldBanishCost(card: DragonCard, context: DragonPolicyContext = {}) {
...
export function rankDragonFieldBanishCosts<Entry extends DragonCandidate>(candidates: Entry[] = [], context: DragonPolicyContext = {}) {
  return rankSameShape(candidates, (card) => scoreDragonFieldBanishCost(card, context));
}

function gySummaryByInstance(entries: Array<{ name?: string; instanceId?: string | number | null }> = []) {
...
function isStateMarkedSafe(card: DragonCard, ctx: ReturnType<typeof makeContext>) {
...
function isStateMarkedProtected(card: DragonCard, ctx: ReturnType<typeof makeContext>) {
...
export function scoreDragonGyBanishCost(card: DragonCard, context: DragonPolicyContext = {}) {
  const ctx = makeContext(context);
...
export function rankDragonGyBanishCosts<Entry extends DragonCandidate>(candidates: Entry[] = [], context: DragonPolicyContext = {}) {
  return rankSameShape(candidates, (card) => scoreDragonGyBanishCost(card, context));
}

function techVoidBuffAmount(card: DragonCard) {
  return Math.floor(Math.max(0, card?.atk || 0) * 0.5);
}

function techVoidBuffMatters(card: DragonCard, ctx: ReturnType<typeof makeContext>) {
...
function hasPendingGyValueForTechVoid(card: DragonCard, ctx: ReturnType<typeof makeContext>) {
  const name = card?.name!;
  const duplicates = countName(ctx.graveyard, name);
  const opt = ctx.dragonState.opt || {};
...
export function scoreTechVoidBanishTarget(card: DragonCard, context: DragonPolicyContext = {}) {
  const ctx = makeContext(context);
...
export function rankTechVoidBanishTargets<Entry extends DragonCandidate>(candidates: Entry[] = [], context: DragonPolicyContext = {}) {
  return rankSameShape(candidates, (card) => scoreTechVoidBanishTarget(card, context));
}

function buildPreferenceFromRanked(ranked: DragonCandidate[] = [], scoreFn: (card: DragonCard, context: DragonPolicyContext) => number, context: DragonPolicyContext = {}, options: { limit?: number; preferMaxScore?: number; preserveMinScore?: number } = {}) {
...
function preferExactCards(preference: DragonPreference = {}, cards: DragonCard[] = []) {
...
export function shouldUseStelyaBanishSummon(context: DragonPolicyContext = {}) {
  const ctx = makeContext(context);
...
export function shouldUsePurifiedBanishSummon(context: DragonPolicyContext = {}) {
  const ctx = makeContext(context);
...
export function buildDragonBanishTargetPreferences(context: DragonPolicyContext = {}) {
  const ctx = makeContext(context);
  const byId: Record<string, DragonPreference | undefined> = {};
