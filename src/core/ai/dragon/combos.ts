...
function availableCombo(name: string, action: { type: string; cardName?: string }, overrides: { priority?: number } = {}) {
  const definition: Partial<(typeof COMBO_DATABASE)[number]> = COMBO_BY_NAME.get(name) || {};
  return {
    ...definition,
    name,
 ...
 * @returns {Array}
 */
export function detectAvailableCombos(analysis: DragonAnalysis, logFn: ((message: string) => void) | null = null) {
  const available: ReturnType<typeof availableCombo>[] = [];
  ...
  const log = (msg: string) => typeof logFn === "function" && logFn(msg);
  ...
  if (
    ...
    (analysis.hasJaggedPeak || (analysis.fieldSpell as DragonCard | string | null | undefined) === "Jagged Peak of the Dragons")
  ) {
