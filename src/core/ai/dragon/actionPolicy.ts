...
function optCanUse(ctx: ReturnType<typeof makeContext>, key: keyof typeof import("./stateAnalysis.js").DRAGON_OPT_NAMES) {
  const status = ctx.dragonState?.opt?.[key];
  return status?.canUse !== false;
}

function hasLevelReductionPayoff(ctx: ReturnType<typeof makeContext>) {
...
function hasStelyaBodyPayoff(ctx: ReturnType<typeof makeContext>) {
...
function hasStelyaSearchTargetPlan(target: DragonCard, ctx: ReturnType<typeof makeContext>) {
...
function bestStelyaSearchTarget(ctx: ReturnType<typeof makeContext>) {
...
function buildTargetPreference(cards: DragonCard[] = [], role = "named_preference") {
  return {
    role,
...
function rankRecruitCards<Card extends DragonCard>(candidates: Card[] = [], context: DragonPolicyContext = {}) {
...
function hasLunarGyRecruitPayoff(ctx: ReturnType<typeof makeContext>, bestCard: DragonCard | null = null) {
...
export function buildDragonRecruitTargetPreference(candidates: DragonCard[] = [], context: DragonPolicyContext = {}) {
...
export function evaluateDragonRecruitCandidate<Card extends DragonCard>(candidates: Card[] = [], context: DragonPolicyContext = {}) {
...
export function evaluateDragonHandIgnition(card: DragonCard, effect: import("../../contracts/effects.js").EffectDefinition | null, context: DragonPolicyContext = {}) {
...
export function evaluateDragonGraveyardIgnition(card: DragonCard, effect: import("../../contracts/effects.js").EffectDefinition | null, context: DragonPolicyContext = {}) {
  if (!card || !effect) return null;
  const ctx = makeContext({
    ...context,
    source: card,
