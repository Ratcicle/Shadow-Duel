// The legacy Polymerization fallback reads an unbound identifier. This erased
// declaration preserves its existing ReferenceError without changing its heuristic.
declare const hasUsefulGYTarget: boolean;
import type {
  DragonCard,
  DragonPlayer,
  DragonAnalysis,
...
function isProtectedDragonFieldBoss(card: DragonCard | null) {
  return isExtremeDragon(card) || card?.name === "Supreme Bahamut Dragon";
}

function getCardRole(card: DragonCard | null) {
  return CARD_KNOWLEDGE[card?.name!]?.role || "";
}

function isPremiumDragonSummon(card: DragonCard | null) {
...
function getDragonBoardValue(card: DragonCard | null, overrides: { projectedAtk?: number; projectedDef?: number } = {}) {
 ...
 */
export function shouldPlaySpell(card: DragonCard, analysis: DragonAnalysis) {
...
function getTributeValue(monster: DragonCard) {
  if (!monster) return 0;
