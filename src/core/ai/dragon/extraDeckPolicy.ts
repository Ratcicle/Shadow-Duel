...
function isFaceupMonster(card: DragonCard) {
...
function getDragonState(context: DragonPolicyContext = {}, player: DragonPlayer = {}, opponent: DragonPlayer = {}) {
...
function materialValue(entry: Partial<DragonMaterialEntry> = {}, context: DragonPolicyContext = {}) {
...
function buildFusionMaterialEntries(player: DragonPlayer = {}) {
...
function selectLowestValue(entries: DragonMaterialEntry[] = [], count: number = 1, context: DragonPolicyContext = {}) {
  return entries
    .filter(Boolean)
    .slice()
    .sort(
...
function getContext(context: DragonPolicyContext = {}) {
...
function hasExtraDeckCard(ctx: ReturnType<typeof getContext>, name: string) {
  return hasName(ctx.extraDeck, name) || ctx.allowMissingExtraDeck === true;
}

function getLowGyBuffTargets(ctx: ReturnType<typeof getContext>) {
...
function selectTechVoidMaterials(ctx: ReturnType<typeof getContext>) {
...
function scoreTechVoid(ctx: ReturnType<typeof getContext>): DragonFusionPlan | null {
...
function selectRadiantMaterials(ctx: ReturnType<typeof getContext>) {
...
function getRadiantRecycleTargets(ctx: ReturnType<typeof getContext>) {
...
function scoreRadiant(ctx: ReturnType<typeof getContext>): DragonFusionPlan | null {
...
export function rankDragonFusionPlans(context: DragonPolicyContext = {}) {
  const ctx = getContext(context);
  return ([scoreTechVoid(ctx), scoreRadiant(ctx)]
    .filter(Boolean) as Array<NonNullable<ReturnType<typeof scoreTechVoid> | ReturnType<typeof scoreRadiant>>>)
    .sort((a, b) => b.score - a.score);
}

export function selectDragonFusionPlan(context: DragonPolicyContext = {}) {
  const ranked = rankDragonFusionPlans(context);
  const best = ranked.find((plan) => plan.ok);
  return best || ranked[0] || null;
}

function preferenceForEntries(entries: DragonMaterialEntry[] = []) {
  return {
...
function buildPlanTargetPreferences(plan: DragonFusionPlan, ctx: ReturnType<typeof getContext>) {
...
function bestOwnProtectionTargets(ctx: ReturnType<typeof getContext>) {
  return ctx.field
...
export function buildDragonExtraDeckActionContext(context: DragonPolicyContext = {}) {
  const ctx = getContext(context);
  const sourceName =
    context.source?.name ||
  ...
  const targetPreferences: Record<string, DragonPreference | undefined> = {};
  let fusionPreferences: { preferredNames: string[]; scoresByName: Record<string, number> } | null = null;
  let fusionPositions: { byName: Record<string, "attack" | "defense"> } | null = null;
  let costPreferences: { preferNames: string[]; preserveNames: string[]; offensivePayoffNames: string[] } | null = null;
...
export function rankDragonAscensionChoices<Card extends DragonCard>(choices: DragonAscensionChoice<Card>[] = [], context: DragonPolicyContext = {}) {
  return ((choices || [])
    .map((choice) => scoreAscensionChoice(choice, context))
    .filter(Boolean) as Array<NonNullable<ReturnType<typeof scoreAscensionChoice<Card>>>>)
    .sort((a, b) => b.score - a.score);
}

export function selectDragonAscensionChoice<Card extends DragonCard>(choices: DragonAscensionChoice<Card>[] = [], context: DragonPolicyContext = {}) {
  const ranked = rankDragonAscensionChoices(choices, context);
  return ranked.find((choice) => choice.ok) || null;
}

export function chooseDragonAscensionPosition({ ascensionCard, material, game, bot, opponent }: DragonAscensionChoice & DragonPolicyContext = {}) {
