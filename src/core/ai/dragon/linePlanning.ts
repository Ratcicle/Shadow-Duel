...
export interface DragonLineContext extends DragonPolicyContext {
  ...
  state?: DragonGame;
  sequence?: DragonPlanningAction[];
  milestones?: Array<DragonMilestone | string>;
  baseScore?: number;
  finalScore?: number;
...
const DEFAULT_PROFILE: AIPlanningProfile & { reasons: string[]; critical: boolean } = {
  enabled: false,
  mode: "off",
  turnMode: "mainOnly",
  beamWidth: 3,
...
function isCurrentDragonListMode(analysis: DragonAnalysis | null = {}) {
  return analysis?.currentDragonBotList !== false;
}

function getPlayer(analysis: DragonAnalysis = {}, context: DragonLineContext = {}) {
  return context.bot || analysis.bot || analysis.player || context.game?.bot || {};
}

function getOpponent(analysis: DragonAnalysis = {}, context: DragonLineContext = {}) {
  return analysis.opponent || context.opponent || context.game?.player || {};
}

function getCards(player: DragonPlayer | null | undefined, zone: DragonZone) {
  if (!player) return [];
  if (zone === "fieldSpell") return player.fieldSpell ? [player.fieldSpell] : [];
  return Array.isArray(player[zone]) ? player[zone].filter(Boolean) : [];
}

function getAnalysisCards(analysis: DragonAnalysis = {}, player: DragonPlayer = {}, zone: DragonZone) {
...
function isDragonMonster(card: DragonCard | null | undefined): card is DragonCard & { cardKind: "monster"; type: "Dragon" } {
  return card?.cardKind === "monster" && card.type === "Dragon";
}

function isFaceupDragon(card: DragonCard | null | undefined): card is DragonCard & { cardKind: "monster"; type: "Dragon" } {
  return isDragonMonster(card) && !card.isFacedown;
}

function hasArchetype(card: DragonCard | null | undefined, archetype: string) {
...
function isExtremeDragon(card: DragonCard | null | undefined) {
...
function getBattleTargetStat(card: DragonCard = {}) {
...
function canDragonAttack(card: DragonCard = {}) {
  return (
    isFaceupDragon(card) &&
...
function canDestroyByBattle(attacker: DragonCard, target: DragonCard) {
...
function getActionCard(action: DragonPlanningAction = {}, player: DragonPlayer = {}) {
...
function actionPreferredNames(action: DragonPlanningAction = {}) {
  const preferences =
    (action.activationContext?.actionContext as { targetPreferences?: Record<string, DragonPreference | undefined> } | undefined)?.targetPreferences ||
...
function actionMentionsName(action: DragonPlanningAction = {}, name: string) {
  if (!name) return false;
  if (action.cardName === name || action.card?.name === name) return true;
  return actionPreferredNames(action).includes(name);
}

function findCardsByNames(player: DragonPlayer = {}, names: string[] = []) {
  const wanted = new Set(names);
  if (wanted.size === 0) return [];
  return ["hand", "field", "graveyard", "deck", "extraDeck", "spellTrap"]
    .flatMap((zone) => getCards(player, zone as DragonZone))
    .concat(getCards(player, "fieldSpell"))
    .filter((card) => wanted.has(card?.name!));
}

function hasExtremePreferredTarget(action: DragonPlanningAction = {}, player: DragonPlayer = {}) {
  return findCardsByNames(player, actionPreferredNames(action)).some(isExtremeDragon);
}

function addRetention(reasons: string[], amount: number, reason: string) {
  reasons.push(reason);
  return amount;
}

function getRetentionAdjustment(action: DragonPlanningAction = {}, context: DragonLineContext = {}) {
...
export function applyDragonRetentionPriorities<Action extends DragonPlanningAction>(actions: Action[] = [], context: DragonLineContext = {}) {
  return (actions || [])
    .map((action, index) => {
...
function getPlanningBot(state: DragonGame = {}) {
  return state?.bot || {};
}

function getPlanningOpponent(state: DragonGame = {}) {
  return state?.player || {};
}
...
function getAllCards(player: DragonPlayer = {}) {
  return [
...
function countActiveCards(player: DragonPlayer = {}) {
  return (
...
function countOpponentBoardCards(player: DragonPlayer = {}) {
  return (
    getCards(player, "field").length +
    getCards(player, "spellTrap").length +
    getCards(player, "fieldSpell").length
...
function getFieldDragons(player: DragonPlayer = {}) {
  return getCards(player, "field").filter(isFaceupDragon);
}

function getUsefulGyResources(player: DragonPlayer = {}) {
  return getCards(player, "graveyard").filter(
    (card) => isDragonMonster(card) || card?.name === "Hellkite Roar",
  );
}

function countDragonCardsInZone(player: DragonPlayer = {}, zone: DragonZone) {
  return countZone(getCards(player, zone), isDragonMonster);
}

function hasProtectedDragon(player: DragonPlayer = {}) {
...
function hasBossOnField(player: DragonPlayer = {}) {
  return getCards(player, "field").some(
    (card) =>
      isFaceupDragon(card) &&
...
function hasAction(sequence: DragonPlanningAction[] = [], name: string, type: string | null = null) {
...
function hasActionType(sequence: DragonPlanningAction[] = [], type: string | null) {
  return (sequence || []).some((action) => action?.type === type);
}

function sequenceCardNames(sequence: DragonPlanningAction[] = []) {
  return (sequence || [])
    .map((action) => action?.cardName || action?.card?.name || action?.attackerName)
    .filter(Boolean);
}

function sequenceMentions(sequence: DragonPlanningAction[] = [], name: string) {
  return sequenceCardNames(sequence).includes(name);
}

function sequenceMentionsAny(sequence: DragonPlanningAction[] = [], names: Set<string> = new Set()) {
  return sequenceCardNames(sequence).some((name) => names.has(name!));
}

function finalHasLiveEclipseStelyaFollowUp(player: DragonPlayer = {}) {
  return (
...
function isRelevantDefenseDragon(card: DragonCard = {}) {
  return (
    isFaceupDragon(card) &&
    (DRAGON_BOSS_NAMES.has(card.name!) ||
      isExtremeDragon(card) ||
...
function getFinalSoloExtreme(player: DragonPlayer = {}) {
...
function hasMajorSoloBreakPayoff({
  ...
  threatReduced = 0,
}: { finalBot?: DragonPlayer; damageDealt?: number; removedOpponentCards?: number; threatReduced?: number } = {}) {
...
function selectCorrectBossPayoff({
  ...
  damageDealt = 0,
  removedOpponentCards = 0,
}: { finalBot?: DragonPlayer; initialOpponent?: DragonPlayer; finalHasProtection?: boolean; damageDealt?: number; removedOpponentCards?: number } = {}) {
...
function finalHandGainedFromInitialGy(initialBot: DragonPlayer = {}, finalBot: DragonPlayer = {}, predicate: (card: DragonCard) => boolean = () => true) {
...
function findFinalFieldCard(player: DragonPlayer = {}, name: string) {
  return getCards(player, "field").find((card) => card?.name === name && !card.isFacedown);
}

function addLineMilestone(entries: DragonMilestone[], label: string, score: number, detail: unknown = null) {
...
function getSimulatedBattleSteps(sequence: DragonPlanningAction[] = []) {
...
export function scoreDragonLineMilestones(context: DragonLineContext = {}) {
  ...
  const milestones: DragonMilestone[] = [];
...
function sumDragonAtk(player: DragonPlayer = {}) {
...
function scoreBossQuality(player: DragonPlayer = {}) {
  let score = 0;
...
function countProtectedDragons(player: DragonPlayer = {}) {
...
function hasHandFollowUp(player: DragonPlayer = {}) {
...
function hasGraveyardFollowUp(player: DragonPlayer = {}) {
...
function scorePurifiedRainbowSetup(player: DragonPlayer = {}) {
...
function scoreTerminalAdjustments(context: DragonLineContext = {}) {
...
export function scoreDragonLineTerminal(context: DragonLineContext = {}) {
  const finalBot = getPlanningBot(context.finalState || {});
  const finalOpponent = getPlanningOpponent(context.finalState || {});
...
function actionLabel(action: DragonPlanningAction = {}) {
  if (!action) return "";
...
function compactActionStep(action: DragonPlanningAction = {}) {
...
function milestoneLabel(entry: DragonMilestone | string = {}) {
  if (typeof entry === "string") return entry;
  return entry.label || entry.name || entry.reason || "";
}

function finalFieldNames(player: DragonPlayer = {}) {
  return getCards(player, "field")
    .filter((card) => card && !card.isFacedown)
    .map((card) => card.name!);
}

function findFinalPayoffName(finalBot: DragonPlayer = {}, preferred: string[] = []) {
...
function inferDragonLineHeadline(context: DragonLineContext = {}) {
  const sequence = Array.isArray(context.sequence) ? context.sequence : [];
  const initialBot = getPlanningBot(context.initialState || {});
  const finalBot = getPlanningBot(context.finalState || {});
  ...
  const currentListMode = isCurrentDragonListMode(context.analysis);
  const hasUsed = (name: string, type: string | null = null) =>
    sequence.some(
      (action) =>
        (type == null || action?.type === type) &&
...
export function describeDragonPlannedLine(context: DragonLineContext = {}) {
  const sequence = Array.isArray(context.sequence) ? context.sequence : [];
...
export function buildDragonPlanningProfile(analysis: DragonAnalysis = {}, context: DragonLineContext = {}): AIPlanningProfile & { reasons: string[]; critical: boolean } {
  const game = context.game || analysis.game || {};
  const player = getPlayer(analysis, context);
  const opponent = getOpponent(analysis, context);
  const hand = getAnalysisCards(analysis, player, "hand");
  ...
  const dragonState: Partial<import("./contracts.js").DragonState> = analysis.dragonState || {};
  const currentListMode = isCurrentDragonListMode(analysis);
  const reasons: string[] = [];
