...
function getSourceName(context: DragonPolicyContext = {}) {
  return (
    context.source?.name ||
...
function getEffectId(action: DragonSearchAction = {}, context: DragonPolicyContext = {}) {
  return (
...
function classifySearch(action: DragonSearchAction = {}, context: DragonPolicyContext = {}) {
...
function scoreGeneric(card: DragonCard, ctx: ReturnType<typeof makeContext>) {
...
function scoreArmored(card: DragonCard, ctx: ReturnType<typeof makeContext>) {
...
function scoreLunar(card: DragonCard, ctx: ReturnType<typeof makeContext>) {
  let score = scoreGeneric(card, ctx);
  const name = card?.name!;
  const ds = ctx.dragonState;
...
function scoreStelya(card: DragonCard, ctx: ReturnType<typeof makeContext>) {
  let score = scoreGeneric(card, ctx);
  const name = card?.name!;
...
function scoreAwakening(card: DragonCard, ctx: ReturnType<typeof makeContext>) {
...
function scoreBlackBull(card: DragonCard, ctx: ReturnType<typeof makeContext>) {
  let score = scoreGeneric(card, ctx);
  const name = card?.name!;
...
function scoreHellkiteRoar(card: DragonCard, ctx: ReturnType<typeof makeContext>) {
...
function scoreCandidate(card: DragonCard, searchKind: string, ctx: ReturnType<typeof makeContext>) {
  switch (searchKind) {
    case "armored":
...
export function rankDragonSearchCandidates<Card extends DragonCard>(candidates: Card[] = [], action: DragonSearchAction = {}, context: DragonPolicyContext = {}) {
...
export function getDragonSearchKind(action: DragonSearchAction = {}, context: DragonPolicyContext = {}) {
