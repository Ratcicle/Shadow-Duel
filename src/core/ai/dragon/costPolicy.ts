...
function hasLevel7Or8SearchTarget(ctx: ReturnType<typeof makeContext>) {
...
function hasLiveReviveAccess(ctx: ReturnType<typeof makeContext>) {
  return ctx.hasCallAccess || ctx.hasJaggedPeakAccess || ctx.dragonState.hasSolarInGY;
}

export function scoreDragonDiscardCandidate(card: DragonCard, context: DragonPolicyContext = {}) {
  if (!card) return 9999;
...
export function rankDragonDiscardCandidates<Card extends DragonCard>(candidates: Card[] = [], context: DragonPolicyContext = {}) {
...
function protectedDiscardNames(ctx: ReturnType<typeof makeContext>, candidates: DragonCard[] = []) {
...
function buildTargetPreferenceForCandidates(candidates: DragonCard[] = [], context: DragonPolicyContext = {}, limit: number = 4) {
  const ctx = makeContext(context);
...
export function buildDragonCostPreferences(context: DragonPolicyContext = {}) {
  const ctx = makeContext(context);
...
export function buildDragonTargetCostPreferences(context: DragonPolicyContext = {}) {
  const ctx = makeContext(context);
