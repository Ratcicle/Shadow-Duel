 ...
 */
export const CARD_KNOWLEDGE: Record<string, DragonCardKnowledge> = {
...
export function isCurrentDragonBotCardName(name: string) {
  return CURRENT_DRAGON_BOT_CARD_NAME_SET.has(name);
}

export function isOutOfPlanDragonCardName(name: string) {
 ...
 * @returns {boolean}
 */
export function isExtremeDragon(card: DragonCard | null | undefined) {
 ...
 * @returns {number}
 */
export function countExtremeInGY(graveyard: DragonCard[]) {
 ...
 * @returns {number}
 */
export function countSafeBanishTargets(graveyard: DragonCard[]) {
  if (!Array.isArray(graveyard)) return 0;
