...
export function assessDragonExtremeResourcePolicy(analysis: DragonAnalysis = {}) {
  const economy =
 ...
 * @returns {number}
 */
export function evaluateBoardDragon(gameOrState: DragonGame, perspectivePlayer: DragonPlayer | null | undefined, getOpponentFn: ((game: DragonGame, perspective: DragonPlayer | null | undefined) => DragonPlayer | null | undefined) | undefined) {
  ...
  const dragonState =
    ...
    (perspective as (DragonPlayer & { strategy?: { currentAnalysis?: DragonAnalysis | null } | null }) | undefined)?.strategy?.currentAnalysis?.dragonState ||
    analyzeDragonState({
      game: gameOrState,
      bot: perspective!,
      opponent: opponent!,
