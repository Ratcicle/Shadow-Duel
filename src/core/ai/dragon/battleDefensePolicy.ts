...
function zoneCards<Card extends DragonCard>(player: DragonPlayer<Card> | null, zone: DragonZone) {
  if (!player) return [];
...
function battleStat(card: DragonCard = {}) {
...
function cardThreatValue(card: DragonCard | null = {}) {
  if (!card) return 0;
...
function cardFieldValue(card: DragonCard = {}) {
  if (!card) return 0;
...
function isSoloExtreme(attacker: DragonCard, bot: DragonPlayer = {}) {
...
function bestBattleTargetForAttacker(attacker: DragonCard, opponentField: DragonCard[] = []) {
...
export function getMajesticBattlePositionPlan({
  source,
  opponentField = [],
}: DragonPolicyContext & { opponentField?: DragonCard[] } = {}) {
...
export function getLuminescentBattleDebuffPlan<Card extends DragonCard>({
  ...
  const candidates: Array<{ target: Card; attacker: Card; score: number }> = [];
...
export function scoreDragonBattleAttack(context: DragonPolicyContext = {}) {
...
function preferredCardsByNames(cards: DragonCard[] = [], names: string[] = []) {
  const entries = (cards || [])
    .filter(Boolean)
    .map((card, index) => {
...
export function buildDragonDefenseTargetPreferences(context: DragonPolicyContext = {}) {
...
export function scoreDragonBackrowSet(card: DragonCard, context: DragonPolicyContext = {}) {
...
export function shouldRecheckBossBeforeBattle(context: DragonPolicyContext = {}) {
  const player = context.player || context.bot || {};
  const opponent = context.opponent || {};
