...
function hasUsefulHellkiteReviveTarget(ctx: ReturnType<typeof buildDragonBossContext>) {
...
function hasMajesticPositionSwing(ctx: ReturnType<typeof buildDragonBossContext>) {
...
function canKeepExtremeSolo(ctx: ReturnType<typeof buildDragonBossContext>) {
...
function isDecisiveExtremeReplacement(card: DragonCard, ctx: ReturnType<typeof buildDragonBossContext>) {
...
export function buildDragonBossContext(context: DragonPolicyContext = {}) {
...
export function isDragonBossCandidate(card: DragonCard) {
  return DRAGON_BOSS_NAME_SET.has(card?.name!);
}

export function hasActiveExtremeDragonConflict(card: DragonCard, context: DragonPolicyContext = {}) {
...
export function hasDragonBossRoute(card: DragonCard, context: DragonPolicyContext = {}) {
...
export function scoreDragonBossCandidate(card: DragonCard, context: DragonPolicyContext = {}) {
...
export function rankDragonBossCandidates<Card extends DragonCard>(candidates: Card[] = [], context: DragonPolicyContext = {}) {
  return (candidates || [])
    .filter(Boolean)
    .map((card, index) => ({
      card,
...
export function selectBestDragonBoss<Card extends DragonCard>(candidates: Card[] = [], context: DragonPolicyContext = {}) {
...
export function buildDragonBossPreferenceMap(context: DragonPolicyContext = {}) {
  const ctx = buildDragonBossContext(context);
  const sourceName =
    context.source?.name ||
  ...
  const effectId =
    ...
    "";
  const map: Record<string, DragonPreference | undefined> = {};
...
export function actionBreaksSoloExtremeProtection(action: DragonSearchAction = {}, context: DragonPolicyContext = {}) {
  const ctx = buildDragonBossContext(context);
