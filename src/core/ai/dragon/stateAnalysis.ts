interface DragonResourceContext {
  opt?: ReturnType<typeof buildOptState>;
  hasDragonFieldBodyForStelya?: boolean;
  hasTwoDragonsForAwakening?: boolean;
  hasLevel7PlusForRoar?: boolean;
  hasVoltaicForTechVoid?: boolean;
  hasLuminousForRadiant?: boolean;
  hasThreeSafeGYDragonsForPurified?: boolean;
  hasCallSetOrHand?: boolean;
  voltaicInGYCount?: number;
  luminousInGYCount?: number;
}
...
const USEFUL_DISCARD_REASONS: Record<string, string> = {
...
function isLevel5PlusDragon(card: DragonCard) {
...
function summarizeCard<Extra extends object>(card: DragonCard, zone: string, index: number, extra: Extra = {} as Extra) {
  return {
    name: card?.name || "",
    zone,
    index,
...
function summarizeCards(cards: DragonCard[], zone: string, reasonFn: ((card: DragonCard, index: number) => object) | null = null) {
  return (cards || []).map((card, index) =>
    summarizeCard(card, zone, index, reasonFn ? reasonFn(card, index) : {}),
  );
}

function getUsageValue(store: unknown, key: string): unknown {
  if (!store) return undefined;
  if (typeof (store as { get?: unknown }).get === "function") return (store as { get(key: string): unknown }).get(key);
  return (store as Record<string, unknown>)[key];
}

function getOptStatus(game: DragonGame | null, player: DragonPlayer, optName: string, { isSimulatedState = false } = {}) {
  ...
  const gameStore = (game.oncePerTurnUsage as Partial<Record<string, unknown>> | undefined)?.[playerId];
...
function buildOptState(game: DragonGame | null, player: DragonPlayer, options = {}) {
  ...
  return {
    ...(byKey as Record<keyof typeof DRAGON_OPT_NAMES, ReturnType<typeof getOptStatus>>),
...
function discardPriority(card: DragonCard, context: DragonResourceContext = {}) {
  if (!card) return null;
...
function stelyaCostReason(card: DragonCard, context: DragonResourceContext = {}) {
...
function isCriticalGyDragon(card: DragonCard, context: DragonResourceContext = {}) {
  const reasons: string[] = [];
...
function buildGyResource(card: DragonCard, context: DragonResourceContext = {}) {
  const reasons: string[] = [];
  let value = 0;
...
function hasCallAccess(hand: DragonCard[], spellTrap: DragonCard[]) {
  return hasName(hand, "Call of the Haunted") || hasName(spellTrap, "Call of the Haunted");
}

function hasRadiantMaterials(cards: DragonCard[]) {
...
}: { game?: DragonGame | null; bot?: DragonPlayer; opponent?: DragonPlayer; isSimulatedState?: boolean } = {}) {
