...
interface DragonStrategyGame extends Omit<DragonGame, "player" | "bot" | "opponent" | "_gameRef"> {
...
function buildDragonActionContext(extra: DragonPolicyContext = {}) {
...
function buildActivationContext(zone: CanonicalZone, actionContext: ReturnType<typeof buildDragonActionContext> | null = null, extra: Partial<AIActivationContext> = {}) {
...
export default class DragonStrategy extends BaseStrategy {
  ...
  rankSearchCandidates<Card extends DragonCard>(cards: Card[], action: DragonSearchAction, context: DragonPolicyContext = {}) {
    ...
    const opponent =
      context.opponent ||
      (game && player ? this.getOpponent((game?._gameRef || game) as AIState, player as AIStrategyBotPort) : null) ||
      {};
  ...
  buildActivationContextForEffect({
    ...
    const opponent =
      game && owner ? this.getOpponent((game?._gameRef || game) as AIState, owner as AIStrategyBotPort) || {} : {};
    ...
    (bot.hand || []).forEach((card, index) => {
      ...
      for (const handIgnitionEffect of handIgnitionEffects) {
        ...
        const freesMonsterZone = (handIgnitionEffect.actions || []).some(
          (action) => Number((action as { fieldSlotsFreedBeforeSummon?: number })?.fieldSlotsFreedBeforeSummon || 0) > 0,
        );
    ...
    if (actions.length === 0 && !isSimulatedState) {
      ...
      if ((realBot2.hand?.length || 0) > 3) {
        ...
        (realBot2.hand || []).forEach((card, index) => {
          ...
          if (preview && (preview as { ok?: boolean }).ok === false) return;
