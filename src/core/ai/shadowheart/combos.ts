import type { ShadowHeartAnalysis } from "./contracts.js";




type Analysis = ShadowHeartAnalysis;
 ...
 */
export function detectAvailableCombos(analysis: Analysis, logFn: ((message: string) => void) | null = null) {
  const available: ReturnType<typeof createAvailableCombo>[] = [];
  ...
  const hasInHand = (name: string) => hasCardName(zoneIndex, "hand", name);
  const hasOnField = (name: string) => hasCardName(zoneIndex, "field", name);
  const hasInHandOrField = (name: string) =>
  ...
  const addCombo = (name: string, details: { logMessage?: string; priority?: number; action?: import("../../contracts/ai.js").AIAction } = {}) => {
...
export function getComboByName(name: string) {
