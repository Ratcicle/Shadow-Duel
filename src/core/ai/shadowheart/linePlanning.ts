...
import type { AIAction, AIPlannedAction, AIPlanningContext, AIPlanningProfile, AIStrategyBotPort } from "../../contracts/ai.js";
...
interface BattleSummary { damage?: number; attackerName?: string; destroyedCards?: Array<{ owner?: string; cardKind?: string }> }
...
const BOSS_NAMES = new Set<string | null>([
  SH.scale,
  SH.arctroth,
  SH.deathWyrm,
  SH.leviathan,
...
const PREMIUM_NAMES = new Set<string | null>([
  SH.scale,
  SH.arctroth,
  SH.deathWyrm,
  SH.leviathan,
...
const ENGINE_NAMES = new Set<string | null>([
  SH.covenant,
  SH.cathedral,
  SH.valley,
  SH.voidMage,
...
function countShadowHeartMonsters(cards: StrategyCard[] = []) {
  return (cards || []).filter(
    (card) => card?.cardKind === "monster" && isShadowHeartCard(card),
  ).length;
}

function countBosses(cards: StrategyCard[] = []) {
  return (cards || []).filter((card) => BOSS_NAMES.has(nameOf(card))).length;
}

function getAttackDamage(field: StrategyCard[] = []) {
...
function getBattleTargetStat(card: StrategyCard) {
...
function isReadyBattleAttacker(card: StrategyCard) {
  return (
    card &&
...
function hasCurrentBattlePayoff(analysis: Analysis = {}) {
...
function hasCurrentCathedralDamagePayoff(analysis: Analysis = {}) {
  const field = getAnalysisCards(analysis, "field");
  const oppField = analysis.oppField || analysis.opponent?.field || [];
...
function hasAccessibleBattlePayoff(analysis: Analysis = {}) {
...
function hasCathedralBattleBridgePayoff(analysis: Analysis = {}) {
  const hand = getAnalysisCards(analysis, "hand");
...
function shouldUseShadowHeartBattleBridge(analysis: Analysis = {}, context: Context = {}) {
  const game: PlanningGame = context.game || analysis.game || {};
...
function opponentThreatensLethal(bot: Player = {}, opponent: Player = {}) {
  const lp = Number(bot?.lp || 0);
  if (lp <= 0) return true;
  return getAttackDamage(getCards(opponent, "field")) >= lp;
}

function getAnalysisOpponentThreatensLethal(analysis: Analysis = {}) {
  const player: Player = analysis.player || analysis.bot || {};
...
function fieldSpellName(source: { fieldSpell?: StrategyCard | string | null; player?: Player | null } = {}) {
  return nameOf(source.fieldSpell) || nameOf(source.player?.fieldSpell);
}

function hasCovenantLine(analysis: Analysis = {}) {
  const hand = getAnalysisCards(analysis, "hand");
...
function hasVoidMageLine(analysis: Analysis = {}) {
  const hand = getAnalysisCards(analysis, "hand");
...
function hasImpLine(analysis: Analysis = {}) {
  const hand = getAnalysisCards(analysis, "hand");
  return (
    analysis.summonAvailable !== false &&
    hasName(hand, SH.imp) &&
...
function hasGeckoLine(analysis: Analysis = {}) {
  const hand = getAnalysisCards(analysis, "hand");
  const field = getAnalysisCards(analysis, "field");
  const deck = getAnalysisCards(analysis, "deck");
...
function getCathedralCounters(card: StrategyCard | null | undefined) {
  return getCounterCount(card, "judgment_marker");
}

function hasCathedralLine(analysis: Analysis = {}) {
  const hand = getAnalysisCards(analysis, "hand");
  const spellTrap = getAnalysisCards(analysis, "spellTrap");
  const deck = getAnalysisCards(analysis, "deck");
...
function hasInfusionLine(analysis: Analysis = {}) {
  const hand = getAnalysisCards(analysis, "hand");
...
function hasFusionLine(analysis: Analysis = {}) {
  const hand = getAnalysisCards(analysis, "hand");
  const field = getAnalysisCards(analysis, "field");
...
function hasPurgeLine(analysis: Analysis = {}) {
  const hand = getAnalysisCards(analysis, "hand");
...
function hasLeviathanBridge(analysis: Analysis = {}) {
  const hand = getAnalysisCards(analysis, "hand");
  const field = getAnalysisCards(analysis, "field");
  return hasName(hand, SH.leviathan) && (hasName(hand, SH.eel) || hasName(field, SH.eel));
}

function hasBossAccess(analysis: Analysis = {}) {
  const hand = getAnalysisCards(analysis, "hand");
  const field = getAnalysisCards(analysis, "field");
...
function hasVisibleConversionPayoff(analysis: Analysis = {}) {
...
function hasStarterAccess(analysis: Analysis = {}) {
  return (
...
function sequenceCardNames(sequence: LineAction[] = []) {
  return (sequence || [])
    .map((action) => action?.card?.name || action?.cardName || action?.attackerName)
    .filter(Boolean);
}

function addMilestone(state: ScoreState, label: string, score: number) {
...
function hasRealPayoff({ finalBot, finalOpponent, initialOpponent }: Omit<ImpactContext, "initialBot" | "sequence">) {
  return (
...
function getActionLabel(action: Partial<LineAction> = {}) {
...
function getAllPlayerCards(player: Player | null | undefined = {}) {
  return [
...
function countNamedCards(cards: StrategyCard[] = [], name: string | null) {
  return (cards || []).filter((card) => nameOf(card) === name).length;
}

function countCardsByNames(cards: StrategyCard[] = [], names: Set<string | null> = new Set()) {
  return (cards || []).filter((card) => names.has(nameOf(card))).length;
}

function countShadowHeartCards(cards: StrategyCard[] = []) {
  return (cards || []).filter(isShadowHeartCard).length;
}

function countEnginePieces(player: Player | null | undefined = {}) {
...
function countUsefulGyResources(player: Player | null | undefined = {}) {
  return getCards(player, "graveyard").filter(
    (card) => isShadowHeartCard(card) && card?.cardKind === "monster",
  ).length;
}

function getTotalAttackPower(field: StrategyCard[] = []) {
  return (field || []).reduce((sum, card) => {
...
function hasFollowUp(player: Player | null | undefined = {}) {
  return (
...
function hasHandFollowUp(player: Player | null | undefined = {}) {
  return countShadowHeartCards(getCards(player, "hand")) > 0;
}

function countReadyThreats(field: StrategyCard[] = []) {
...
function getFieldAttackers(field: StrategyCard[] = []) {
  return (field || []).filter(
    (card) =>
      card?.cardKind === "monster" &&
      !card.isFacedown &&
...
function hasActiveCathedralWithCounters(player: Player | null | undefined = {}) {
  return getCards(player, "spellTrap").some(
    (card) => nameOf(card) === SH.cathedral && getCathedralCounters(card) > 0,
  );
}

function hasValleyWithRelevantAttacker(player: Player | undefined = {}) {
...
function hasOpponentBackrowEngine(opponent: Player = {}) {
  return (
...
function scoreEngineMilestones(scoreState: ScoreState, context: ScoringContext) {
...
function scoreConversionMilestones(scoreState: ScoreState, context: ScoringContext) {
  const { initialBot, finalBot, sequence, usedNames, impact } = context;
  const finalHand = getCards(finalBot, "hand");
...
function scoreBossRemovalMilestones(scoreState: ScoreState, context: ScoringContext) {
  const { usedNames, impact } = context;
  const reached = (name: string) => impact.finalNames.has(name) && !impact.initialNames.has(name);
  const stayedWithImpact = (name: string) =>
...
function scoreDamageClosingMilestones(scoreState: ScoreState, context: ScoringContext) {
...
function scoreBattleBridgeMilestones(scoreState: ScoreState, context: ScoringContext) {
...
function scoreGlobalLinePenalties(scoreState: ScoreState, context: ScoringContext) {
...
function scoreTerminalQuality(context: ScoringContext = {} as ScoringContext) {
...
function scoreTerminalRisk(context: ScoringContext = {} as ScoringContext) {
  const { initialBot, finalBot, finalOpponent, sequence, usedNames, impact } =
    context;
  const finalField = getCards(finalBot, "field");
  const finalHand = getCards(finalBot, "hand");
...
function scoreTerminalAdjustments(context: ScoringContext = {} as ScoringContext) {
  return scoreTerminalQuality(context) + scoreTerminalRisk(context);
}

function getRetentionFloor(action: Partial<LineAction> = {}, analysis: Analysis = {}) {
...
export function applyShadowHeartCandidateRetention(actions: AIAction[] = [], analysis: Analysis = {}, context: Context = {}) {
...
export function buildShadowHeartPlanningProfile(analysis: Analysis = {}, context: Context = {}): Profile {
  const game: PlanningGame = context.game || analysis.game || {};
...
function addCounter(card: { counters?: Map<string, number> | object; addCounter?(type: string, amount: number): void }, counterType: string, amount = 1) {
  ...
  if (typeof card.addCounter === "function") {
    ...
    (card.counters as Record<string, number>)[counterType] = next;
  }
  return next;
}
...
export function applyShadowHeartSimulatedBattleRewards({
  ...
  summary,
  bot,
}: { state?: State; summary?: BattleSummary; bot?: Player } = {}): string[] {
  const player: Player = bot || state?.bot || {};
  const opponent: Player = state?.player || {};
...
export function scoreShadowHeartBattleAttackCandidate(context: { attacker?: StrategyCard | null; target?: StrategyCard | null; lethalNow?: boolean; attackerSurvived?: boolean; targetSurvived?: boolean; bot?: Player } = {}) {
  const {
    attacker,
    target,
    lethalNow = false,
...
export function scoreShadowHeartLineMilestones(context: Context = {}) {
  ...
  const scoreState: ScoreState = { score: 0, milestones: [] };
...
export function scoreShadowHeartLineTerminal(context: Context = {}) {
  const initialState = context.initialState || {};
  const finalState = context.finalState || {};
  const initialBot = getBotState(initialState);
  const finalBot = getBotState(finalState);
  ...
  const profile: Partial<Profile> = context.profile || {};
...
export function describeShadowHeartPlannedLine(context: Context = {}) {
  const sequence = context.sequence || [];
