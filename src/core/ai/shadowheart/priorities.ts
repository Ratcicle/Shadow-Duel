...
type Player = Omit<Partial<AIStrategyBotPort>, "field"> & { field?: StrategyCard[] };
...
function isShadowHeartDragon(card: StrategyCard) {
  return (
...
function cardCountByName(cards: StrategyCard[] = []) {
...
function allCards(analysis: Analysis, zones: Array<"hand" | "field" | "graveyard" | "deck" | "extraDeck" | "spellTrap"> = ["hand", "field"]) {
...
function hasShadowHeartStarter(cards: StrategyCard[] = []) {
  return cards.some((card) => SHADOW_HEART_STARTERS.includes(card?.name!));
}

function hasLevel8PlusShadowHeart(cards: StrategyCard[] = [], { excludeScale = false } = {}) {
  return cards.some(
    (card) =>
      card &&
      card.cardKind === "monster" &&
...
function hasUsefulImpTarget(cards: StrategyCard[] = []) {
  return cards.some(
    (card) =>
      card &&
      card.cardKind === "monster" &&
...
function hasOpponentPressure(analysis: Analysis) {
...
function getCandidateByName(candidates: StrategyCard[] = [], names: string[] = []) {
...
function isReadyShadowHeartAttacker(card: StrategyCard) {
  return (
    card &&
    card.cardKind === "monster" &&
    isShadowHeartByName(card.name) &&
...
function heartbearerCompletesTributeLine(analysis: Analysis = {}) {
  const field = analysis.field || [];
...
function isExtraDeckBoss(monster: StrategyCard) {
...
export function getTributeRequirementFor(card: StrategyCard, playerState: Player) {
  ...
  const alt = card.altTribute as (GameCard["altTribute"] & { type?: string }) | null | undefined;
  if (
