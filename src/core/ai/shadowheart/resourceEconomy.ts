import type { ShadowHeartCard, ShadowHeartAnalysis } from "./contracts.js";
...
function isShadowHeartMonster(card: StrategyCard) {
  return (
...
function cardsIn(analysis: Analysis = {}, zone: "hand" | "field" | "graveyard") {
  const cards = analysis?.[zone];
  return Array.isArray(cards) ? cards : [];
}

function hasName(cards: StrategyCard[] = [], name: string) {
  return cards.some((card) => card?.name === name);
}

function countLowAtkReviveTargets(cards: StrategyCard[] = []) {
  return cards.filter(
    (card) => isShadowHeartMonster(card) && (card.atk || 0) <= 1600,
  ).length;
}

function buildZoneSummary(analysis: Analysis = {}) {
...
function getEnablers(analysis: Analysis = {}) {
...
function canAccessGraveyard(enablers: Partial<ReturnType<typeof getEnablers>> = {}) {
  return (
...
function computeAccessibility({ countsByZone, enablers }: Pick<ResourceContext, "countsByZone" | "enablers">) {
...
function computePotential({ analysis, countsByZone, enablers }: Pick<ResourceContext, "analysis" | "countsByZone" | "enablers">) {
  const hand = cardsIn(analysis, "hand");
  const field = cardsIn(analysis, "field");
  const graveyard = cardsIn(analysis, "graveyard");
...
export function buildShadowHeartResourceEconomy(analysis: Analysis = {}) {
  return analyzeResourceEconomy<Analysis, ReturnType<typeof getEnablers>, ReturnType<typeof computePotential>, ReturnType<typeof computeFlags>>(analysis, {
...
export function getShadowHeartResourcePressure(analysis: Analysis = {}, context: Parameters<typeof scoreResourcePressure>[2] = {}) {
  const economy =
...
export function buildShadowHeartResourcePreferences(analysis: Analysis = {}) {
  const economy =
    analysis.resourceEconomy || buildShadowHeartResourceEconomy(analysis);
