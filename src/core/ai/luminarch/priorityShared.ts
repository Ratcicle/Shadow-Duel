...
export function hasLoggedPriorityError(errorKey: string) {
  return _loggedErrors.has(errorKey);
}

export function markPriorityErrorLogged(errorKey: string) {
  _loggedErrors.add(errorKey);
}

export function getBattleStatForTarget(card: SimulatedCardState) {
  return getBattleStatForAttackTarget(card, { facedownValue: 0 });
}

export function getBattleReadyLuminarchAttackers(analysis: LuminarchAnalysis) {
  return (analysis.field || []).filter((card) =>
    isBattleReadyAttacker(card, { archetype: "Luminarch" })
  );
}

export function getBestTemporaryCombatDebuffTarget(analysis: LuminarchAnalysis) {
...
export function getBattleStatToAttack(card: SimulatedCardState) {
  return getBattleStatForAttackTarget(card);
}

export function getThreatAtk(card: SimulatedCardState) {
  return getAttackThreatStat(card, { facedownValue: 1500 });
}

export function isDefensiveLuminarch(card: SimulatedCardState) {
