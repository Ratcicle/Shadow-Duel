...
const NAMES: { aegisbearer: string; arbiter: string; aurora: string; barbarias: string; citadel: string; crescentShield: string; enchantedHalberd: string; fortress: string; holyAscension: string; holyShield: string; knightsConvocation: string; magicSickle: string; marshal: string; moonblade: string; moonlit: string; polymerization: string; protector: string; pureKnight: string; radiantLancer: string; radiantWave: string; sacredJudgment: string; spear: string; sunforgedBlade: string; valiant: string } = Object.freeze({
...
const HIGH_VALUE_SICKLE_SPELL_TARGETS = new Set<string | undefined>([
  NAMES.citadel,
  NAMES.holyAscension,
  NAMES.holyShield,
  NAMES.moonlit,
...
function getZones(analysis: LuminarchAnalysis = {}) {
...
function hasName(cards: SimulatedCardState[] = [], name: string) {
  return cards.some((card) => card?.name === name);
}

function hasAnyName(cards: SimulatedCardState[] = [], names: string[] = []) {
  return cards.some((card) => card?.name && names.includes(card.name!));
}

function countMatching(cards: SimulatedCardState[] = [], predicate: (card: SimulatedCardState) => boolean) {
  return cards.filter((card) => card && predicate(card)).length;
}

function countLuminarchMonsters(cards: SimulatedCardState[] = []) {
  return countMatching(cards, isLuminarchMonster);
}

function collectNames(cards: SimulatedCardState[] = []) {
  return [...new Set(cards.map((card) => card?.name).filter(Boolean))];
}

function hasUsefulSickleSpellTarget(graveyard: SimulatedCardState[] = []) {
...
function hasSpellRecoveryTarget(graveyard: SimulatedCardState[] = []) {
  return hasName(graveyard, NAMES.magicSickle) && hasUsefulSickleSpellTarget(graveyard);
}

function buildPackageState(analysis: LuminarchAnalysis = {}) {
...
function packageData(packageId: string) {
  return COMBO_BY_PACKAGE.get(packageId as typeof COMBO_DATABASE[number]["package"]) || null;
}

function makePackage(packageId: string, details: { priorityBoost?: number; status?: string; ready?: boolean; milestone?: string; signals?: string[]; cards?: Array<string | undefined>; conditions?: object; diagnostics?: object; description?: string; steps?: string[] } = {}) {
...
function evaluateStarterPackage(state: ReturnType<typeof buildPackageState>) {
...
function evaluateCitadelPackage(state: ReturnType<typeof buildPackageState>) {
...
function evaluateWallPackage(state: ReturnType<typeof buildPackageState>) {
...
function evaluateFusionPackage(state: ReturnType<typeof buildPackageState>) {
...
function evaluateAscensionPackage(state: ReturnType<typeof buildPackageState>) {
...
function evaluateGrindPackage(state: ReturnType<typeof buildPackageState>) {
...
function evaluateBattleConversionPackage(state: ReturnType<typeof buildPackageState>) {
  const battleCards = [
    NAMES.spear,
    NAMES.magicSickle,
    NAMES.holyAscension,
...
function evaluateLpPayoffPackage(state: ReturnType<typeof buildPackageState>) {
  const hasLpPayoff =
    state.hasCitadelActive ||
...
function evaluateComebackPackage(state: ReturnType<typeof buildPackageState>) {
...
export function detectAvailableLinePackages(analysis: LuminarchAnalysis = {}) {
  if (!analysis) return [];
  const state = buildPackageState(analysis);
  return PACKAGE_EVALUATORS.map((evaluate) => evaluate(state))
    .filter(Boolean)
    .sort((a, b) => (b!.priority || 0) - (a!.priority || 0)) as Array<NonNullable<ReturnType<typeof makePackage>>>;
}

export function detectAvailableCombos(analysis: LuminarchAnalysis = {}) {
  return detectAvailableLinePackages(analysis);
}

export function getLinePackageById(packageId: string) {
  return packageData(packageId);
}

export function getLinePackageCoverage(packageId: string) {
  return packageData(packageId)?.coveredCombos || [];
}

export function getLinePackageMilestones(packageId: string) {
...
export function shouldExecuteCombo(combo: NonNullable<ReturnType<typeof makePackage>> | null) {
...
export function shouldPrioritizeDefense(analysis: LuminarchAnalysis = {}) {
  const state = buildPackageState(analysis);
...
export function shouldTurtleStrategy(analysis: LuminarchAnalysis = {}) {
  const state = buildPackageState(analysis);
...
export function canAttemptLethal(analysis: LuminarchAnalysis = {}) {
  const state = buildPackageState(analysis);
