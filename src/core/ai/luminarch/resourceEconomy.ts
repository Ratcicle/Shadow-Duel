...
const LOW_VALUE_COST_NAMES = new Set<string | undefined>([
  LUMINARCH.enchantedHalberd,
  LUMINARCH.valiant,
  LUMINARCH.arbiter,
]);

const RECOVERY_ENABLER_NAMES = new Set<string | undefined>([
...
const CORE_PRESERVE_NAMES = new Set<string | undefined>([
...
const MAGIC_SICKLE_SPELL_TARGET_NAMES = new Set<string | undefined>([
  LUMINARCH.citadel,
...
function cardsIn(analysis: LuminarchAnalysis = {}, zone: "hand" | "field" | "graveyard" | "deck" | "extraDeck" | "spellTrap" | "oppField") {
  const cards = analysis?.[zone];
  return Array.isArray(cards) ? cards : [];
}

function cardNames(cards: SimulatedCardState[] = []) {
  return new Set(cards.map((card) => card?.name).filter(Boolean));
}

function hasFaceupName(cards: SimulatedCardState[] = [], name: string) {
  return cards.some(
    (card) => card?.name === name && card.cardKind === "monster" && !card.isFacedown,
  );
}

function countHighValueReviveTargets(cards: SimulatedCardState[] = []) {
  return cards.filter(
    (card) =>
...
function countLowDefReviveTargets(cards: SimulatedCardState[] = []) {
  return cards.filter(
    (card) => isLuminarchMonster(card) && (card.def || 0) <= 2000,
  ).length;
}

function countLowLevelReviveTargets(cards: SimulatedCardState[] = []) {
  return cards.filter(
    (card) => isLuminarchMonster(card) && (card.level || 0) <= 4,
  ).length;
}

function countUsefulMagicSickleSpellTargets(cards: SimulatedCardState[] = []) {
  return cards.filter(
    (card) =>
      card &&
      card.cardKind === "spell" &&
...
function buildZoneSummary(analysis: LuminarchAnalysis = {}) {
...
function getEnablers(analysis: LuminarchAnalysis = {}) {
...
function computeAccessibility({ analysis, countsByZone, enablers }: { analysis: LuminarchAnalysis; countsByZone: Record<string, number>; enablers: ReturnType<typeof getEnablers> }) {
  const graveyard = cardsIn(analysis, "graveyard");
...
function computePotential({ analysis, countsByZone, enablers }: { analysis: LuminarchAnalysis; countsByZone: Record<string, number>; enablers: ReturnType<typeof getEnablers> }) {
  const graveyard = cardsIn(analysis, "graveyard");
...
export function buildLuminarchResourceEconomy(analysis: LuminarchAnalysis = {}): LuminarchResourceEconomy {
  return analyzeResourceEconomy<LuminarchAnalysis, ReturnType<typeof getEnablers>, ReturnType<typeof computePotential>, ReturnType<typeof computeFlags>>(analysis, {
...
export function getLuminarchResourcePressure(analysis: LuminarchAnalysis = {}, context: Parameters<typeof scoreResourcePressure>[2] = {}) {
  const economy =
...
export function assessLuminarchResourceSpend(
  ...
  spend: NonNullable<Parameters<typeof assessResourceSpend>[0]>["spend"] = {},
  context: Parameters<typeof scoreResourcePressure>[2] = {},
) {
  const economy =
    analysis.resourceEconomy || buildLuminarchResourceEconomy(analysis);
...
export function assessLuminarchResourceRecovery(
  analysis: LuminarchAnalysis = {},
  recovery: NonNullable<Parameters<typeof assessResourceRecovery>[0]>["recovery"] = {},
  context: Parameters<typeof scoreResourcePressure>[2] = {},
) {
  const economy =
    analysis.resourceEconomy || buildLuminarchResourceEconomy(analysis);
...
export function buildLuminarchResourcePreferences(analysis: LuminarchAnalysis = {}) {
  const economy =
    analysis.resourceEconomy || buildLuminarchResourceEconomy(analysis);
  const pressure = scoreResourcePressure(economy, LUMINARCH_RESOURCE_POLICY);
  const preferNames = new Set<string>();
  ...
  const potential: Partial<ReturnType<typeof computePotential>> = economy.potential || {};
