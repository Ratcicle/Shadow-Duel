...
import type { LuminarchContext, LuminarchPlayer } from "./contracts.js";
import {
...
export function isRadiantLancer(card: SimulatedCardState) {
  return card?.name === "Luminarch Radiant Lancer";
}

export function spendsAegisbearerProtectorCore(cards: SimulatedCardState[]) {
...
export function getLuminarchTributeRequirementFor(card: SimulatedCardState, playerState: SimulatedPlayerState) {
  return getTributeRequirementFor(card, playerState);
}

function getLuminarchBoardValue(card: SimulatedCardState, context: LuminarchContext = {}) {
  ...
  if (
    (card.effects || []).some(
      (effect) =>
        ...
        (effect as { replacementEffect?: {type?: string} }).replacementEffect?.type === "destruction",
    )
  ) {
    value += 2;
  }
...
function isHighValueLuminarchTribute(card: SimulatedCardState, context: LuminarchContext = {}) {
...
function getBestMoonbladeReviveTarget(bot: Partial<LuminarchPlayer>) {
...
function evaluateLuminarchTributeSummonPayoff(cardToSummon: SimulatedCardState, tributes: readonly SimulatedCardState[], context: LuminarchContext) {
  const bot: Partial<SimulatedPlayerState> = context.bot || {};
  const opponent: Partial<SimulatedPlayerState> = context.opponent || {};
...
function evaluateLuminarchTributeKeepScore(card: SimulatedCardState, evaluationContext: LuminarchContext, context: LuminarchContext) {
...
function buildLuminarchEvaluationContext(botState: Partial<LuminarchPlayer> = {}, field: SimulatedCardState[] = []) {
  return {
    field: field || [],
...
export function selectBestLuminarchTributes(
  ...
  context: LuminarchContext = {},
) {
  const botState = context.botState || {};
  return selectBestTributes<SimulatedCardState, LuminarchContext>(
    field,
    tributesNeeded,
    cardToSummon,
    {
...
export function evaluateLuminarchTributeSummonCost(
  cardToSummon: SimulatedCardState,
  tributes: SimulatedCardState[],
  context: LuminarchContext = {},
) {
  const bot: Partial<SimulatedPlayerState> = context.bot || {};
  return evaluateTributeSummonCost<SimulatedCardState, LuminarchContext>(
    cardToSummon,
    tributes,
    {
      ...context,
