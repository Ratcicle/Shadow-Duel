...
import type { LuminarchAnalysis, LuminarchHooks, LuminarchPlan, LuminarchGame } from "./contracts.js";
import {
  createFinisherPlan,
  getBestFinisherPlan,
  rankFinisherPlans,
...
function getMonsters(cards: SimulatedCardState[] = []) {
  return (cards || []).filter((card) => card && card.cardKind === "monster");
}

function getMaterialEntries(bot: SimulatedPlayerState) {
  return [
...
function hasDistinctMegashieldMaterials(bot: SimulatedPlayerState) {
...
function canSummonMegashield(bot: SimulatedPlayerState | null | undefined, game: LuminarchGame | null, fusionCard: SimulatedCardState) {
...
function canActivatePolymerization(polyCard: SimulatedCardState, bot: SimulatedPlayerState, game: LuminarchGame | null, activationContext = null) {
...
function evaluateBarbariasActionPriority(bot: SimulatedPlayerState, opponent: SimulatedPlayerState | null | undefined, defensePlan: Partial<ReturnType<typeof import("./defensePlanning.js").evaluateLuminarchDefensePlan>>, hooks: LuminarchHooks = {}) {
  let priority = 10;
...
export function evaluateLuminarchFusionPlan(
  ...
  game: LuminarchGame | null = null,
  analysis: LuminarchAnalysis | null = null,
  ...
  return createFinisherPlan<LuminarchPlan["details"]>({
    kind: "fusion",
...
function findFortressAscensionCandidate(bot: SimulatedPlayerState, game: LuminarchGame | null) {
...
function evaluateFortressActionPriority(material: SimulatedCardState, bot: SimulatedPlayerState, opponent: SimulatedPlayerState | null | undefined, defensePlan: Partial<ReturnType<typeof import("./defensePlanning.js").evaluateLuminarchDefensePlan>>) {
  let priority = 11;
...
function getTributeRequirement(card: SimulatedCardState) {
...
function canNormalSummonFromHand(card: SimulatedCardState, analysis: LuminarchAnalysis) {
  if (!card || card.cardKind !== "monster") return false;
  if ((analysis?.normalSummonsAvailable ?? 1) <= 0) return false;
  return (analysis?.field || []).length >= getTributeRequirement(card);
}

function evaluateLuminarchNormalFinisher(card: SimulatedCardState, analysis: LuminarchAnalysis, defensePlan: Partial<ReturnType<typeof import("./defensePlanning.js").evaluateLuminarchDefensePlan>>) {
...
function evaluateNormalSummonFinisherPlans(bot: SimulatedPlayerState, opponent: SimulatedPlayerState | null | undefined, game: LuminarchGame | null, analysis: LuminarchAnalysis, defensePlan: Partial<ReturnType<typeof import("./defensePlanning.js").evaluateLuminarchDefensePlan>>) {
...
export function getBestLuminarchFinisherPlan(plans: LuminarchPlan[] = [], predicateOrKind: ((plan: LuminarchPlan) => boolean) | string | null = null) {
