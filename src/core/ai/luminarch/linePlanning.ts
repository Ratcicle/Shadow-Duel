...
import type { SimulatedCardState, SimulatedPlayerState, AiStateShape } from "../../contracts/aiState.js";
import type { LuminarchAnalysis, LuminarchContext, LuminarchLineAction } from "./contracts.js";
import {
...
const NAMES: { aegisbearer: string; arbiter: string; barbarias: string; citadel: string; fortress: string; knightsConvocation: string; magicSickle: string; marshal: string; moonblade: string; moonlit: string; polymerization: string; protector: string; pureKnight: string; aurora: string; radiantLancer: string; holyAscension: string; holyShield: string; crescentShield: string; radiantWave: string; sacredJudgment: string; spear: string; sunforgedBlade: string; valiant: string } = Object.freeze({
...
const USEFUL_SICKLE_SPELL_TARGETS = new Set<string | undefined>([
  NAMES.citadel,
...
const WALL_NAMES = new Set<string | undefined>([
  NAMES.aegisbearer,
  NAMES.aurora,
  NAMES.barbarias,
  NAMES.fortress,
...
const LP_COST_FOLLOW_UP_NAMES = new Set<string | undefined>([
  NAMES.citadel,
  NAMES.holyAscension,
  NAMES.sacredJudgment,
]);

const LP_PAYOFF_NAMES = new Set<string | undefined>([
  NAMES.aurora,
  NAMES.barbarias,
  NAMES.citadel,
  NAMES.holyShield,
...
const MAIN2_BATTLE_PAYOFF_NAMES = new Set<string | undefined>([
  NAMES.barbarias,
  NAMES.fortress,
  NAMES.moonlit,
  NAMES.pureKnight,
...
const BATTLE_SIM_MILESTONE_KEYS = new Set<string | undefined>([
...
const MAIN_ONLY_PACKAGES = new Set<string | undefined>([
...
const MAIN_BATTLE_PACKAGES = new Set<string | undefined>([
  ...MAIN_ONLY_PACKAGES,
  LUMINARCH_LINE_PACKAGES.BATTLE_CONVERSION,
  LUMINARCH_LINE_PACKAGES.LP_PAYOFF,
]);

function cards(analysis: LuminarchAnalysis = {}, zone: "hand" | "field" | "graveyard" | "deck" | "extraDeck" | "spellTrap" | "oppField" | "oppSpellTrap") {
  const value = analysis?.[zone];
  return Array.isArray(value) ? value.filter(Boolean) : [];
}

function hasName(list: SimulatedCardState[] = [], name: string) {
  return list.some((card) => card?.name === name);
}

function isLuminarch(card: SimulatedCardState) {
  if (!card) return false;
  if (card.archetype === "Luminarch") return true;
  return Array.isArray(card.archetypes) && card.archetypes.includes("Luminarch");
}

function isLuminarchMonster(card: SimulatedCardState) {
  return card?.cardKind === "monster" && isLuminarch(card);
}

function isFaceupLuminarchMonster(card: SimulatedCardState) {
  return isLuminarchMonster(card) && !card.isFacedown;
}

function getOpponentTotalAttack(analysis: LuminarchAnalysis = {}) {
...
function scoreMagicSickleSpellTarget(card: SimulatedCardState, analysis: LuminarchAnalysis = {}) {
  if (!card) return 0;
...
function countLuminarchMaterials(analysis: LuminarchAnalysis = {}) {
  return [...cards(analysis, "hand"), ...cards(analysis, "field")].filter(
    isLuminarchMonster,
  ).length;
}

function hasPureKnightFusionAccess(analysis: LuminarchAnalysis = {}) {
  return (
...
function hasBarbariasFusionAccess(analysis: LuminarchAnalysis = {}) {
...
function opponentThreatensLethal(analysis: LuminarchAnalysis = {}) {
...
function hasStarterAccess(analysis: LuminarchAnalysis = {}) {
...
function hasCitadelAccess(analysis: LuminarchAnalysis = {}) {
  const hand = cards(analysis, "hand");
...
function hasFortressRevive(analysis: LuminarchAnalysis = {}) {
...
function hasMagicSickleRecovery(analysis: LuminarchAnalysis = {}) {
  const gy = cards(analysis, "graveyard");
  return (
...
function hasAegisbearerFortressSetup(analysis: LuminarchAnalysis = {}) {
...
function hasComebackOrPressure(analysis: LuminarchAnalysis = {}) {
  const hand = cards(analysis, "hand");
...
function isMain1Phase(phase: string | null | undefined) {
  const normalized = String(phase || "main1").toLowerCase();
  return normalized === "main1" || normalized === "main";
}

function isBattleReadyAttacker(card: SimulatedCardState) {
  return (
...
function collectBattleReadyAttackers(analysis: LuminarchAnalysis = {}) {
  return cards(analysis, "field").filter(isBattleReadyAttacker);
}

function getPotentialBattleAttackers(analysis: LuminarchAnalysis = {}) {
...
function getTargetBattleStat(card: SimulatedCardState) {
  return getBattleStatForAttackTarget(card, { facedownValue: 1500 });
}

function canDestroyBattleTarget(attacker: SimulatedCardState, target: SimulatedCardState, atkBoost = 0) {
...
function canAnyAttackerDestroyTarget(attackers: SimulatedCardState[] = [], targets: SimulatedCardState[] = [], atkBoost = 0) {
  return attackers.some((attacker) =>
    targets.some((target) => canDestroyBattleTarget(attacker, target, atkBoost)),
  );
}

function hasDirectOrNearLethal(analysis: LuminarchAnalysis = {}, attackers: SimulatedCardState[] = []) {
...
function hasBattleMain2Payoff(analysis: LuminarchAnalysis = {}) {
  const hand = cards(analysis, "hand");
  const field = cards(analysis, "field");
  const gy = cards(analysis, "graveyard");
...
function collectBattleBridgeSignals(analysis: LuminarchAnalysis = {}, context: LuminarchContext = {}) {
...
function getBattleStepLimit(analysis: LuminarchAnalysis = {}, battleSignals: string[] = []) {
  const readyAttackers = collectBattleReadyAttackers(analysis);
...
function collectPlanningReasons(analysis: LuminarchAnalysis = {}) {
...
function isMain2(phase: string | null | undefined) {
...
function isCriticalProfile(profile: Partial<AIPlanningProfile> & { critical?: boolean } = {}) {
  return profile.critical === true || profile.mode === "critical";
}

function getPlanningBot(state: Partial<AiStateShape> = {}): Partial<SimulatedPlayerState> {
  return state?.bot || state?.player || {};
}

function getPlanningOpponent(state: Partial<AiStateShape> = {}): Partial<SimulatedPlayerState> {
  if (state?.bot) return state.player || state.opponent || {};
  return state?.opponent || {};
}

function stateCards(player: Partial<SimulatedPlayerState> = {}, zone: "hand" | "field" | "spellTrap" | "graveyard" | "deck" | "extraDeck" | "banished" | "fieldSpell") {
  if (!player) return [];
...
function hasStateName(player: Partial<SimulatedPlayerState> = {}, zone: "hand" | "field" | "spellTrap" | "graveyard" | "deck" | "extraDeck" | "banished" | "fieldSpell", name: string) {
  return stateCards(player, zone).some((card) => card?.name === name);
}

function countBoardCards(player: Partial<SimulatedPlayerState> = {}) {
  return (
...
function getAttackDamage(player: Partial<SimulatedPlayerState> = {}) {
...
function stateThreatensLethal(defender: Partial<SimulatedPlayerState> = {}, attacker: Partial<SimulatedPlayerState> = {}) {
  const lp = Number(defender?.lp || 0);
  if (lp <= 0) return true;
  return getAttackDamage(attacker) >= lp;
}

function hasCitadelOnline(player: Partial<SimulatedPlayerState> = {}) {
  return (
...
function countFaceupLuminarchs(player: Partial<SimulatedPlayerState> = {}) {
  return stateCards(player, "field").filter(isFaceupLuminarchMonster).length;
}

function isLuminarchWall(card: SimulatedCardState) {
...
function countWalls(player: Partial<SimulatedPlayerState> = {}) {
  return stateCards(player, "field").filter(isLuminarchWall).length;
}

function countFusionPayoffs(player: Partial<SimulatedPlayerState> = {}) {
...
function countUsefulGyLuminarchs(player: Partial<SimulatedPlayerState> = {}) {
...
function getActionNames(sequence: LuminarchLineAction[] = []) {
  return (sequence || [])
    .map((action) => action?.cardName || action?.card?.name || action?.name)
    .filter(Boolean);
}

function sequenceUses(sequence: LuminarchLineAction[] = [], name: string, type: string | null = null) {
...
function getSimulatedBattleSteps(sequence: LuminarchLineAction[] = []) {
  return (sequence || []).filter((action) => action?.type === "simulatedBattle");
}

function getDetailedBattleSteps(sequence: LuminarchLineAction[] = []) {
...
function getBattleRewardNames(steps: LuminarchLineAction[] = []) {
  return steps.flatMap((step) =>
    Array.isArray(step?.rewardNames) ? step.rewardNames.filter(Boolean) : [],
  );
}

function getBattleLpGain(steps: LuminarchLineAction[] = []) {
  return steps.reduce(
    (sum, step) =>
      sum +
      (step?.lpGains || []).reduce(
...
function getDestroyedBattleCards(steps: LuminarchLineAction[] = [], owner: string) {
  return steps.flatMap((step) =>
...
function getDestroyedBattleCardValue(card: import("../../contracts/aiState.js").SimulatedCardShape) {
...
function isDestroyedBattleWall(card: import("../../contracts/aiState.js").SimulatedCardShape) {
  if (!card || card.cardKind !== "monster") return false;
  if (WALL_NAMES.has(card.name)) return true;
  return Number(card.def || 0) >= 2400;
}

function getMain2PayoffActionsAfterBattle(sequence: LuminarchLineAction[] = []) {
...
export function isLuminarchMain2BattlePayoffAction(action: LuminarchLineAction = {}) {
  return MAIN2_BATTLE_PAYOFF_NAMES.has(
    action?.cardName || action?.card?.name || action?.name,
  );
}

function rewardNameMatches(rewards: unknown[] = [], pattern: RegExp) {
  return rewards.some((name) => pattern.test(String(name || "")));
}

function getLineImpact(context: LuminarchContext = {}) {
...
function hasStateCard(player: Partial<SimulatedPlayerState> = {}, zones: Array<Parameters<typeof stateCards>[1]> = [], predicate: (card: SimulatedCardState) => boolean = () => false) {
  return zones.some((zone) => stateCards(player, zone).some(predicate));
}

function countStateCards(player: Partial<SimulatedPlayerState> = {}, zones: Array<Parameters<typeof stateCards>[1]> = [], predicate: (card: SimulatedCardState) => boolean = () => true) {
  return zones.reduce(
...
function hasStateNameInZones(player: Partial<SimulatedPlayerState> = {}, zones: Array<Parameters<typeof stateCards>[1]> = [], name: string) {
  return hasStateCard(player, zones, (card) => card?.name === name);
}

function getBattleWallValue(card: SimulatedCardState) {
  if (!card || card.cardKind !== "monster") return 0;
  const atk =
    Number(card.atk || 0) +
...
function getBestWallValue(player: Partial<SimulatedPlayerState> = {}) {
...
function hasWallOverThreat(player: Partial<SimulatedPlayerState> = {}, opponent: Partial<SimulatedPlayerState> = {}) {
...
function hasProtectedWall(player: Partial<SimulatedPlayerState> = {}, opponent: Partial<SimulatedPlayerState> = {}) {
...
function countLpCostFollowUps(player: Partial<SimulatedPlayerState> = {}) {
...
function hasLpPayoffEngine(player: Partial<SimulatedPlayerState> = {}, options: { excludeNames?: string[] } = {}) {
...
function hasFortressReviveFollowUp(player: Partial<SimulatedPlayerState> = {}) {
...
function hasGrindFollowUp(player: Partial<SimulatedPlayerState> = {}) {
...
function hasRealAttackPressure(player: Partial<SimulatedPlayerState> = {}, opponent: Partial<SimulatedPlayerState> = {}) {
...
function getLpPayments(impact: ReturnType<typeof getLineImpact> = {} as ReturnType<typeof getLineImpact>): Array<Pick<import("../../contracts/aiState.js").SimulatedLuminarchLpPayment, "cardName" | "cost" | "beforeLp" | "afterLp"> & Partial<import("../../contracts/aiState.js").SimulatedLuminarchLpPayment>> {
...
function scoreTerminalQuality(impact: ReturnType<typeof getLineImpact> = {} as ReturnType<typeof getLineImpact>) {
...
function scoreTerminalLpPolicy(impact: ReturnType<typeof getLineImpact> = {} as ReturnType<typeof getLineImpact>) {
...
function scoreTerminalRisk(impact: ReturnType<typeof getLineImpact> = {} as ReturnType<typeof getLineImpact>) {
  const finalBot = impact.finalBot || {};
  const finalOpponent = impact.finalOpponent || {};
...
function scoreTerminalAdjustments(impact: ReturnType<typeof getLineImpact> = {} as ReturnType<typeof getLineImpact>) {
  return (
...
function addMilestone(entries: LuminarchMilestone[], score: number, label: string) {
  if (!Number.isFinite(score) || score === 0 || !label) return;
  entries.push({ score, label, weight: Math.abs(score) });
}

function addSimMetaMilestones(entries: LuminarchMilestone[], impact: ReturnType<typeof getLineImpact>) {
  const raw: Array<string | {label?: string; name?: string}> = Array.isArray(impact.simMeta?.milestones)
  ...
  raw.forEach((entry) => {
    ...
    const mapped = SIM_MILESTONE_SCORES[key as keyof typeof SIM_MILESTONE_SCORES];
    if (mapped) {
...
function scoreEngineMilestones(entries: LuminarchMilestone[], impact: ReturnType<typeof getLineImpact>) {
...
function scoreWallMilestones(entries: LuminarchMilestone[], impact: ReturnType<typeof getLineImpact>) {
...
function scoreFusionMilestones(entries: LuminarchMilestone[], impact: ReturnType<typeof getLineImpact>) {
...
function scoreGrindMilestones(entries: LuminarchMilestone[], impact: ReturnType<typeof getLineImpact>) {
...
function scoreBoardImpactMilestones(entries: LuminarchMilestone[], impact: ReturnType<typeof getLineImpact>) {
...
function scoreBattleBridgeMilestones(entries: LuminarchMilestone[], impact: ReturnType<typeof getLineImpact>) {
  ...
  if (rewardNameMatches(rewards, /Sunforged/)) {
    const sunforgedEvents = rewards.filter((name) => /Sunforged/.test(name as string)).length;
    addMilestone(
      entries,
...
function scoreRiskMilestones(entries: LuminarchMilestone[], impact: ReturnType<typeof getLineImpact>) {
  if (
    impact.finalFaceupLuminarchs === 0 &&
...
function formatMilestone(entry: LuminarchMilestone) {
  const sign = entry.score! > 0 ? "+" : "";
  const rounded = Number(entry.score.toFixed(1));
  return `${sign}${rounded} ${entry.label}`;
}

export function buildLuminarchPlanningProfile(analysis: LuminarchAnalysis = {}, context: LuminarchContext = {}) {
  const game = context.game || analysis.game || {};
  ...
  return {
    ...DEFAULT_PROFILE,
    enabled,
    mode: (manual && reasons.length === 0 ? "manual" : enabled ? "critical" : "off") as AIPlanningProfile["mode"],
    turnMode,
...
function collectSimMilestones(state: Partial<AiStateShape> | null | undefined = {}) {
  const raw = state?._simLuminarch?.milestones as Array<string | { label?: string; score?: number }> | undefined;
...
export function scoreLuminarchLineMilestones(context: LuminarchContext = {}) {
  const impact = getLineImpact(context);
  const entries: LuminarchMilestone[] = [];
...
export function scoreLuminarchLineTerminal(context: LuminarchContext = {}) {
  const finalBot = getPlanningBot(context.finalState || {});
  const finalOpponent = getPlanningOpponent(context.finalState || {});
...
function describeAction(action: LuminarchLineAction = {}) {
...
function describeMilestone(entry: string | {label?: string; name?: string; milestone?: string; score?: number} | null) {
...
export function describeLuminarchPlannedLine(context: LuminarchContext = {}) {
  const sequence = Array.isArray(context.sequence) ? context.sequence : [];
...
interface LuminarchMilestone { label: string; score: number; weight: number; }
