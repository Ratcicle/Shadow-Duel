...
function getSelfLuminarchTargetIds(effect: import("../../contracts/effects.js").EffectDefinition) {
  return new Set(
    (effect?.targets || [])
      .filter(
        (target) =>
...
function getStatBuffOptionsFromEffect(source: SimulatedCardState, effect: import("../../contracts/effects.js").EffectDefinition, sourceZone: string) {
  ...
  (effect.actions || []).forEach((action) => {
    ...
    if (!targetIds.has((action as {targetRef?: string}).targetRef!)) return;
...
function getMoonlitBuffOptions(analysis: LuminarchAnalysis) {
  const options: BuffOption[] = [];
...
function getBestAttackLine(projectedAtk: number, oppMonsters: SimulatedCardState[]) {
  return (oppMonsters || [])
    .map((card) => ({
      card,
...
export function evaluateMoonlitReviveCandidate(card: SimulatedCardState, analysis: LuminarchAnalysis = {}): MoonlitCandidate {
  ...
  let position: "attack" | "defense" = atk >= def ? "attack" : "defense";
...
export function getMoonlitTargetPlan(analysis: LuminarchAnalysis): MoonlitTargetPlan {
