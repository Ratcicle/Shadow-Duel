...
import type { LuminarchContext, LuminarchHooks, LuminarchPlan, LuminarchGame, LuminarchActionGenerationContext, LuminarchActivationContext } from "./contracts.js";
import {
...
function mergeFusionCostPreferences(existing: Partial<import("../common/preferencePolicy.js").CostPreferences> = {}) {
  return {
    ...existing,
...
function getLuminarchFusionMaterialEntries(bot: SimulatedPlayerState) {
...
function findLuminarchFusionMaterialCombos(game: LuminarchGame, fusionCard: SimulatedCardState, bot: SimulatedPlayerState) {
...
function getMaterialZone(card: SimulatedCardState, entries: MaterialEntry[], bot: SimulatedPlayerState | null | undefined) {
...
function getFieldMaterialValue(card: SimulatedCardState, context: LuminarchContext = {}) {
  if (!card) return 0;
...
function getHandMaterialValue(card: SimulatedCardState) {
  if (!card) return 0;
...
function getFusionBoardValue(fusionCard: SimulatedCardState, context: LuminarchContext = {}) {
...
function evaluateFusionMaterialSpend(combo: SimulatedCardState[], fusionCard: SimulatedCardState, context: LuminarchContext, entries: MaterialEntry[]) {
  const bot = context.bot;
  const fieldMaterials: SimulatedCardState[] = [];
  const handMaterials: SimulatedCardState[] = [];
...
function getBestFusionMaterialSpend(combos: SimulatedCardState[][], fusionCard: SimulatedCardState, context: LuminarchContext, entries: MaterialEntry[]) {
...
function getPureKnightUtilityNeed(bot: SimulatedPlayerState | null | undefined) {
  const hasCitadel = bot?.fieldSpell?.name === CITADEL_NAME;
  const hasCitadelInHand = (bot?.hand || []).some(
    (c) => c?.name === CITADEL_NAME,
  );
...
function shouldSkipLuminarchFusionAction({
  ...
  materialCombos,
  materialEntries,
}: { targetName: string; fusionCard: SimulatedCardState; context: LuminarchContext; materialCombos: SimulatedCardState[][]; materialEntries: MaterialEntry[] }) {
...
function getContextFinisherPlans(context: LuminarchContext = {}) {
...
function getContextFinisherPlan(context: LuminarchContext, kind: string, targetName: string | null | undefined) {
...
function buildLuminarchFusionActivationContext(context: LuminarchContext, fusionPlan: Partial<LuminarchPlan>): LuminarchActivationContext {
  ...
  const baseContext: LuminarchActivationContext = {
...
export function evaluateLuminarchFusionPriority(
  ...
  if (fusionName === "Luminarch Megashield Barbarias") {
    ...
    const projectedBarbarias: import("../../contracts/aiState.js").SimulatedCardShape = {
...
function canSummonFusionTarget(game: LuminarchGame, fusionCard: SimulatedCardState, bot: SimulatedPlayerState) {
...
export function detectLuminarchFusionOpportunities(context: LuminarchContext & {game: LuminarchGame;bot: SimulatedPlayerState}) {
  ...
  try {
    ...
    const fusionCandidates = [PURE_KNIGHT_NAME, BARBARIAS_NAME]
      ...
      .filter((entry) => entry.fusionCard) as Array<{targetName: string; fusionCard: SimulatedCardState; fusionPlan: ReturnType<typeof getContextFinisherPlan>}>;
    ...
    console.warn(`[LuminarchStrategy] detectFusionOpportunities error:`, (e as Error).message);
  }

  return actions;
}

export function chooseLuminarchAscensionPosition(ascensionCard: SimulatedCardState, bot: SimulatedPlayerState, opponent: SimulatedPlayerState | null | undefined) {
...
export function detectLuminarchAscensionOpportunities(context: LuminarchContext & {game: LuminarchGame;bot: SimulatedPlayerState}) {
...
export function getLuminarchExtraDeckActions(context: LuminarchActionGenerationContext) {
  const { bot } = context;
  const actions: AIAction[] = [];
...
interface MaterialEntry { card: SimulatedCardState; zone: string }
