...
const LUMINARCH_SUMMON_ASSESSMENT_PROFILE: NonNullable<NonNullable<Parameters<typeof assessSummonEntry>[1]>["profile"]> = {
...
export function assessLuminarchSummonEntry(card: SimulatedCardState, analysis: LuminarchAnalysis = {}, context: Omit<NonNullable<Parameters<typeof assessSummonEntry>[1]>, "analysis"> = {}) {
...
function getLuminarchAttackThreat(analysis: LuminarchAnalysis) {
 ...
 */
export function shouldSummonMonster(card: SimulatedCardState, analysis: LuminarchAnalysis) {
  try {
    const name = card.name;
    const knowledge = CARD_KNOWLEDGE[name as keyof typeof CARD_KNOWLEDGE];
    ...
    const errorKey = `monster_${card?.name}_${(e as Error).message}`;
    ...
    return { yes: false, reason: `Erro interno: ${(e as Error).message}` };
  }
}
