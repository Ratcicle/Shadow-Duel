...
function getBattleDestroyAtkGain(card: SimulatedCardState) {
  return (card?.effects || []).reduce((total, effect) => {
    ...
    return total + (effect.actions || []).reduce((sum, action) => {
      const targetsSelf = !(action as { targetRef?: string }).targetRef || (action as { targetRef?: string }).targetRef === "self";
...
export function evaluateRadiantLancerBattlePlan(card: SimulatedCardState, analysis: LuminarchAnalysis = {}) {
  ...
  const evaluatedLines = destroyable.map((target) => {
    ...
    const nextThreat = remainingThreats.reduce<typeof visibleTargets[number] | null>(
