...
import type { LuminarchAnalysis, LuminarchContext } from "./contracts.js";
// ─────────────────────────────────────────────────────────────────────────────
...
function isUsefulMagicSickleSpellTarget(card: SimulatedCardState) {
  return (
    card &&
    card.cardKind === "spell" &&
    isLuminarch(card) &&
 ...
 */
export function evaluateCardExpendability(card: SimulatedCardState, context: LuminarchContext) {
 ...
 */
export function evaluateFieldSpellUrgency(analysis: LuminarchAnalysis) {
 ...
 */
export function detectSacrificialProtection(card: SimulatedCardState, context: LuminarchContext) {
  ...
  const protections: Array<{ type: string; card: string; description: string }> = [];
 ...
 */
export function evaluateRiskWithProtection(action: { type?: string; card?: SimulatedCardState }, context: LuminarchContext) {
