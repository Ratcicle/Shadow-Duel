...
export function buildLuminarchCostPreferences({
  ...
  extra = {},
}: import("../common/preferencePolicy.js").BuildCostPreferencesOptions & { analysis?: LuminarchAnalysis } = {}) {
...
function getBattleReadyLuminarchAttackers(cards: SimulatedCardState[] | undefined) {
  return (cards || []).filter((card) =>
    isBattleReadyAttacker(card, { archetype: "Luminarch" })
  );
}

function getBestSpearTargetScore(analysis: LuminarchAnalysis) {
  const attackers = getBattleReadyLuminarchAttackers(analysis.field);
  return (analysis.oppField || []).reduce<{ target: SimulatedCardState | null; score: number }>(
    (best, target) => {
...
function mergeTargetPreferences(actionContext: LuminarchActionContext, targetProfiles: object) {
...
function getMoonlitPurpose(analysis: LuminarchAnalysis) {
...
export function buildLuminarchSpellActionContext(
  ...
  baseActionContext: LuminarchActionContext = {},
) {
  ...
  if (card?.name === "Luminarch Moonlit Blessing") {
    ...
    const byName: Record<string, import("../../contracts/cards.js").BattlePositionInput> = {};
    (plan?.candidatePlans || [])
