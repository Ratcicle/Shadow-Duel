 ...
 */
export const LUMINARCH_LINE_PACKAGES: { STARTER: string; CITADEL: string; WALL: string; FUSION: string; ASCENSION: string; GRIND: string; BATTLE_CONVERSION: string; LP_PAYOFF: string; COMEBACK: string } = Object.freeze({
  STARTER: "starter",
  CITADEL: "citadel",
  WALL: "wall",
  FUSION: "fusion",
...
export const LUMINARCH_PACKAGE_STATUS: { SUPPORTED: string; PARTIAL: string; NEEDS_ACTION_GENERATION: string; NEEDS_SIMULATION: string; NEEDS_MAIN_BATTLE_MAIN2: string; REACTIVE_ENGINE_ONLY: string } = Object.freeze({
  SUPPORTED: "supported",
  PARTIAL: "partial",
 ...
 */
export function isLuminarchByName(name: string | undefined) {
 ...
 */
export function getCardKnowledge(cardName: string | undefined) {
  return CARD_KNOWLEDGE[cardName as keyof typeof CARD_KNOWLEDGE] || null;
}

/**
 * Helper: lista roles estratégicos declarados para uma carta.
 */
export function getCardRoles(cardName: string | undefined) {
  const declared = LUMINARCH_CARD_ROLES[cardName as keyof typeof LUMINARCH_CARD_ROLES] || [];
  const knowledge = CARD_KNOWLEDGE[cardName as keyof typeof CARD_KNOWLEDGE];
  const primary = knowledge?.role ? [knowledge.role] : [];
  return [...new Set<string | undefined>([...primary, ...declared])];
}

/**
 * Helper: verifica se uma carta tem determinado role.
 */
export function cardHasRole(cardName: string | undefined, role: string) {
 ...
 */
export function getCardsByRole(role: string) {
 ...
 */
export function getCardsByLinePackage(linePackage: string) {
  return Object.entries(CARD_KNOWLEDGE)
    .filter(([_, knowledge]) => ((knowledge as { linePackages?: string[] }).linePackages || []).includes(linePackage))
    .map(([name, _]) => name);
}
 ...
 */
export function getCardPriority(cardName: string | undefined) {
  const knowledge = CARD_KNOWLEDGE[cardName as keyof typeof CARD_KNOWLEDGE];
