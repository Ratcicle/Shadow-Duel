...
const LUMINARCH_SPELL_RECOVERY_TARGETS = new Set<string | undefined>([
...
function getRadiantLancerAnalysis(bot: SimulatedPlayerState, opponent: SimulatedPlayerState, game: LuminarchGame) {
  return {
    hand: bot?.hand || [],
    field: bot?.field || [],
...
function getOpponentTotalThreat(opponent: SimulatedPlayerState) {
...
function hasStableWall(bot: SimulatedPlayerState, opponent: SimulatedPlayerState) {
...
function canPayLpForLuminarchAction({
  ...
  createsWall = false,
  createsPayoff = false,
}: {bot: SimulatedPlayerState; opponent: SimulatedPlayerState; cost: number; createsWall?: boolean; createsPayoff?: boolean}) {
  const lp = bot?.lp || 0;
...
function getBestFortressReviveTarget(graveyard: SimulatedCardState[] = [], opponent: SimulatedPlayerState | null = null) {
  const oppStrongest = getStrongestAttackThreat(opponent?.field || [], {
    facedownValue: 1500,
    includeBoosts: false,
  });
...
function getUsefulSickleSpellTargets(graveyard: SimulatedCardState[] = []) {
...
function scoreSickleSpellTarget(card: SimulatedCardState, { bot, opponent }: {bot: SimulatedPlayerState; opponent: SimulatedPlayerState}) {
  if (!card) return 0;
...
function compactRadiantLancerPlan(plan: ReturnType<typeof evaluateRadiantLancerBattlePlan> | null | undefined) {
  if (!plan) return null;
  return {
...
function getNormalSummonFinisherPlan(context: LuminarchActionGenerationContext, card: SimulatedCardState) {
  if (!card) return null;
...
function getNormalSummonActions(context: LuminarchActionGenerationContext) {
  const {
    game,
    bot,
    opponent,
...
function getSanctumProtectorActions(context: LuminarchActionGenerationContext) {
  ...
  const isAscensionReady = (material: SimulatedCardState) => {
...
function getCelestialMarshalHandIgnitionActions(context: LuminarchActionGenerationContext) {
  ...
  const handActivationContext: LuminarchActivationContext = {
    ...(activationContext || {}),
    fromHand: true,
    activationZone: "hand",
    sourceZone: "hand",
...
function getFortressAegisReviveActions(context: LuminarchActionGenerationContext) {
  const { game, bot, opponent, activationContext, macroStrategy } = context;
  const actions: AIAction[] = [];
  if ((bot.field || []).length >= 5) return actions;
  ...
  (bot.field || []).forEach((card, fieldIndex) => {
    ...
    const reviveContext: LuminarchActivationContext = {
      ...(activationContext || {}),
      fromHand: false,
...
function getMagicSickleGraveyardActions(context: LuminarchActionGenerationContext) {
  const { game, bot, opponent, activationContext, macroStrategy } = context;
  const actions: AIAction[] = [];
...
export function getLuminarchMonsterIgnitionActions(context: LuminarchActionGenerationContext) {
  return [
...
export function getLuminarchSummonActions(context: LuminarchActionGenerationContext) {
  return [
