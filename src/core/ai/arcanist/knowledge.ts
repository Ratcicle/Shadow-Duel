...
export const ARCANIST_IDS = {
  GRIMOIRE: 301,
  APPRENTICE: 302,
  CRIMSON_EXPLOSION: 303,
  LIGHTNING_LANCE: 304,
  VIRIDIS: 305,
  TERA: 306,
  ALBUS: 307,
  MASTER_OF_MIRRORS: 308,
  MEETING: 309,
  ICE_BARRIER: 310,
  INK_RIVER: 311,
  GRAND_LIBRARY: 312,
...
export function getCardKnowledge(cardOrName: string | GameCard | SimulatedCardShape | null | undefined) {
  const name =
    typeof cardOrName === "string" ? cardOrName : cardOrName?.name || "";
  return CARD_KNOWLEDGE[name!] || null;
}

export function hasArchetype(card: GameCard | SimulatedCardShape | null | undefined, archetype: string) {
...
export function isArcanist(card: GameCard | SimulatedCardShape | null | undefined) {
  return hasArchetype(card, "Arcanist");
}

export function isArcanistMonster(card: GameCard | SimulatedCardShape | null | undefined) {
  return !!card && card.cardKind === "monster" && isArcanist(card);
}

export function isArcanistSpell(card: GameCard | SimulatedCardShape | null | undefined) {
  return !!card && card.cardKind === "spell" && isArcanist(card);
}

export function isArcanistEquip(card: GameCard | SimulatedCardShape | null | undefined) {
  return isArcanistSpell(card) && card!.subtype === "equip";
}

export function isFaceUp(card: GameCard | SimulatedCardShape | null | undefined) {
  return !!card && !card.isFacedown;
}

export function hasArcanistEquip(card: GameCard | SimulatedCardShape | null | undefined) {
  if (!card || !Array.isArray(card.equips)) return false;
  return card.equips.some((equip) => isFaceUp(equip) && isArcanistEquip(equip));
}

export function hasFaceUpArcanistMonster(player: Partial<import("../../contracts/ai.js").AIStrategyBotPort> | null | undefined) {
  return (player?.field || []).some(
    (card: GameCard | SimulatedCardShape | null | undefined) => isArcanistMonster(card) && isFaceUp(card),
  );
}

export function controlsArcanistEquip(player: Partial<import("../../contracts/ai.js").AIStrategyBotPort> | null | undefined) {
  return (player?.spellTrap || []).some(
    (card: GameCard | SimulatedCardShape | null | undefined) => isFaceUp(card) && isArcanistEquip(card),
  );
}

export function getInkCounters(card: { counters?: object } | null | undefined) {
  if (!card) return 0;
  if (card.counters instanceof Map) return card.counters.get("ink") || 0;
  if (card.counters && typeof card.counters === "object") {
    return (card.counters as { ink?: number; INK?: number }).ink || (card.counters as { ink?: number; INK?: number }).INK || 0;
  }
  return 0;
}

export function getStoredBlueprintCount(card: ((GameCard | SimulatedCardShape) & { state?: { blueprintStorage?: { storedBlueprints?: unknown[] } } }) | null | undefined) {
  const storage = card?.state?.blueprintStorage || card?.blueprintStorage;
  const blueprints = (storage as { storedBlueprints?: unknown[] } | undefined)?.storedBlueprints;
  return Array.isArray(blueprints) ? blueprints.length : 0;
}

export function getNameRank(name: string | undefined, order: readonly (string | undefined)[], fallback = 999) {
  const index = order.indexOf(name);
  return index >= 0 ? index : fallback;
}

export function sortByNameOrder<Card extends GameCard | SimulatedCardShape>(cards: readonly Card[] | null | undefined, order: readonly string[]) {
