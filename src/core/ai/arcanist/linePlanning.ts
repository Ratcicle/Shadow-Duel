import type { AIPlannedAction, AIPlanningContext, AIPlanningProfile, AIStrategyBotPort } from "../../contracts/ai.js";
...
type LineAction = AIPlannedAction & { cardName?: string; card?: StrategyCard | null; name?: string; destroyedCards?: Array<{ owner?: string }> };
interface Milestone { label: string; score: number; weight: number; }
type Context = Omit<AIPlanningContext, "sequence" | "planningContext"> & { game?: PlanningGame; sequence?: LineAction[]; planningContext?: { profile?: AIPlanningProfile } };
...
const DEFAULT_PROFILE: AIPlanningProfile & { reasons: string[] } = {
  enabled: false,
  mode: "off",
  turnMode: "mainOnly",
  beamWidth: 3,
...
function getPlayer(state: State | null | undefined): Player {
  return state?.bot || state?.player || {};
}

function getOpponent(state: State | null | undefined): Player {
  return state?.player || {};
}

function faceUpArcanists(player: Player | null | undefined) {
...
function countOpponentCards(player: Player) {
  return (
...
function opponentThreatensLethal(stateOrAnalysis: Analysis | State = {}) {
  const player: Player = stateOrAnalysis.bot || stateOrAnalysis.player || {};
  const lp = player.lp || (stateOrAnalysis as Analysis).lp || 0;
  if (lp <= 0) return true;
  const field =
    (stateOrAnalysis as Analysis).oppField ||
...
function hasVisiblePayoff(analysis: Analysis = {}) {
  const hand = analysis.hand || [];
...
function getOwnStrongestBattle(analysis: Analysis = {}) {
  return getStrongestBattleThreat(analysis.field || [], {
    includeBoosts: true,
  });
}

function hasGrandLibraryAccess(analysis: Analysis = {}) {
  return (
    analysis.fieldSpell?.name === ARCANIST_NAMES.GRAND_LIBRARY ||
...
function getMonsterBattleStatForAttackTarget(card: StrategyCard) {
...
function collectCurrentArcanistAttackers(analysis: Analysis = {}) {
...
function collectPotentialArcanistAttackers(analysis: Analysis = {}) {
...
function shouldUseArcanistBattleBridge(analysis: Analysis = {}, context: Context = {}) {
...
export function buildArcanistPlanningProfile(analysis: Analysis = {}, context: Context = {}): AIPlanningProfile & { reasons: string[] } {
  const game = context.game || analysis.game || null;
...
function getEquippedHosts(player: Player, name: string | null = null) {
...
function cardInstanceKey(card: StrategyCard, fallbackIndex: number | string = 0) {
  return `${card?.instanceId || card?.uuid || card?.id || card?.name || "card"}:${fallbackIndex}`;
}

function collectEquips(player: Player) {
  const equips: Array<{ card: StrategyCard; key: string }> = [];
...
function getArcanistEquipCount(player: Player) {
  return collectEquips(player).filter((entry) => !entry.card?.isFacedown).length;
}

function getInkRiverValue(player: Player) {
...
function getRecoverableSpellCount(player: Player) {
  return (player?.graveyard || []).filter(isArcanistSpell).length;
}

function getActionNames(sequence: LineAction[] = []) {
  return (sequence || [])
    .map((action) => action?.cardName || action?.card?.name || action?.name)
    .filter(Boolean);
}

function getActionTypes(sequence: LineAction[] = []) {
  return (sequence || [])
    .map((action) => action?.type)
    .filter(Boolean);
}

function getSimulatedBattleSteps(sequence: LineAction[] = []) {
  return (sequence || []).filter((action) => action?.type === "simulatedBattle");
}

function lineUsed(actionNames: Array<string | undefined>, name: string | null) {
  return actionNames.includes(name!);
}

function hasUsefulMeetingResult(finalPlayer: Player) {
  return [
...
function getActiveCardCount(player: Player) {
  return (
...
function getArcanistActiveCardCount(player: Player) {
  const cards = [
    ...(player?.hand || []),
    ...(player?.field || []),
    ...(player?.spellTrap || []),
...
function getEquippedGrimoireHosts(player: Player) {
  return faceUpArcanists(player).filter((card) =>
    (card?.equips || []).some((equip) => equip?.name === ARCANIST_NAMES.GRIMOIRE),
  );
}

function getGrimoireHostScore(host: StrategyCard) {
  switch (host?.name) {
...
function getProtectionSignals(player: Player) {
...
function opponentSummary(state: State | null | undefined) {
...
function getLineImpact(context: Context = {}) {
...
function addMilestone(entries: Milestone[], score: number, label: string) {
...
function hasElementalistImpact(elementalist: StrategyCard, impact: ReturnType<typeof getLineImpact>) {
...
function scoreElementalistMilestones(entries: Milestone[], impact: ReturnType<typeof getLineImpact>) {
...
function scoreGrimoireMilestones(entries: Milestone[], impact: ReturnType<typeof getLineImpact>) {
...
function scoreEngineMilestones(entries: Milestone[], impact: ReturnType<typeof getLineImpact>) {
...
function scoreTacticalMilestones(entries: Milestone[], impact: ReturnType<typeof getLineImpact>) {
...
function scoreBoardImpactMilestones(entries: Milestone[], impact: ReturnType<typeof getLineImpact>) {
...
function scoreSimulatedBattleMilestones(entries: Milestone[], impact: ReturnType<typeof getLineImpact>) {
...
function scoreRiskMilestones(entries: Milestone[], impact: ReturnType<typeof getLineImpact>) {
...
export function scoreArcanistLineMilestones(context: Context = {}) {
...
export function scoreArcanistLineTerminal(context: Context = {}) {
  const finalPlayer = getPlayer(context.finalState);
  ...
  const profile: Partial<AIPlanningProfile> = context.profile || context.planningContext?.profile || {};
  const critical =
...
export function describeArcanistPlannedLine(context: Context = {}) {
  const sequence = context.sequence || [];
