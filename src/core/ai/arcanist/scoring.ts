...
type Analysis = Partial<ReturnType<typeof buildStrategyAnalysis>> & { arcanistSpellsInGY?: number };
...
function getCardBaseValue(card: StrategyCard) {
  if (!card) return 0;
...
function getArcanistMonsterValue(card: StrategyCard, analysis: Analysis = {}) {
...
function getArcanistSpellTrapValue(card: StrategyCard, analysis: Analysis = {}) {
  if (!card) return 0;
  let value = getCardBaseValue(card);
...
export function evaluateArcanistCardValue(card: StrategyCard, analysis: Analysis = {}) {
  if (!card) return 0;
  if (card.cardKind === "monster") {
...
export function evaluateBoardArcanist(gameOrState: AIState, perspectivePlayer?: SimulatedPlayerState, getOpponent?: (state: AIState, player?: SimulatedPlayerState | null) => SimulatedPlayerState | null) {
  const perspective =
    (perspectivePlayer?.id ? perspectivePlayer : gameOrState?.bot || null) as AIStrategyBotPort | null;
  const opponent =
    typeof getOpponent === "function"
      ? getOpponent(gameOrState, perspective as SimulatedPlayerState | null)
      : (gameOrState?.player || null) as AIStrategyBotPort | null;
