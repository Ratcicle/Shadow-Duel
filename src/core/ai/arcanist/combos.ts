...
interface ArcanistCombo { id: string; name: string; priority: number; cards: string[]; description: string; available?: boolean; }
import {
  ARCANIST_NAMES,
...
function pushCombo(combos: ArcanistCombo[], combo: ArcanistCombo) {
  combos.push({
    available: true,
    ...combo,
  });
  ...
  if (inkRiver && recoverableSpells > 0) {
    const counters =
      inkRiver.counters instanceof Map
        ...
        : (inkRiver.counters as { ink?: number } | undefined)?.ink || 0;
    if (counters >= 2) {
      pushCombo(combos, {
        id: "ink_river_spell_loop",
        name: "Ink River spell loop",
