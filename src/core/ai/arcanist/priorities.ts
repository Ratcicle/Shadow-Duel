...
type StrategyCard = (GameCard | SimulatedCardState) & { usedEffectThisTurn?: boolean; state?: { blueprintStorage?: { storedBlueprints?: Array<{ name?: string }> } } };
type Analysis = Partial<ReturnType<typeof buildStrategyAnalysis>> & { oppStrongestBattle?: number };
type Context = { oppField?: StrategyCard[]; analysis?: Analysis; source?: StrategyCard; evaluationContext?: Analysis };
...
function hasFaceUpByName(cards: StrategyCard[] = [], name: string) {
  return (cards || []).some(
    (card) => card?.name === name && !card.isFacedown,
  );
}

function hasOpponentFaceUpSpell(analysis: Analysis = {}) {
  return (
...
function getOpponentCardCount(analysis: Analysis = {}) {
  return (
...
function getCurrentBattleStat(card: StrategyCard) {
...
function getSwitchedBattleStat(card: StrategyCard) {
  if (!card || card.cardKind !== "monster") return 0;
...
function evaluateTeraPositionPlan(tera: StrategyCard, analysis: Analysis = {}) {
  if (!tera || tera.name !== ARCANIST_NAMES.TERA || tera.isFacedown) {
    return { ok: false as const, reason: "not Tera" };
  }
  ...
  if (targets.length === 0) {
    return { ok: false as const, reason: "no opponent monsters" };
  }
  ...
  if (!best) return { ok: false as const, reason: "no useful Tera target" };
...
function isNormalArcanistSpellCounterSource(card: StrategyCard) {
  return (
...
function hasMeetingIgnitionCost(analysis: Analysis = {}) {
...
function canUseGrandLibraryForInkCounter(analysis: Analysis = {}) {
...
function canUseSpellTrapIgnitionForInkCounter(card: StrategyCard, analysis: Analysis = {}) {
  if (
    !card ||
    card.isFacedown ||
    !isArcanistSpell(card) ||
...
function getInkCounterFlowAfterInk(analysis: Analysis = {}) {
...
function describeInkCounterFlow(flow: Partial<ReturnType<typeof getInkCounterFlowAfterInk>> = {}) {
  const parts = [];
...
function getNonCounterSetupSpellsAfterInk(analysis: Analysis = {}) {
...
function getStoredBlueprints(card: StrategyCard) {
  const storage = (card?.state?.blueprintStorage || card?.blueprintStorage) as { storedBlueprints?: Array<{ sourceCardName?: string; displayName?: string; effectSnapshot?: { sourceCardName?: string } }> } | undefined;
  return Array.isArray(storage?.storedBlueprints) ? storage.storedBlueprints : [];
}

function getStoredBlueprintName(card: StrategyCard) {
...
function canUseCombatBuffThisTurn(monster: StrategyCard, analysis: Analysis = {}) {
  return (
    monster &&
...
function evaluateLightningLancePlan(analysis: Analysis = {}) {
  ...
  if (!lockPlan) {
    return { ok: false as const, reason: "no useful Lightning Lance target" };
  }
...
function isLowValueDiscard(card: StrategyCard, analysis: Analysis = {}) {
...
function hasLowValueDiscard(analysis: Analysis = {}) {
  return (analysis.hand || []).some((card) => isLowValueDiscard(card, analysis));
}

export function rankGrimoireHosts(hosts: StrategyCard[] = [], analysis: Analysis = {}) {
...
export function getBestGrimoireHostNames(analysis: Analysis = {}) {
  return rankGrimoireHosts(getOwnArcanistHosts(analysis), analysis).map(
    (card) => card.name,
  );
}

function getDuplicateNames(cards: StrategyCard[] = []) {
...
export function buildArcanistCostPreferences(analysis: Analysis = {}) {
  const hand = analysis.hand || [];
...
function getHighestValueOpponentNames(analysis: Analysis = {}) {
  const cards = [
    ...(analysis.oppField || []),
...
function getCardInstanceId(card: StrategyCard | null | undefined) {
  return card?.instanceId ?? card?.fieldPresenceId ?? null;
}

function isSameCardInstance(a: StrategyCard | null | undefined, b: StrategyCard | null | undefined) {
  if (!a || !b) return false;
...
function isEquipAttachedToMonster(equip: StrategyCard, monster: StrategyCard) {
...
function getFaceUpArcanistEquipCards(analysis: Analysis = {}) {
  return (analysis.spellTrap || []).filter(
    (card) => card && !card.isFacedown && isArcanistEquip(card),
  );
}

function hasSeismicImpactSetup(analysis: Analysis = {}) {
  return (
...
function hasPremiumSeismicTarget(analysis: Analysis = {}) {
...
function getBattleProtectionThreat(analysis: Analysis = {}) {
  return getStrongestBattleThreat(analysis.oppField || [], {
    includeBoosts: true,
  });
}

function hasPlausibleIceBarrierThreat(analysis: Analysis = {}, { amplified = false } = {}) {
...
function evaluateIceBarrierPlan(analysis: Analysis = {}) {
  const hosts = getOwnArcanistHosts(analysis);
  if (hosts.length === 0) {
  ...
  const battleThreat = getBattleProtectionThreat(analysis);
  const sortByValue = (a: StrategyCard, b: StrategyCard) =>
...
function getCrimsonDamage(card: StrategyCard) {
  return Math.max(0, Math.floor(((card?.atk || 0) * 0.5)));
}

function evaluateCrimsonExplosionPlan(analysis: Analysis = {}) {
  ...
  if (selfTargets.length === 0 || opponentTargets.length === 0) {
    return { ok: false as const, reason: "need face-up targets for Crimson" };
  }
  ...
  return best || { ok: false as const, reason: "no safe Crimson trade" };
}

function getCheapArcanistNamesForCrimson(analysis: Analysis = {}) {
...
function getLibraryChoiceNames(analysis: Analysis = {}) {
...
function getMeetingChoiceNames(analysis: Analysis = {}) {
...
export function buildArcanistTargetPreferences(card: StrategyCard, analysis: Analysis = {}) {
...
export function buildArcanistActivationContext(card: StrategyCard, analysis: Analysis = {}) {
  return {
...
export function shouldSummonMonster(card: StrategyCard, analysis: Analysis = {}, tributeInfo: { tributesNeeded?: number } = {}) {
...
export function shouldPlaySpell(card: StrategyCard, analysis: Analysis = {}) {
...
export function shouldActivateSpellTrapEffect(card: StrategyCard, analysis: Analysis = {}) {
...
export function shouldActivateMonsterEffect(card: StrategyCard, analysis: Analysis = {}) {
...
export function shouldActivateHandIgnition(card: StrategyCard, analysis: Analysis = {}) {
  if (card?.name !== ARCANIST_NAMES.ALBUS) {
...
export function getTributeRequirementFor(card: StrategyCard, playerState: AIStrategyBotPort) {
  return getGenericTributeRequirementFor(card, playerState);
}

function evaluateArcanistTributeValue(card: StrategyCard, analysis: Analysis = {}) {
  if (!card) return 0;
...
export function selectBestTributes(field: StrategyCard[], tributesNeeded: number, cardToSummon: StrategyCard, context: Context = {}) {
...
export function evaluateArcanistTributeTrade(cardToSummon: StrategyCard, tributes: StrategyCard[], analysis: Analysis = {}) {
  return evaluateTributeSummonCost(
    ...
    { evaluationContext: analysis },
    {
      ...
      evaluateSummonPayoff: (card) => {
        if (card?.name === ARCANIST_NAMES.ELEMENTALIST) {
          return { ok: true as const, reason: "Elementalist is a finisher payoff" };
        }
        if (
          card?.name === ARCANIST_NAMES.MASTER_OF_MIRRORS &&
          (analysis.graveyard || []).some(isArcanistSpell)
        ) {
          return { ok: true as const, reason: "Master recycles spells and draws" };
        }
        return { ok: false as const, reason: "no immediate Arcanist payoff" };
      },
    },
  );
}

export function rankSearchCandidates<Card extends StrategyCard>(cards: Card[] = [], action: { zone?: string } = {}, ctx: Context = {}) {
...
export function evaluateRecruitCandidate<Card extends StrategyCard>(cards: Card[] = [], ctx: Context = {}) {
  const analysis = ctx.analysis || {};
  const sourceName = ctx.source?.name || "";
