...
export function countControlledSpellTrapCards(player: Partial<import("../../contracts/ai.js").AIStrategyBotPort> | null = {}) {
...
function countOpponentSpellTrapCardsFromAnalysis(analysis: Analysis = {}) {
 ...
 */
export function detectAvailableCombos(analysis: Analysis) {
  ...
  const hasInHand = (id: number) => hasCardId(zoneIndex, "hand", id);
  const hasOnField = (id: number) => hasCardId(zoneIndex, "field", id);
  const hasInGY = (id: number) => hasCardId(zoneIndex, "graveyard", id);
  const hasInDeck = (id: number) => hasCardId(zoneIndex, "deck", id);
  const hasInExtra = (id: number) => hasCardId(zoneIndex, "extraDeck", id);
  const countInHand = (id: number) => countCardId(zoneIndex, "hand", id);
  const countOnField = (id: number) => countCardId(zoneIndex, "field", id);
  const countInGY = (id: number) => countCardId(zoneIndex, "graveyard", id);
  ...
  const comboByName = (name: string) => findComboByName(COMBO_DATABASE, name);
  const addCombo = (name: string, details: Partial<Omit<DetectedCombo, "combo">> & { projectedDraws?: number; projectedAtk?: number }) => {
    detected.push(
      createDetectedCombo({
 ...
 */
export function getComboSequence(combo: { combo?: { sequence?: Array<{ action: string; cardId?: number; note?: string }> } }, analysis: Analysis) {
 ...
 */
export function calculateFusionValue(fusionId: number, analysis: Analysis) {
