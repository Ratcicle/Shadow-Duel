...
interface Game extends Partial<Omit<AiStateShape, "effectEngine" | "_gameRef" | "bot" | "player" | "opponent" | "_isPerspectiveState">> {
  _isPerspectiveState?: boolean;
  bot?: Player; player?: Player; opponent?: Player;
  _gameRef?: Game;
  effectEngine?: { checkOncePerTurn?(card: StrategyCard, player: Player, effect: EffectDefinition): { ok?: boolean } | null } | null;
...
function isSafeSealingTarget(monster: StrategyCard, bot: Player, game: Game | null) {
...
function isProtectedVoidFusionMaterial(card: StrategyCard, bot: Player) {
...
export function shouldPlayVoidSpell(card: StrategyCard, game: Game | null, bot: Player, opponent: Player | null) {
...
export function shouldSummonVoidMonster(card: StrategyCard, game: Game | null, bot: Player, opponent: Player | null) {
...
export function assessVoidSummonEntry(card: StrategyCard, context: Context = {}) {
  ...
  const action: NonNullable<Context["action"]> = context.action || {};
  ...
  if (card.id === VOID_IDS.RAVEN) {
    const position = "defense" as const;
    if (!isEmergency) {
      return {
        shouldSummon: false,
        position,
...
export function assessVoidNormalSummonEntry(card: StrategyCard, context: Context = {}): ReturnType<typeof assessVoidSummonEntry> & { facedown?: boolean; projectedAtk?: number } {
  ...
  return {
    ...base,
    position: shouldSetDefensively ? "defense" as const : "attack" as const,
    facedown: shouldSetDefensively,
    projectedAtk: effectiveAtk,
  };
}

export function chooseVoidSummonPosition(card: StrategyCard, opponent: Player | null, analysis: Analysis | null = null) {
 ...
 */
function shouldReserveForHydra(bot: Player, voidsAccessible: number) {
...
function countCards(cards: StrategyCard[] = [], predicate: (card: StrategyCard) => boolean) {
  return (cards || []).filter((card) => card && predicate(card)).length;
}

function hasCardId(cards: StrategyCard[] = [], id: number) {
  return (cards || []).some((card) => card?.id === id);
}

function canUseAscensionFinisher(game: Game | null, bot: Player, materialId: number, ascensionId: number) {
...
function estimateVoidHydraDraws(_bot: Player, opponent: Player | null) {
  return countControlledSpellTrapCards(opponent);
}

function strongestBattleStat(monsters: StrategyCard[] = []) {
  return getStrongestOpponentBattleStat(monsters);
}

function countDestroyableByAtk(monsters: StrategyCard[] = [], atk = 0) {
...
export function evaluateVoidFinisherPlans(bot: Player, opponent: Player | null, game: Game | null = null, analysis: Analysis | null = null) {
  const field = bot?.field || [];
  const hand = bot?.hand || [];
  const resolvedOpponent: Player | null =
    opponent ||
...
export function evaluateVoidFusionPriority(bot: Player) {
...
function countOpponentFaceupCards(opponent: Player | null = {}) {
  return [
    ...(opponent?.field || []),
    ...(opponent?.spellTrap || []),
...
function hasAberrationGraveyardConversion(bot: Player = {}, game: Game | null = null) {
  ...
  const canUseIgnition = (card: StrategyCard | undefined, effectId: string) => {
...
function evaluateVoidPressure(bot: Player = {}, opponent: Player | null = {}, analysis: Analysis = {}) {
  const myLP = bot?.lp || analysis?.myLP || 8000;
...
function getStrongestOpponentBattleStat(field: StrategyCard[] = []) {
  return getStrongestBattleStat(field, { facedownValue: 1500 });
}

function hasVoidFaceupNormalSummonValue(card: StrategyCard) {
