 ...
 */
export function buildVoidCostPreferences(analysis: Analysis = {}) {
  ...
  const conjurerOnField = field.some(
    (c) => c?.id === VOID_IDS.CONJURER && !(c as (GameCard | SimulatedCardState) & { usedEffectThisTurn?: boolean }).usedEffectThisTurn,
  );
  ...
  if (field.some((c) => c?.id === VOID_IDS.CONJURER && (c as (GameCard | SimulatedCardState) & { usedEffectThisTurn?: boolean }).usedEffectThisTurn)) {
...
function buildVoidTargetPreferences(costPreferences: Partial<ReturnType<typeof buildVoidCostPreferences>> = {}) {
  ...
  const genericVoidCostProfile = <Extra extends object = object>(extra: Extra = {} as Extra) => ({
    role: "cost",
    intent: "cost",
 ...
 */
export function buildVoidActivationContext(analysis: Analysis = {}) {
 ...
 */
export function buildVoidTributePolicy(analysis: Analysis = {}) {
  ...
  return {
    evaluateCardValue: (monster: (GameCard | SimulatedCardState) & { usedEffectThisTurn?: boolean }) => {
      if (!monster) return 0;
