 ...
 */
export function analyzeHollowEconomy(analysis: Analysis) {
...
export function assessVoidHollowResourcePolicy(analysis: Analysis & { hollowEconomy?: ReturnType<typeof analyzeHollowEconomy>; bestFinisherPlan?: FinisherPlan | null } = {}) {
 ...
 */
export function evaluateVoidMonster(monster: GameCard | SimulatedCardState | null | undefined, context: MonsterContext = {}) {
 ...
 * @returns {number}
 */
export function evaluateBoardVoid(gameOrState: AIState, perspectivePlayer?: SimulatedPlayerState) {
  const perspective = (perspectivePlayer?.id
    ? perspectivePlayer
    : gameOrState.bot) as AIStrategyBotPort | undefined;
  const opponent = (
