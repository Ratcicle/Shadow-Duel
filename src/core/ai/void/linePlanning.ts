import type { AIState, AIPlannedAction, AIPlanningContext, AIPlanningProfile, AIStrategyBotPort } from "../../contracts/ai.js";
...
type Player = Partial<AIStrategyBotPort>;
...
function getBotState(state: State): Player {
  return (state?.bot || state?.player || {}) as Player;
}

function getOpponentState(state: State): Player {
  return (state?.player || state?.opponent || {}) as Player;
}
...
function addMilestone(list: Milestone[], label: string, score: number, detail: string | null = null) {
  if (!Number.isFinite(score) || score === 0) return;
  list.push({ label, score, detail });
}

function lineHasRealPayoff({ finalBot, finalOpponent, initialOpponent, sequence }: { finalBot: Player; finalOpponent: Player; initialOpponent: Player; sequence: LineAction[] }) {
...
export function buildVoidPlanningProfile(analysis: Analysis = {}, context: Context = {}): AIPlanningProfile & { reasons: string[]; critical: boolean; bestFinisherPlan: FinisherPlan | null } {
  const game: PlanningGame = context.game || analysis.game || {};
  const bot = (analysis.bot || analysis.player || game.bot || {}) as Player;
  const opponent = (analysis.opponent || game.player || {}) as Player;
...
export function scoreVoidLineMilestones(context: Context = {}) {
  const initialState = context.initialState || {};
  const finalState = context.finalState || {};
  const sequence = context.sequence || [];
  const profile: NonNullable<Context["profile"]> = context.profile || {};
...
export function scoreVoidLineTerminal(context: Context = {}) {
...
export function describeVoidPlannedLine(context: Context = {}) {
  const sequence = context.sequence || [];
  const milestones = (context.milestones || []).slice(0, 4) as Milestone[];
...
export function evaluateVoidLineBaseScore(state: AIState) {
  return evaluateBoardVoid(state, state?.bot as SimulatedPlayerState);
}
