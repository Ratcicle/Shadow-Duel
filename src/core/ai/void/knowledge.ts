...
export const VOID_CARD_KNOWLEDGE: Record<number, { role: string; tags: string[] }> = {
  ...
  208: { role: "control", tags: ["lock", "hollow_cost", "hollow_revive"] },
  ...
  212: { role: "boss", tags: ["conditional_banish", "berserker_material"] },
  214: { role: "boss", tags: ["hollow_cost", "banish_protection", "survival"] },
  221: {
  ...
  225: { role: "fusion_boss", tags: ["revenge", "gy_revival", "recovery"] },
...
export function isVoid(card: GameCard | SimulatedCardShape | null | undefined) {
  if (!card) return false;
  if (Array.isArray(card.archetypes)) return card.archetypes.includes("Void");
  return card.archetype === "Void";
}

export function getVoidCardKnowledge(cardOrId: number | GameCard | SimulatedCardShape | null | undefined) {
  const id = typeof cardOrId === "number" ? cardOrId : cardOrId?.id;
  if (!id) return null;
  return VOID_CARD_KNOWLEDGE[id] || null;
}

export function getVoidCardsByRole(role: string) {
