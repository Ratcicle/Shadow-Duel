import type { AIState, AIAction, AIPlanningContext, AIPlannedAction, AIStrategyBotPort } from "../contracts/ai.js";
import type { SimulatedCardState, SimulatedPlayerState, SimulationGameState, PerspectiveGameState, GameTreeSimulationGameState } from "../contracts/aiState.js";
...
function findBestOffensiveTemporaryBuffTarget(monsters: SimulatedCardState[], opponent: SimulatedPlayerState | null | undefined, preference: { atkBoost?: number }) {
  return (monsters || []).reduce<{ monster: SimulatedCardState | null; score: number }>(
    (best, monster) => {
...
function evaluateBarbariasStanceDance(card: import("../contracts/aiState.js").SimulatedCardShape, opponent: SimulatedPlayerState | null | undefined, options: { afterManualDefense?: boolean } = {}) {
...
export default class LuminarchStrategy extends BaseStrategy {
  buildPlanningAnalysis(game: LuminarchGame, context: LuminarchContext = {}) {
    const bot = (context.bot || this.bot || game?.bot || null) as LuminarchPlayer;
    const opponent = (bot ? this.getOpponent(game, bot) : game?.player || null) as SimulatedPlayerState | null;
    const analysis: LuminarchAnalysis =
      context.analysis ||
  ...
  getPlanningProfile(game: LuminarchGame, context: AIPlanningContext = {}) {
    ...
    return buildLuminarchPlanningProfile(analysis, {
      ...(context as LuminarchContext),
      game,
      bot: ((context as LuminarchContext).bot || this.bot || game.bot) as SimulatedPlayerState,
    });
  }
  ...
  isPostBattlePayoffAction(action: AIAction, _context = {}) {
    return isLuminarchMain2BattlePayoffAction(action);
  }

  prepareSimulatedBattle(context: Parameters<typeof prepareLuminarchSimulatedBattle>[0] = {} as Parameters<typeof prepareLuminarchSimulatedBattle>[0]) {
    return prepareLuminarchSimulatedBattle(context);
  }

  applySimulatedBattleRewards(context: Parameters<typeof applyLuminarchSimulatedBattleRewards>[0] = {} as Parameters<typeof applyLuminarchSimulatedBattleRewards>[0]) {
    return applyLuminarchSimulatedBattleRewards(context);
  }

  scoreBattleAttackCandidate(context: Parameters<typeof scoreLuminarchBattleAttackCandidate>[0] = {}) {
  ...
  generateMainPhaseActions(game: LuminarchGame) {
    ...
    // === COMBO DETECTION ===
    try {
      const analysis: LuminarchAnalysis = buildStrategyAnalysis({ bot, opponent, game });
    ...
    const luminarchActionContext: LuminarchActionGenerationContext = {
      game,
      bot,
      opponent: opponent!,
      activationContext: activationContext as import("./luminarch/contracts.js").LuminarchActivationContext,
      macroStrategy,
      gameStance,
    ...
    if (actions.length === 0) {
      ...
      const emergencySpellNames: string[] = [];
  ...
  chooseSummonPosition(card: SimulatedCardState, game: LuminarchGame) {
  ...
  chooseSpecialSummonPosition(card: Parameters<typeof chooseLuminarchSpecialSummonPosition>[0], context: {game?:AIState;state?:AIState;player?:AIStrategyBotPort | null;action?:AIAction;sourceAction?:AIAction | null;activationContext?:import("../contracts/ai.js").AIActivationContext;actionPosition?:BattlePositionInput | null;position?:BattlePositionInput} = {}) {
    ...
    return chooseLuminarchSpecialSummonPosition(card, {
      ...
      action: {
        ...(context.action || {}),
        position:
          (context.action as {position?: BattlePositionInput} | undefined)?.position ??
    ...
    } as Parameters<typeof chooseLuminarchSpecialSummonPosition>[1]);
  }

  shouldSetFacedown(card: SimulatedCardState, position: BattlePositionInput) {
  ...
  getBarbariasMonsterEffectActions(game: LuminarchGame, bot: SimulatedPlayerState, opponent: SimulatedPlayerState | null | undefined) {
    const actions: AIAction[] = [];
    (bot?.field || []).forEach((card, fieldIndex) => {
      if (!card || card.name !== "Luminarch Megashield Barbarias") return;
  ...
  resolveSimulatedHandIndex(player: SimulatedPlayerState, action: AIAction, expectedKind: Parameters<typeof resolveGenericSimulatedHandIndex>[2] = null) {
    return resolveGenericSimulatedHandIndex(player, action, expectedKind);
  }

  resolveSimulatedFieldIndex(player: SimulatedPlayerState, action: AIAction, predicate: Parameters<typeof resolveGenericSimulatedFieldIndex>[2] = null) {
    return resolveGenericSimulatedFieldIndex(player, action, predicate);
  }

  rankSearchCandidates(cards: SimulatedCardState[], action: Parameters<typeof rankLuminarchSearchCandidates>[1] = {}, ctx: Parameters<typeof rankLuminarchSearchCandidates>[2] = {}) {
  ...
  simulateLuminarchSearch(player: SimulatedPlayerState, sourceCard: SimulatedCardState, action: Parameters<typeof simulateLuminarchSearchAction>[2], state: SimulationGameState | PerspectiveGameState | GameTreeSimulationGameState) {
    return simulateLuminarchSearchAction(player, sourceCard, action, state as Parameters<typeof simulateLuminarchSearchAction>[3], {
      strategy: this,
      getOpponent: this.getOpponent.bind(this),
    });
  }
    ...
    return simulateLuminarchMainPhaseAction(state as Parameters<typeof simulateLuminarchMainPhaseAction>[0], action, {
      ...
      selectBestTributes: (field, tributesNeeded, cardToSummon, context) =>
        this.selectBestTributes(field, tributesNeeded, cardToSummon, {
          ...(context || {}),
          botState: state.bot as SimulatedPlayerState,
          oppField: (state.player?.field || []) as SimulatedCardState[],
          game: state,
        }),
   ...
   */
  evaluateMacroStrategy(game: LuminarchGame): import("./luminarch/contracts.js").LuminarchMacroDecision {
    try {
      const isSimulatedState = game?._isPerspectiveState === true;
      const bot = (isSimulatedState ? game.bot : this.bot || game.bot) as LuminarchPlayer;
      const opponent = this.getOpponent(game, bot);
   ...
   */
  detectFusionOpportunities(game: LuminarchGame, bot: SimulatedPlayerState) {
   ...
   */
  evaluateFusionPriority(fusionName: string, bot: SimulatedPlayerState, opponent: SimulatedPlayerState | null | undefined, game: LuminarchGame) {
  ...
  chooseAscensionPosition(ascensionCard: SimulatedCardState, bot: SimulatedPlayerState, opponent: SimulatedPlayerState | null | undefined) {
   ...
   */
  detectAscensionOpportunities(game: LuminarchGame, bot: SimulatedPlayerState) {
   ...
   */
  evaluateAscensionPriority(material: SimulatedCardState, ascensionCard: SimulatedCardState, bot: SimulatedPlayerState, opponent: SimulatedPlayerState | null | undefined, game: LuminarchGame) {
   ...
   */
  shouldSummonMonsterSafely(card: SimulatedCardState, game: LuminarchGame, opponent: SimulatedPlayerState | null | undefined): ReturnType<Required<import("./luminarch/contracts.js").LuminarchHooks>["shouldSummonMonsterSafely"]> {
    try {
      // === USO DO MÓDULO DE PRIORIDADES ===
      const bot = (game?._isPerspectiveState ? game.bot : this.bot || game.bot) as LuminarchPlayer;
      const analysis: LuminarchAnalysis = buildStrategyAnalysis({ bot, opponent, game });
      ...
      return {
        yes: true,
        position: (decision.position || "defense") as "attack" | "defense",
