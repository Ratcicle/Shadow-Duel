  ...
  generateMainPhaseActions(game: StrategyGame): AIAction[] {
    ...
    (bot.hand || []).forEach((card, index) => {
      ...
      if (!isSimulatedState) {
        ...
        if (!(check as PreviewGuardResult | null | undefined)?.ok) return;
      ...
      if (card.name === COURT_OF_THE_DEAD) {
        ...
        if (card.isFacedown) {
          if (!isSimulatedState) {
            ...
            if ((check as PreviewGuardResult | null | undefined)?.ok === false) return;
          }
          actions.push({
    ...
    if (bot.fieldSpell && !isSimulatedState) {
      ...
      if (effect) {
        ...
        if ((check as PreviewGuardResult | null | undefined)?.ok) {
