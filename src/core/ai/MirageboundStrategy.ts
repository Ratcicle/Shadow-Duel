import type { MirageboundCard, MirageboundPlayer, MirageboundAnalysis, MirageboundGame, MirageboundContext, MirageboundLineAction, MirageboundPreference, MirageboundActivationContext, MirageboundOwner, MirageboundChainOption, MirageboundChainContext, MirageboundBattleContext, MirageboundStatCard } from "./miragebound/contracts.js";
...
const DEFENSIVE_CHAIN_RESPONSE_NAMES = new Set<string | null | undefined>([
  MB.FALSE_HORIZON,
  MB.VANISHING_STEP,
]);

function isSimulatedState(game: MirageboundGame) {
  return game?._isPerspectiveState === true;
}

function isMiragebound(card: MirageboundCard | null | undefined): card is MirageboundCard {
  return (
...
function isFaceUpMonster(card: MirageboundCard | null | undefined): card is MirageboundCard {
  return card?.cardKind === "monster" && !card.isFacedown;
}

function isFaceUpMirageboundMonster(card: MirageboundCard | null | undefined): card is MirageboundCard {
  return isFaceUpMonster(card) && isMiragebound(card);
}

function hasMirageboundDefenseResponseInChain(chainSystem: MirageboundChainContext["chainSystem"], player: MirageboundPlayer) {
...
function hasName(cards: MirageboundCard[] = [], name: string) {
  return (cards || []).some((card) => card?.name === name);
}

function hasPositionChangeEffectAccess(base: Partial<import("./common/analysis.js").StrategyAnalysis<MirageboundPlayer>> = {}, faceUpMiragebounds: MirageboundCard[] = []) {
...
function getRebelPositionEffectBonus(analysis: MirageboundAnalysis = {} as MirageboundAnalysis) {
...
function getBattleStat(card: MirageboundStatCard | null | undefined) {
...
function getMaterialEffectActivations(game: MirageboundGame | null | undefined, player: MirageboundPlayer, materialId: number) {
...
function getFieldCapacity(player: MirageboundPlayer) {
  return Math.max(0, 5 - ((player?.field || []).length || 0));
}

function getOpponentCards(analysis: Pick<MirageboundAnalysis,"oppField"|"oppSpellTrap"|"oppFieldSpell">): MirageboundCard[] {
  return [
    ...(analysis.oppField || []),
...
function getCardsByNames(cards: MirageboundCard[] = [], names: string[] = []) {
  const wanted = new Set<string | undefined>(names);
  return (cards || []).filter((card) => wanted.has(card?.name));
}

function getInstanceIds(cards: Array<MirageboundCard | null | undefined> = []) {
...
function buildMaterialHint(card: MirageboundCard, index: number) {
  return {
    index,
    id: card?.id,
    name: card?.name,
...
function hasCardStatus(card: MirageboundCard, status: Extract<keyof MirageboundCard,string>) {
...
function getOwnerId(ownerLike: MirageboundOwner | MirageboundCard | undefined) {
...
function mergePreference(base: MirageboundPreference = {}, patch: MirageboundPreference = {}) {
  return {
    ...(base || {}),
    ...(patch || {}),
...
function mergeTargetPreference(activationContext: MirageboundActivationContext, targetId: string, patch: MirageboundPreference) {
...
function getIncomingBattleThreat(context: MirageboundContext = {}, player: MirageboundPlayer | null, analysis: MirageboundAnalysis = {} as MirageboundAnalysis) {
...
function isMeaningfulChainBattleThreat(threat: Partial<ReturnType<typeof getIncomingBattleThreat>> = {}, analysis: MirageboundAnalysis = {} as MirageboundAnalysis) {
...
function isOpponentEffectTargetingSelf(context: MirageboundContext = {}, player: MirageboundPlayer | null) {
...
function hasUsefulChainBounceTarget(analysis: MirageboundAnalysis = {} as MirageboundAnalysis) {
...
function getBestOwnBattleStat(analysis: MirageboundAnalysis = {} as MirageboundAnalysis) {
...
function isAttackPositionThreat(card: MirageboundCard) {
  return card?.cardKind === "monster" && !card.isFacedown && card.position === "attack";
}

function canReUseViperAfterBounce(analysis: MirageboundAnalysis = {} as MirageboundAnalysis) {
  return analysis.availableMonsterZonesAfterBounce > 0;
}

function buildBounceTargetBuckets(analysis: MirageboundAnalysis = {} as MirageboundAnalysis) {
...
function buildBounceNameProfile(analysis: MirageboundAnalysis = {} as MirageboundAnalysis) {
...
function scorePositionTarget(card: MirageboundCard, analysis: MirageboundAnalysis = {} as MirageboundAnalysis, { preferDefenseOutcome = true } = {}) {
...
function rankOpponentMonstersForPosition(analysis: MirageboundAnalysis = {} as MirageboundAnalysis, options: MirageboundContext = {}) {
...
function rankOpponentCardsForRemoval(analysis: MirageboundAnalysis = {} as MirageboundAnalysis) {
...
function buildOpponentPositionPreference(analysis: MirageboundAnalysis = {} as MirageboundAnalysis, sourceCard: MirageboundCard | null = null) {
...
function buildOpponentDebuffPreference(analysis: MirageboundAnalysis = {} as MirageboundAnalysis, sourceCard: MirageboundCard | null = null) {
  const reduction =
...
function buildOpponentRemovalPreference(analysis: MirageboundAnalysis = {} as MirageboundAnalysis) {
...
function buildMirageboundTargetPreferences(sourceCard: MirageboundCard, analysis: MirageboundAnalysis = {} as MirageboundAnalysis) {
...
function buildMirageboundCostPreferences(analysis: MirageboundAnalysis = {} as MirageboundAnalysis) {
  const bounceProfile = buildBounceNameProfile(analysis);
  return {
...
function buildSpecialSummonPositions(analysis: MirageboundAnalysis = {} as MirageboundAnalysis) {
  const viperPosition =
...
function shouldPlayMirageboundSpell(card: MirageboundCard, analysis: MirageboundAnalysis = {} as MirageboundAnalysis) {
...
function shouldSetMirageboundBackrow(card: MirageboundCard, analysis: MirageboundAnalysis = {} as MirageboundAnalysis) {
...
function shouldSummonMirageboundMonster(card: MirageboundCard, analysis: MirageboundAnalysis = {} as MirageboundAnalysis, tributeInfo: {tributesNeeded?:number} = {}): {yes:boolean;priority?:number;position?:"attack"|"defense";reason?:string} {
...
function shouldActivateHandIgnition(card: MirageboundCard, analysis: MirageboundAnalysis = {} as MirageboundAnalysis) {
  if (card?.name === MB.DANCER) {
...
function shouldActivateMonsterEffect(card: MirageboundCard, analysis: MirageboundAnalysis = {} as MirageboundAnalysis, context: {effect?: import("../contracts/effects.js").EffectDefinition} = {}) {
...
function shouldActivateFieldSpell(card: MirageboundCard, analysis: MirageboundAnalysis = {} as MirageboundAnalysis) {
...
function shouldActivateSpellTrapEffect(card: MirageboundCard, analysis: MirageboundAnalysis = {} as MirageboundAnalysis) {
...
function getMirageboundCardValue(card: MirageboundCard) {
  if (!card) return 0;
...
function getDesertLeviathanCard(bot: MirageboundPlayer) {
  return (bot?.extraDeck || []).find((card) => card?.name === MB.DESERT_LEVIATHAN);
}

function getGlassSovereignCard(bot: MirageboundPlayer) {
  return (bot?.extraDeck || []).find((card) => card?.name === MB.GLASS_SOVEREIGN);
}

function getLocalLeviathanMaterialCombos(bot: MirageboundPlayer) {
...
function getLeviathanMaterialCombos(game: MirageboundGame, bot: MirageboundPlayer, leviathan: MirageboundCard | undefined) {
...
function getLeviathanSecondMaterialScore(card: MirageboundCard, analysis: MirageboundAnalysis = {} as MirageboundAnalysis) {
  if (!card) return -100;
...
function evaluateLeviathanCombo(combo: MirageboundCard[] = [], analysis: MirageboundAnalysis = {} as MirageboundAnalysis) {
...
function chooseBestLeviathanCombo(combos: MirageboundCard[][] = [], analysis: MirageboundAnalysis = {} as MirageboundAnalysis) {
  return combos
    .map((combo) => ({
      combo,
...
function buildLeviathanProcedureAction({
  ...
  return {
    type: "extraDeckProcedure" as const,
    cardId: leviathan.id,
...
export default class MirageboundStrategy extends BaseStrategy {
  ...
  analyzeGameState(game: MirageboundGame): MirageboundAnalysis {
    ...
    const actor = (simulated ? game.bot : this.bot || game?.bot) as MirageboundPlayer;
    const opponent = (actor ? this.getOpponent(game, actor) : null) as MirageboundPlayer | null;
  ...
  async chooseChainResponse({
    ...
    const resolvedGame = (game || (context as MirageboundContext)?.game || this.currentAnalysis?.game || null) as MirageboundGame | null;
    const analysis: MirageboundAnalysis = resolvedGame
      ? this.analyzeGameState(resolvedGame)
      : this.currentAnalysis || {} as MirageboundAnalysis;
    ...
    const evaluated = relevant
      .map(
        (option) =>
          this.evaluateFalseHorizonChainResponse(option, analysis, context as MirageboundContext) ||
          this.evaluateVanishingStepChainResponse(option, analysis, context as MirageboundContext),
      )
      .filter(Boolean)
      .sort((a, b) => {
  ...
  rankSearchCandidates(cards: MirageboundCard[] = [], action: MirageboundLineAction = {}, ctx: MirageboundContext = {}) {
    ...
    const analysis = ctx?.game ? this.analyzeGameState(ctx.game) : this.currentAnalysis || {} as MirageboundAnalysis;
