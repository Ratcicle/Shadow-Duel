...
type TargetPreference = { intent: string; role: string; purpose?: string; attackers?: BloomrotCard[]; opponentLp?: number; atkReduction?: number; defReduction?: number; preferredInstanceIds?: (string | number)[]; preferredNames?: string[]; preferNames?: string[]; avoidNames?: string[]; preserveNames?: string[] };
import {
  BLOOMROT_NAMES,
  getSporeCount,
  isBloomrot,
...
function getInstanceId(card: BloomrotCard | null | undefined) {
...
function isFaceup(card: BloomrotCard | null | undefined): card is BloomrotCard {
  return (card && card.isFacedown !== true) as boolean;
}
...
function cardValue(card: BloomrotCard | null | undefined) {
  if (!card) return 0;
...
function faceupOpponentCards(analysis: BloomrotAnalysis = {}) {
  return [
    ...(analysis.oppField || []),
...
function faceupOpponentMonsters(analysis: BloomrotAnalysis = {}) {
  return (analysis.opponentMonsters || []).filter(isFaceup);
}

function readyBloomrotAttackers(analysis: BloomrotAnalysis = {}) {
...
function scoreSporeTarget(card: BloomrotCard, analysis: BloomrotAnalysis = {}) {
...
function scoreRemovalTarget(card: BloomrotCard | null | undefined, analysis: BloomrotAnalysis = {}) {
  let score = cardValue(card);
  if (card?.cardKind === "monster") {
...
function scoreOwnEquipTarget(card: BloomrotCard) {
...
function sortedByScore(cards: BloomrotCard[], scoreFn: (card: BloomrotCard) => number) {
  return (cards || [])
    .filter(Boolean)
...
function topIds(cards: BloomrotCard[], scoreFn: (card: BloomrotCard) => number, count = 1) {
  return instanceIds(sortedByScore(cards, scoreFn).slice(0, count));
}

function sporePreference(analysis: BloomrotAnalysis = {}, count = 1) {
  return {
    intent: "harm",
...
function debuffPreference(analysis: BloomrotAnalysis = {}) {
  return {
    intent: "harm",
...
function removalPreference(analysis: BloomrotAnalysis = {}, options: { monstersOnly?: boolean } = {}) {
...
function ownEquipPreference(analysis: BloomrotAnalysis = {}) {
  return {
    intent: "benefit",
    role: "named_preference",
    preferredNames: [
...
function opponentEquipPreference(analysis: BloomrotAnalysis = {}) {
  return {
    intent: "harm",
    role: "removal",
    preferredInstanceIds: topIds(
...
function mycoWeaverCostPreference(analysis: BloomrotAnalysis = {}) {
...
export function buildBloomrotTargetPreferences(sourceCard: BloomrotCard | null | undefined, analysis: BloomrotAnalysis = {}) {
  const preferences: Record<string, TargetPreference> = {};
...
function searchScore(card: BloomrotCard | null | undefined, action: { zone?: string } = {}, _ctx: object = {}) {
  if (!card) return -1000;
...
export function rankBloomrotSearchCandidates<Card extends BloomrotCard>(cards: Card[] = [], action: { zone?: string } = {}, ctx: object = {}) {
  return (cards || [])
    .slice()
