...
function hasOpponentFaceupTarget(analysis: BloomrotAnalysis = {}) {
  return [
    ...(analysis.oppField || []),
...
function hasOpponentFaceupMonster(analysis: BloomrotAnalysis = {}) {
...
function hasAnyNonTokenBloomrotMonster(analysis: BloomrotAnalysis = {}) {
  return (analysis.faceUpBloomrotField || []).some(
    (card) => isBloomrotMonster(card) && !isBloomrotToken(card),
  );
}

function hasAnyBloomrotMonster(analysis: BloomrotAnalysis = {}) {
  return [
...
function isMainPhase1(analysis: BloomrotAnalysis = {}) {
  return String(analysis.phase || "main1").toLowerCase().includes("main1");
}

function hasRecoverableBloomrot(analysis: BloomrotAnalysis = {}) {
  return (analysis.bloomrotGraveyard || []).length > 0;
}

function withSpendPolicy(card: BloomrotCard | null | undefined, analysis: BloomrotAnalysis, options: AIActivationContext & { purpose?: string; amount?: number } = {}) {
...
function alreadyHasRottingGround(analysis: BloomrotAnalysis = {}) {
  return (analysis.spellTrap || []).some(
    (card) => card?.name === BLOOMROT_NAMES.ROTTING_GROUND,
  );
}

export function buildBloomrotActivationContext(card: BloomrotCard | null | undefined, analysis: BloomrotAnalysis = {}, options: AIActivationContext & { purpose?: string; amount?: number } = {}) {
...
export function shouldSummonBloomrotMonster(card: BloomrotCard | null | undefined, analysis: BloomrotAnalysis = {}, tributeInfo: { tributesNeeded?: number } = {}) {
...
export function shouldPlayBloomrotSpell(card: BloomrotCard | null | undefined, analysis: BloomrotAnalysis = {}) {
...
export function shouldActivateBloomrotHandIgnition(card: BloomrotCard | null | undefined, analysis: BloomrotAnalysis = {}) {
...
export function shouldActivateBloomrotMonsterEffect(card: BloomrotCard | null | undefined, analysis: BloomrotAnalysis = {}) {
  if (!card || !isBloomrot(card)) return { yes: false };
...
export function shouldActivateBloomrotSpellTrapEffect(card: BloomrotCard | null | undefined, analysis: BloomrotAnalysis = {}) {
  if (!card || !isBloomrot(card)) return { yes: false };
...
export function shouldActivateBloomrotFieldEffect(card: BloomrotCard | null | undefined, analysis: BloomrotAnalysis = {}) {
...
export function shouldSetBloomrotBackrow(card: BloomrotCard | null | undefined, analysis: BloomrotAnalysis = {}) {
...
export function shouldSkipDuplicateBloomrotBackrow(card: BloomrotCard | null | undefined, analysis: BloomrotAnalysis = {}) {
  return card?.name === BLOOMROT_NAMES.ROTTING_GROUND && alreadyHasRottingGround(analysis);
}

export function getSporeInvestmentScore(analysis: BloomrotAnalysis = {}) {
