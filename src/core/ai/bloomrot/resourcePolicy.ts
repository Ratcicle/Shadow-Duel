...
function fieldCounterCards(analysis: BloomrotAnalysis = {}) {
  return [
...
function opponentCards(analysis: BloomrotAnalysis = {}) {
  return [
    ...asArray(analysis.oppField),
    ...asArray(analysis.oppSpellTrap),
    analysis.oppFieldSpell,
...
function isRelevantHarvestTarget(card: BloomrotCard | null | undefined) {
...
function hasQueenMaterial(analysis: BloomrotAnalysis = {}) {
  return asArray(analysis.faceUpBloomrotField).some(
    (card) => card?.cardKind === "monster" && Number(card.level || 0) >= 5,
  );
}

function protectedCounterCards(analysis: BloomrotAnalysis = {}) {
  const protectedCards = new Set<BloomrotCard>();
...
export function getBloomrotCounterSpendSummary(analysis: BloomrotAnalysis = {}) {
...
function canPayWithoutProtectedCounters(analysis: BloomrotAnalysis, amount: number) {
...
function queenReserveAllows(summary: ReturnType<typeof getBloomrotCounterSpendSummary>, amount: number, purpose: string) {
...
function opponentFaceUpTargets(analysis: BloomrotAnalysis = {}) {
  return opponentCards(analysis).filter(faceUp);
}

function opponentFaceUpMonsters(analysis: BloomrotAnalysis = {}) {
  return asArray(analysis.opponentMonsters).filter(faceUp);
}

function harvestAttackers(analysis: BloomrotAnalysis = {}) {
...
function bestHarvestTarget(analysis: BloomrotAnalysis = {}) {
  return opponentCards(analysis)
    .map((card) => ({ card, score: battleValue(card) }))
    .sort((a, b) => b.score - a.score)[0]?.card || null;
}

function bestInfectedMonster(analysis: BloomrotAnalysis = {}) {
...
function canWidowLeaveValidTarget(analysis: BloomrotAnalysis = {}, amount: number) {
...
function bestRecoveryValue(analysis: BloomrotAnalysis = {}) {
  return asArray(analysis.bloomrotGraveyard).reduce<{ card: BloomrotCard | null; value: number }>((best, card) => {
    const value = RECOVERY_VALUES.get(card?.name!) || 0;
    return value > best.value ? { card, value } : best;
  }, { card: null, value: 0 });
}

function allow(reason: string, priorityBonus = 0) {
  return { allow: true, priorityBonus, reason };
}

function block(reason: string, priorityBonus = 0) {
  return { allow: false, priorityBonus, reason };
}

function evaluateFixedSpend({ purpose, amount, analysis }: { purpose: string; amount: number; analysis: BloomrotAnalysis }) {
...
function harvestBuffCreatesBattleBreakthrough(analysis: BloomrotAnalysis = {}, buffAmount = 0) {
...
function blocksCounterSpendHandSummonZone(analysis: BloomrotAnalysis = {}) {
  return analysis.hasLivingColonyActive === true && (analysis.freeMonsterZones || 0) <= 1;
}

function evaluateRotStag(amount: number, analysis: BloomrotAnalysis = {}) {
...
function evaluateAncientHusk(amount: number, analysis: BloomrotAnalysis = {}) {
  if (blocksCounterSpendHandSummonZone(analysis)) {
    return block("preserve a Monster Zone for Living Colony token before hand summon");
  }
  const base = evaluateFixedSpend({
...
function evaluateWidow(amount: number, analysis: BloomrotAnalysis = {}) {
  if (blocksCounterSpendHandSummonZone(analysis)) {
    return block("preserve a Monster Zone for Living Colony token before hand summon");
  }
  const base = evaluateFixedSpend({
...
function evaluateRootNetwork(amount: number, analysis: BloomrotAnalysis = {}) {
  const base = evaluateFixedSpend({
...
function evaluateAncientMycelium(amount: number, analysis: BloomrotAnalysis = {}) {
  const base = evaluateFixedSpend({
...
function evaluateHarvest(analysis: BloomrotAnalysis = {}) {
  const summary = getBloomrotCounterSpendSummary(analysis);
...
function evaluateQueenHeal(amount: number, analysis: BloomrotAnalysis = {}) {
  const base = evaluateFixedSpend({
    purpose: "queen_heal",
    amount,
    analysis,
