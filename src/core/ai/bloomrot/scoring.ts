...
function getCards(player: BloomrotPlayer | null | undefined, zone: BloomrotZone) {
  if (!player) return [];
...
function opponentThreatScore(monsters: BloomrotCard[] = []) {
...
function countFaceUpBloomrots(cards: BloomrotCard[] = []) {
  return asArray(cards).filter((card) => isBloomrotMonster(card) && faceUp(card)).length;
}

function countBloomrotTokens(cards: BloomrotCard[] = []) {
...
function scoreSporeCounters(analysis: BloomrotAnalysis = {}) {
  let score = 0;
  const opponentCards = [
...
function scoreEngineCards(analysis: BloomrotAnalysis = {}) {
  let score = 0;
...
function scoreBloomrotBodies(analysis: BloomrotAnalysis = {}) {
...
function scoreBossAccess(analysis: BloomrotAnalysis = {}) {
  let score = 0;
  const field = asArray(analysis.field);
...
function scoreHandsAndGraveyard(analysis: BloomrotAnalysis = {}) {
  let score = 0;
...
function scorePenalties(analysis: BloomrotAnalysis = {}) {
  let penalty = 0;
  const field = asArray(analysis.field);
...
export function evaluateBloomrotBoardBonus(gameOrState: AIState, perspectivePlayer: AIStrategyBotPort | undefined) {
  const perspective = (perspectivePlayer?.id ? perspectivePlayer : gameOrState?.bot) as AIStrategyBotPort | undefined;
  const opponent = (
    gameOrState?._isPerspectiveState === true
      ...
      : gameOrState?.bot === perspective
        ...
        : gameOrState?.player === perspective
          ? gameOrState?.bot
          : gameOrState?.player) as AIStrategyBotPort | undefined;
...
export function evaluateBoardBloomrot(gameOrState: AIState, perspectivePlayer: SimulatedPlayerState | undefined, options: { baseScore?: number; baseEvaluator?(state: AIState, perspective?: SimulatedPlayerState): number } = {}) {
