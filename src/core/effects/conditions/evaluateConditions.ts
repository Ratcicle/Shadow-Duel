...
import type { EffectContext } from "../../contracts/actionRuntime.js";
import type { FilterCard, RuntimeCardFilter } from "../filters/cardFilters.js";
import type {
  ConditionCard,
  ConditionPlayer,
  ConditionContext,
  ConditionActivationContext,
  ConditionScope,
  ConditionAction,
  ConditionHost,
  ConditionGame,
...
function isSameCardReference(
  left: ConditionCard | null | undefined,
  right: ConditionCard | null | undefined,
) {
...
function resolveSourceRef(
  ...
  const value: unknown = (
    ctx as ConditionContext & { [reference: string]: unknown }
  )?.[sourceRef];
  return (
    Array.isArray(value) ? value[0] || null : value || null
  ) as ConditionCard | null;
}

function markerMatchesSource(
  marker: CardEffectMarker | null | undefined,
  source: ConditionCard | null | undefined,
) {
  ...
  return (
    marker?.sourceCardId !== undefined && marker.sourceCardId === source.id
  );
}
...
function readContextPath(ctx: ConditionContext, key: string | undefined) {
  ...
  const readPath = (root: unknown) => {
    ...
    for (const part of parts) {
      ...
      value = (value as { [path: string]: unknown })[part];
    }
    return value;
  };
  return (
...
function compareContextNumbers(current: number, op: string, expected: number) {
...
function getBattleParticipantForOwner(
  ctx: ConditionContext,
  owner: ConditionPlayer | null | undefined,
) {
  if (!ctx || !owner) return null;
  const attacker = ctx.attacker || null;
  const defender = ctx.defender || ctx.target || null;
  const controlsCard = (
    card: ConditionCard | null | undefined,
    explicitOwner: ConditionPlayer | null | undefined,
  ) =>
    !!card &&
    (explicitOwner === owner ||
      ...
      card.owner === owner.id);
  if (attacker && controlsCard(attacker, ctx.attackerOwner)) {
    return attacker;
  }
  if (
    defender &&
...
function getConditionOwners(
  ownerRule: string,
  player: ConditionPlayer | null | undefined,
  opponent: ConditionPlayer | null | undefined,
) {
  if (ownerRule === "opponent") return [opponent].filter((value) => !!value);
  if (ownerRule === "any" || ownerRule === "both") {
    return [player, opponent].filter((value) => !!value);
  }
  return [player].filter((value) => !!value);
}

function getConditionZoneCards(
  owner: ConditionPlayer | null | undefined,
  zoneKey: string,
) {
  ...
  const zone = owner[zoneKey as ConditionZone] || [];
  return Array.isArray(zone) ? zone.filter(Boolean) : [];
}

function getEventCardByRef(cond: RuntimeCondition, ctx: ConditionContext) {
  const ref = cond.cardRef || cond.eventCardRef || null;
  const eventCard =
    (ref &&
      (
        ctx as ConditionContext & {
          [reference: string]: ConditionCard | ConditionCard[] | undefined;
        }
      )?.[ref]) ||
    ctx?.destroyed ||
    ctx?.eventCard ||
...
function getCardPropertyValues(
  card: ConditionCard | null | undefined,
  property: string,
): readonly unknown[] {
  const value = (card as { [property: string]: unknown } | null | undefined)?.[
    property
  ];
...
function declarationIsActive(
  ...
  declaration: unknown,
): declaration is CardDeclaredValueDetail {
  if (!declaration || typeof declaration !== "object") return false;
  return (
    !Number.isFinite((declaration as CardDeclaredValueDetail).expiresOnTurn) ||
    !game ||
    Number(game.turnCounter || 0) <=
      ((declaration as CardDeclaredValueDetail).expiresOnTurn as number)
  );
}

function sourceMatchesDeclaredFilters(
...
function activeDeclarationSourcesForOwner(
  owner: ConditionPlayer | null | undefined,
) {
  if (!owner) return [];
  const sources: ConditionCard[] = [];
...
function findConditionCardOwner(
  game:
    | Pick<ConditionGame, "getOwnerByCard" | "effectEngine" | "player" | "bot">
    | undefined,
  card: ConditionCard | null | undefined,
) {
  ...
  const nestedEngine = game.effectEngine as
    | Pick<ConditionGame, "getOwnerByCard">
    | undefined;
  if (typeof nestedEngine?.getOwnerByCard === "function") {
    const owner = nestedEngine.getOwnerByCard(card);
    if (owner) return owner;
  }
  for (const owner of [game.player, game.bot]) {
    ...
    for (const zone of [
      ...
      const zoneCards = owner[zone as ConditionZone];
...
function cardVisibleToConditionViewer(
  engine: ConditionHost,
  card: ConditionCard | null | undefined,
  viewer: ConditionPlayer | null | undefined,
) {
...
function cardIsInAllowedConditionZones(
  engine: ConditionHost,
  card: ConditionCard,
  zones: string | readonly string[] | undefined,
) {
...
function appendUniqueCard(
  cards: ConditionCard[],
  card: ConditionCard | null | undefined,
) {
  if (!card) return;
  const key = getCardInstanceId(card) || card;
  if (cards.some((entry) => (getCardInstanceId(entry) || entry) === key)) {
    return;
  }
  cards.push(card);
}

function actionFilterFromConfig(config: ConditionScope = {}) {
  ...
  for (const key of [
    ...
    if (config[key] !== undefined && filters[key] === undefined) {
      // The key selects the same filter capability in both objects.
      Object.assign(filters, { [key]: config[key] });
    }
  }
  return filters;
}

function getConditionOwnersFromRule(rule: string, ctx: ConditionContext) {
  if (rule === "opponent") return [ctx.opponent].filter((value) => !!value);
  if (rule === "any" || rule === "both" || rule === "either") {
    return [ctx.player, ctx.opponent].filter((value) => !!value);
  }
  return [ctx.player].filter((value) => !!value);
}

function collectCardsFromScope(
  engine: ConditionHost,
...
function collectContextTargetCards(
  targetRef: string | undefined,
  ctx: ConditionContext = {},
) {
  if (!targetRef) return [];
  const refs: { [reference: string]: ConditionCard | null | undefined } = {
    self: ctx.source,
    source: ctx.source,
    target: ctx.target,
  ...
  if (targetRef === "battle_opponent") {
    if (ctx.source && ctx.source === ctx.attacker) {
      return [ctx.defender || ctx.target].filter((value) => !!value);
    }
    if (ctx.source && ctx.source === (ctx.defender || ctx.target)) {
      return [ctx.attacker].filter((value) => !!value);
    }
  }
  if (
    ctx?._actionTargets &&
    ...
    return asArray(ctx._actionTargets[targetRef]).filter((value) => !!value);
  }
  return asArray(refs[targetRef]).filter((value) => !!value);
}

function cardsFromSelectionValue(value: unknown) {
  const cards: ConditionCard[] = [];
  const visit = (entry: unknown): void => {
    ...
    // Envelope fields are unknown until recursively inspected; primitive reads stay inert.
    ...
    if (typeof entry === "object" && (selection.name || selection.cardKind)) {
      appendUniqueCard(cards, entry as ConditionCard);
    }
  };
  visit(value);
  return cards;
...
function collectSelectedTargetCards(
  targetRef: string | undefined,
  ...
  for (const selections of selectionSources) {
    ...
    for (const card of cardsFromSelectionValue(
      selections[targetRef as string],
    )) {
      appendUniqueCard(cards, card);
    }
  }
...
function collectTargetRefCandidateCards(
  engine: ConditionHost,
  targetRef: string | undefined,
  ...
  // Target resolution receives the same live action context, with preview flags.
  const targetResult: ConditionTargetResult = engine.resolveTargets(
    effect.targets,
    targetCtx as Parameters<ConditionHost["resolveTargets"]>[1],
    null,
  );
  // resolveTargets normalizes target envelopes to card references before returning.
  const resolved = targetResult?.targets?.[targetRef] as
    | ConditionCard
    | ConditionCard[]
    | null
    | undefined;
  ...
  for (const requirement of targetResult?.selectionContract?.requirements ||
    ...
    for (const candidate of requirement.candidates || []) {
      // Normalized candidates carry the live cardRef; card is the legacy alias.
      appendUniqueCard(
        cards,
        (candidate.cardRef || candidate.card) as
          | ConditionCard
          | null
          | undefined,
      );
...
function collectActionDestroyCandidates(
  engine: ConditionHost,
  input: unknown,
  ctx: ConditionContext,
  effect: EffectDefinition | null | undefined,
  ...
  const action = input as ConditionAction;
  const cards: ConditionCard[] = [];
...
function collectActionsDestroyCandidates(
  engine: ConditionHost,
  actions: unknown,
  ctx: ConditionContext,
  effect: EffectDefinition | null | undefined,
...
function conditionCardOwnerMatches(
  ...
  card: ConditionCard | null | undefined,
  ownerRule: string,
  responsePlayer: ConditionPlayer | null | undefined,
  responseOpponent: ConditionPlayer | null | undefined,
) {
...
function actionMovesToBanished(action: ConditionAction) {
...
function collectGraveyardBanishCandidates(
  engine: ConditionHost,
...
function collectActionBanishCandidates(
  ...
  const action = input as ConditionAction;
  const cards: ConditionCard[] = [];
  const addTargetRefCards = (targetRef: string | undefined) => {
    for (const card of collectTargetRefCandidateCards(
      engine,
      targetRef,
      effect,
...
function collectActionsBanishCandidates(
  engine: ConditionHost,
  actions: unknown,
  ctx: ConditionContext,
  effect: EffectDefinition | null | undefined,
...
function activationWouldDestroyCardsMatchingFilters(
  ...
  const actionCtx: ConditionContext = {
    ...ctx,
...
function normalizeDestinationZone(zone: string | null | undefined) {
  return zone === "banish" ? "banished" : zone;
}

function actionMovesToNonField(action: ConditionAction) {
  const toZone = normalizeDestinationZone(
    action?.to || action?.toZone || action?.destination || null,
  );
...
function activationWouldBanishCardsMatchingFilters(
  engine: ConditionHost,
  cond: RuntimeCondition,
  ctx: ConditionContext,
) {
...
function activationWouldMakeCardLeaveField(
  engine: ConditionHost,
  cond: RuntimeCondition,
  ctx: ConditionContext,
) {
  ...
  if (matching.length === 0) {
    return {
      ok: false,
      reason:
        cond.reason || "Activation would not make that card leave the field.",
    };
  }
  return { ok: true, matches: matching };
}

function buildTemporaryEffectSource(
  game: ConditionGame | undefined,
  entry: ConditionTemporaryEffect | null | undefined,
) {
  const data =
    game?.resolveCardData?.(entry?.sourceCardId) ||
    game?.resolveCardData?.(entry?.sourceName) ||
    {};
  const archetypes: readonly string[] = Array.isArray(entry?.sourceArchetypes)
  ...
  return {
    ...
    archetype:
      entry?.sourceArchetype || data?.archetype || archetypes[0] || null,
    archetypes,
  };
}
...
export function evaluateConditions(
  this: ConditionHost,
  conditions: unknown,
  context: EffectContext | ConditionContext,
): ConditionResult {
  // The legacy runtime carries replay and event fields beyond the action context port.
  const ctx = context as ConditionContext;
  ...
  for (const cond of conditions as readonly RuntimeCondition[]) {
    ...
    switch (cond.type) {
      case "context_number_compare": {
        ...
        const expected = Number(
          rawExpected ??
            cond.value ??
            cond.amount ??
            cond.defaultExpectedValue ??
            0,
        );
        if (
          !Number.isFinite(current) ||
          !Number.isFinite(expected) ||
          !compareContextNumbers(
            current,
            cond.op || cond.operator || "gt",
            expected,
          )
        ) {
          return {
      ...
      case "playerFieldCount": {
        ...
        if (cond.max !== undefined && count > cond.max) {
          return {
            ok: false,
            reason:
              cond.reason || `You must control at most ${cond.max} monster(s).`,
          };
        }
        break;
      }
      case "control_card": {
        ...
        const zone = owner?.[zoneName as ConditionZone] || [];
      ...
      case "destroyed_card_matches_declared_value": {
        ...
        if (
          Number.isFinite(
            (typeof declaration === "object" ? declaration : {}).expiresOnTurn,
          ) &&
          this.game &&
          this.game.turnCounter >
            ((typeof declaration === "object" ? declaration : {})
              .expiresOnTurn as number)
        ) {
          if (source?.declaredValues && stateKey)
        ...
        const actualValue: unknown = (
          destroyedCard as
            | (ConditionCard & { [property: string]: unknown })
            | null
        )?.[property];
        const actualValues: readonly unknown[] = Array.isArray(actualValue)
          ? actualValue
        ...
        if (
          !actualValues.includes(
            (typeof declaration === "object" ? declaration : {}).value,
          )
        ) {
          return {
            ok: false,
      ...
      case "battle_destroyer_matches_filters": {
        ...
        const battleDestroyers = Array.isArray(ctx?.battleDestroyers)
          ...
          : [ctx?.battleDestroyer, ctx?.attacker].filter((value) => !!value);

        const getController = (card: ConditionCard | null | undefined) => {
          if (!card) return null;
          ...
          if (card.controller === opponent?.id || card.owner === opponent?.id) {
            return opponent;
          }
          return null;
        };
        ...
        if (!found) {
          return {
            ok: false,
            reason: cond.reason || "No battle destroyer matched the condition.",
          };
        }
        break;
      }
      case "event_card_matches_filters": {
        ...
        if (ownerKey !== "any") {
          ...
          const eventOwner =
            ...
            null;
          const eventOwnerId =
            (typeof eventOwner === "object" ? eventOwner?.id : undefined) ||
            eventOwner ||
            card?.controller ||
            card?.owner;
      ...
      case "event_card_matches_declared_value_from_effect_sources": {
        ...
        const declarationMatches = (
          declaredValues: { [key: string]: unknown } | undefined,
        ) => {
          if (!declaredValues || typeof declaredValues !== "object") {
            return false;
          }
          const entries: [string, unknown][] = stateKey
        ...
        const owners = getConditionOwners(
          cond.owner || "self",
          player,
          opponent,
        );
        ...
        const temporaryEffects = Array.isArray(this.game?.temporaryEventEffects)
          ? (this.game.temporaryEventEffects as ConditionTemporaryEffect[])
          : [];
        const temporaryMatch = owners.some((owner) =>
          temporaryEffects.some((entry) => {
            ...
            if (
              ...
              this.game &&
              Number(this.game.turnCounter || 0) >
                (entry.expiresOnTurn as number)
            ) {
              return false;
            }
      ...
      case "battle_participant_matches_filters": {
        ...
        const participants = expectedOwner
          ...
          : [ctx?.attacker, ctx?.defender || ctx?.target].filter(
              (value) => !!value,
            );
      ...
      case "battle_opponent_matches_declared_value": {
        ...
        const actualValues = asArray(
          (battleOpponent as { [key: string]: unknown } | null)?.[property],
        );
        if (
          !actualValues.includes(
            (typeof declaration === "object" ? declaration : {}).value,
      ...
      case "summoned_card_has_marker": {
        ...
        if (
          ...
          this.game &&
          this.game.turnCounter > (marker.expiresOnTurn as number)
        ) {
          if (summonedCard?.effectMarkers && key)
      ...
      case "source_has_marker": {
        ...
        if (
          ...
          if (source?.effectMarkers && key) delete source.effectMarkers[key];
          return {
            ok: false,
        ...
        if (
          ...
          Number(marker.matchingCostCount || 0) <
            (cond.minMatchingCostCount as number)
        ) {
          return {
            ok: false,
      ...
      case "field_card_count": {
        ...
        let count = 0;
        for (const owner of getConditionOwners(
          cond.owner || "self",
          player,
          opponent,
        )) {
        ...
        if (cond.count !== undefined && count !== cond.count) {
          return {
            ok: false,
            reason:
              cond.reason || `Expected exactly ${cond.count} matching card(s).`,
          };
        }
        if (cond.min !== undefined && count < cond.min) {
          return {
            ok: false,
            reason:
              cond.reason || `Expected at least ${cond.min} matching card(s).`,
          };
        }
        if (cond.max !== undefined && count > cond.max) {
          return {
            ok: false,
            reason:
              cond.reason || `Expected at most ${cond.max} matching card(s).`,
          };
        }
        break;
      }
      case "field_card_count_comparison": {
        const zones =
          Array.isArray(cond.zones) && cond.zones.length > 0
            ? cond.zones
            : [cond.zone || "field"];
        const countFor = (ownerRule: string, filters: RuntimeCardFilter = {}) =>
        ...
        const zone =
          zoneName === "fieldSpell"
            ? owner?.fieldSpell
              ? [owner.fieldSpell]
              : []
            : owner?.[zoneName as ConditionZone] || [];
      ...
      case "control_card_filters": {
        ...
        const copyFilter = <K extends keyof RuntimeCardFilter>(
          key: K,
          value: RuntimeCardFilter[K],
        ) => {
        ...
        const matchesFilters = (card: ConditionCard | null | undefined) => {
          if (!card) return false;
          if (
            cond.excludeSource === true &&
            source &&
          ...
          const zone =
            zoneKey === "fieldSpell"
              ? owner?.fieldSpell
                ? [owner.fieldSpell]
                : []
              : owner?.[zoneKey as ConditionZone] || [];
      ...
      case "equipped_with_filters": {
        ...
        if (Number.isFinite(min) && count < min) {
          return {
            ok: false,
            reason:
              cond.reason || "This card is not equipped with a matching card.",
          };
        }
        if (max !== undefined && count > max) {
          return {
      ...
      case "has_stored_blueprint": {
        ...
        const storageState = this.getBlueprintStorageState?.(sourceCard, false);
      ...
      case "control_type_min_level": {
        ...
        const zone = player?.[zoneName as ConditionZone] || [];
      ...
      case "attacker_matches": {
        ...
        if (requiredType) {
          ...
          const aTypes = Array.isArray(attacker.types) ? attacker.types : null;
      ...
      case "source_counters_at_least": {
        ...
        if (count < min) {
          return {
            ok: false,
            reason:
              cond.reason || `Need at least ${min} ${counterType} counter(s).`,
          };
        }
        break;
      }
      case "field_counters_at_least": {
        ...
        for (const owner of owners.filter((value) => !!value)) {
          for (const zoneKey of zones) {
            const zone =
              zoneKey === "fieldSpell"
                ? owner.fieldSpell
