...
class BotLogger {
  ...
  declare enabledCategories: Set<string>;
  declare stats: { logsCounted: number; duplicatesDetected: number };
  constructor() {
  ...
  getLogLevel() {
    try {
      ...
      const level = parseInt(stored as string, 10);
   ...
   */
  shouldLog(botId: string, category: string) {
   ...
   */
  logActionGeneration(botId: string, turn: number, phase: string, hand: readonly unknown[], field: readonly unknown[], summonAvailable: boolean, generatedActions: readonly AIAction[]) {
   ...
   */
  logDecision(botId: string, turn: number, phase: string, totalActions: number, ranking: number, coherence: number, chosenAction: AIAction) {
   ...
   */
  logEmptyPhase(botId: string, turn: number, phase: string, reason: string, context?: BotLogContext) {
   ...
   */
  logPhaseTransition(botId: string, turn: number, fromPhase: string, toPhase: string, actionCount: number, timeMs: number) {
   ...
   */
  logStateChange(botId: string, action: string, before: BotLogState, after: BotLogState) {
   ...
   */
  logDuplicate(category: string, description: string, callCount: number, source: string) {
   ...
   */
  logCardEffect(botId: string, card: string, effectId: string, result: unknown) {
