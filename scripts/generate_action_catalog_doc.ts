...
function describeField(
  field: ActionFieldDefinition & { readonly default?: unknown } = {},
) {
  const parts = [];
...
function formatExamples(examples: readonly CardAction[] = []) {
...
function formatEntry(type: string, entry: CatalogEntry) {
  const fieldNames = [
